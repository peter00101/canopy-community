#!/usr/bin/env bun
/**
 * canopy — Canopy 命令行工具入口。
 *
 * 用法：
 *   canopy <command> [subcommand] [args] [--flags]
 *   canopy session list|info|outline|search|export ...
 *
 * 全局 flag：
 *   --json            输出机器可读 JSON
 *   --config-dir DIR  指定 Canopy 配置目录（默认 ~/.canopy，CANOPY_DEV=1 → ~/.canopy-dev）
 *   --dev             使用 ~/.canopy-dev
 *
...
function printHelp(): void {
  info(`canopy — ${CANOPY_BRAND.productName} 会话渐进式读取 CLI\n`)
  info('用法: canopy session <command> [args] [--flags]\n')
  info('命令:')
...
async function main(): Promise<number> {
  ...
  // 命名空间：当前只有 session，支持 `canopy session <cmd>` 与直接 `canopy <cmd>`
  let rest = argv
