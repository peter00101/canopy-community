---
name: in-app-browser
description: Canopy 内嵌受管浏览器使用指南。当用户要求打开、展示、访问、浏览或操作网页，或提到小红书、X/Twitter、LinkedIn、BOSS 直聘、登录后站内搜索、动态页面、截图或本地 HTML/React 预览时使用。对邮件、消息、文档、项目管理等已有匹配专用 MCP/API/CLI 的服务，必须优先使用专用工具；仅在没有匹配工具、工具无法完成当前能力、网络搜索工具不可用或无法取得足够好的结果、或用户明确要求网页时改用 Browser。浏览器工具出现在当前工具列表时，必须先阅读本 Skill 再进行网页操作；不要因为工具直接可见就跳过。
group: canopy
version: "1.1.2"
---

# Canopy In-App Browser

Canopy 的 `Browser*` 工具控制当前会话关联的受管浏览器。网页以应用内可见的原生 View 呈现；点击、输入和跳转都会留下状态与操作轨迹。浏览器 profile 仅持久保存在本机，并按工作区隔离。
...
- `BrowserNavigate`：打开 URL 或搜索查询；明确 URL、裸域名、localhost 和 IP 直达，普通文本使用必应（cn.bing.com）搜索；支持 `about:blank` 作为空白页。页面触发的下载会自动保存到受控下载目录（用 `BrowserListDownloads` 查看），popup 会留在受管浏览器标签中。
...
- `BrowserExportPdf`：把当前页面导出为 PDF，保存到受控下载目录并返回绝对路径；适合留存报告、文章或需要交给用户的页面快照。
- `BrowserListDownloads`：查看本会话的下载与导出记录（状态、保存路径、字节进度）。页面触发的下载（如 BrowserClick 点击下载按钮）是异步的，用它确认是否完成后再读取文件。
...
- 受管浏览器允许公网、本机 loopback 和局域网/其他私网地址；弹窗留在受管浏览器内，网页权限请求仍默认拒绝，不要尝试绕过这些边界。
- 页面触发的下载受控进行：文件统一保存到应用数据目录内的浏览器下载目录（文件名消毒、单文件上限 1GB），网页无法指定落盘路径。下载结果用 `BrowserListDownloads` 确认；不要下载与用户目标无关的文件。
