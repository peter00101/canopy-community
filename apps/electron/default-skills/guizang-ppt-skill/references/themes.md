...
5 套精心调配的主题色板,保证"电子杂志 × 电子墨水"的美学不垮。预设是快捷选项;用户有自己的配色或从 canopy-ppt 带来 `design.json` 时,按文末「自定义主题（派生规则）」算一套,门槛与预设同一水准。
...
- ❌ **不要拿用户给的 hex 直接往 :root 里塞**——先走文末的派生规则(底色够浅、字色够深),不达标就微调并说明,不要拒绝
...
来源:用户给的底色 / 字色,或 canopy-ppt 的 `design.json`(`colors.bg` 当底色、`colors.text` 当字色;bg 是深色时改用 `colors.surface`,仍然深就改推风格 B——电子墨水风必须浅底深字)。

亮度用 YIQ 估算:`Y = (R×299 + G×587 + B×114) / 1000`,范围 0～255。

| 变量 | 怎么算 | 门槛 |
|---|---|---|
| `--paper` | 底色原值 | Y ≥ 225(不够就把每个通道向 255 提,直到达标) |
| `--paper-rgb` | paper 的十进制三元组 | — |
| `--ink` | 字色原值 | Y ≤ 60(不够就把每个通道向 0 压,直到达标) |
| `--ink-rgb` | ink 的十进制三元组 | — |
| `--paper-tint` | paper 每通道减 8～10(略深一档,做区块底) | 仍 Y ≥ 215 |
| `--ink-tint` | ink 每通道加 12～16(略浅一档,做暗底区块) | 仍 Y ≤ 75 |

- 风格 A 没有 accent 变量,品牌色只能进 `--ink`:`colors.brand` 够深(Y ≤ 60)时可以直接当 ink;否则 ink 仍用字色,并在回复里说明「品牌色没进网页版,电子墨水风只有一种墨色」。
- 算完把六行**整体替换** `template.html` 的 `:root` 里标「主题色」的那几行,其余不动;回复里列出六个值和微调项。
- 例:远洋蓝 `design.json`(bg `#EAF1FB`,text `#0B2545`)→ paper `#EAF1FB`(Y≈240)、ink `#0B2545`(Y≈33)、paper-tint `#E0E7F1`、ink-tint `#1A3454`。
