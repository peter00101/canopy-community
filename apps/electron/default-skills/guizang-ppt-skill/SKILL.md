...
## 与 canopy-ppt 联动（收到 outline.json / design.json 时）

用户在 canopy-ppt 的 Step 0 选了「网页 PPT」，受众、页数、语言、大纲、风格都已经确认过，会带两份文件：
`ppt/<slug>/outline.json`（逐页 `title` / `key_message` / `content` / `visual` / `sources`）与 `design.json`
（`colors` 七色 token、`fonts`、`shape`、`footer`，可能还有 `background`）。接手规则：

- **不要重问 Step 1 的七问**。只剩「风格 A 还是 B」一项要定：先按 design.json 推断——`fonts.title` 含衬线
  （Noto Serif / Songti / SimSun）或风格卡是暖沙、素笺、松涛 → 风格 A；无衬线且风格卡是墨石、霓虹夜、远洋蓝 → 风格 B；
  用户模板包或旧 PPT 底图 → 风格 A（图片网格擅长）。推断结果用 `AskUserQuestion` 确认一次，然后直接进 Step 2。
- **页面映射**：outline 的每一页 = 一个 slide。封面 → 首页 hero；目录页 → 章节幕封或索引；`key_message` → 页标题大字；
  `content` 条目 → 要点或卡片；`visual` = 数字卡 → 数据大字报，对比 → 双栏，流程 / 时间线 → 对应布局，图表 → 数据大字报 + 简表，
  图片 → 图片网格或左文右图（图片只用用户给的，按下面「图片约定」）；`sources` 编号 → 页脚小字。页数与顺序不增不减。
- **主题色**：默认按 `design.json` 的七色**派生一套自定义主题**（规则在 themes*.md 末尾：风格 A 用 bg / text 派生六个变量，
  风格 B 用 bg / text / brand / line / muted 派生十一个变量，自带亮度与对比门槛），让网页版和 PPTX 版是同一套配色；
  回复里列出算出的变量与微调项。用户明确要预设时，才取与 `colors.brand` 最接近的一套；用户要重新一起设计配色也行（2.2 的共创）。`fonts` 不改模板字体；
  `footer.text` 进页脚；`background.type = image` 的底图不用（网页 PPT 自带 WebGL 背景），只借它的配色。
- **产物位置**：`ppt/<slug>/web/index.html`（与 canopy-ppt 的目录并列，`images/` 在同级），做完仍按 Step 4 自检、Step 5 预览，
  交付时写明主题色选择与没做的事，并提醒用户 canopy-ppt 那边的 `outline.json` 是唯一的内容来源，改内容改它再重生成。
...
#### 2.2 · 选定主题色(5 套预设，或按用户配色 / canopy-ppt 的 design.json 派生自定义)

预设是快捷选项，不是天花板。用户给了自己的颜色、或从 canopy-ppt 带来 `design.json` 时，按 `references/themes.md`（风格 A）/ `references/themes-swiss.md`（风格 B）末尾的「自定义主题（派生规则）」算出一套完整的 `:root` 变量——派生规则自带亮度与对比门槛，搭配不会垮；达不到门槛就按规则微调并说明，不要拒绝。
...
**操作**:
1. 用户给了主色 / 底色 / 字色，或从 canopy-ppt 带来了 `design.json` → **自定义**：按 themes*.md 末尾的派生规则算出整块 `:root` 变量（风格 A 六个、风格 B 十一个），下面第 3 步替换同样那几行。没给颜色 → 基于内容主题推荐一套预设，或直接问用户选哪一套；用户想要预设和派生之外的、或说「一起设计」→ **共创**：Agent 提 2～3 组原创配色（纸色 / 墨色 / 高亮色，各说一句气质），每组都按派生规则过门槛，用户挑一组或改，做一页预览再定
...
- 用户给的颜色先过派生规则的门槛（底色够浅、字色够深、高亮色与底色拉得开）；不达标就按规则微调，并在回复里写明改了什么、为什么，不要拒绝
- 不要混搭(例如 ink 取墨水经典、paper 取沙丘)——会彻底违和；从同一套配色派生出来的自定义主题不算混搭
...
- 不允许临时发明 `P23/P24`、`Swiss Image Split`、`Evidence Grid` 这类原始 22P 之外的正文结构,除非用户明确要求实验版式——用户说「想要不一样的版式」「一起设计一页」就算明确要求;做了照 Step 4 清单自检,交付时标「实验版式」。
...
└── references/
    ...
    ├── themes.md             ← 风格 A · 5 套主题色预设 + 自定义派生规则
    ├── themes-swiss.md       ← 风格 B · 4 套瑞士风主题色预设（IKB / 柠檬黄 / 柠檬绿 / 安全橙）+ 自定义派生规则
