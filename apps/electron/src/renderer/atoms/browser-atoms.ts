...
/**
 * 删除 / 归档会话时清掉它的全部受管浏览器 UI 状态（收上游 #2078）。
 *
 * 主进程删除会话时会关掉原生 WebContentsView 并广播关闭事件；但该事件可能因删除过程中的重渲染
 * 晚到或丢失，渲染层自己也要清，否则右侧工作区会继续渲染已删除会话的浏览器。
 * 归档不关主进程浏览器，这里清掉也无妨：再切回该会话时 MainArea 会经 getAgentBrowserState 重新同步。
 */
