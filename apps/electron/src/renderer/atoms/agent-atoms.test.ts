...
describe('右侧工作区组件', () => {
  ...
  test('Agent 变更只在来源 session 上加 Tab；非当前会话不抢焦点，改标未读', () => {
    const store = createStore()

    // 没设 currentAgentSessionIdAtom = 这条变更来自后台会话
    ...
    // 0.19.5 起（维护者点名「我选了改动就一直显示改动」）：后台会话不再被强行切标签，
    // 改为给该标签打未读点，用户切回去时才看得见
    expect(store.get(agentDiffPanelTabAtom).get('source-session')).toBeUndefined()
    expect(store.get(agentUnreadSidePanelTabsAtom).get('source-session')?.has('memory')).toBe(true)
    expect(store.get(agentDiffPanelTabAtom).get('other-session')).toBeUndefined()
    expect(store.get(agentUnreadSidePanelTabsAtom).get('other-session')).toBeUndefined()
  })

  test('当前会话且用户没手点过标签时，Agent 变更仍会带用户过去看一眼', () => {
    const store = createStore()
    store.set(currentAgentSessionIdAtom, 'source-session')
    ...
    // 抢到了焦点就不打未读
    expect(store.get(agentUnreadSidePanelTabsAtom).get('source-session')).toBeUndefined()
  })

  test('用户手点过标签后，同一会话的 Agent 变更一次都不许抢焦点', () => {
    const store = createStore()
    store.set(currentAgentSessionIdAtom, 'source-session')
    store.set(pinSidePanelTabByUserAtom, { sessionId: 'source-session', tab: 'changes' })
    store.set(agentDiffPanelTabAtom, new Map([['source-session', 'changes' as const]]))

    store.set(revealChangedWorkspaceComponentAtom, { sessionId: 'source-session', component: 'memory' })

    // 维护者的核心诉求：选了「改动」就一直是「改动」
    expect(store.get(agentDiffPanelTabAtom).get('source-session')).toBe('changes')
    expect(store.get(agentSessionComponentTabsAtomFamily('source-session'))).toEqual(['memory'])
    expect(store.get(agentUnreadSidePanelTabsAtom).get('source-session')?.has('memory')).toBe(true)
  })

  test('用户点进被标未读的标签后，未读点清除', () => {
    const store = createStore()
    store.set(currentAgentSessionIdAtom, 'source-session')
    store.set(pinSidePanelTabByUserAtom, { sessionId: 'source-session', tab: 'changes' })
    store.set(revealChangedWorkspaceComponentAtom, { sessionId: 'source-session', component: 'memory' })
    ...
    store.set(pinSidePanelTabByUserAtom, { sessionId: 'source-session', tab: 'memory' })
