...
import type { AgentSessionMeta, AgentEvent, AgentWorkspace, AgentPendingFile, RetryAttempt, AppPermissionMode, PermissionRequest, AskUserRequest, ExitPlanModeRequest, ThinkingConfig, AgentEffort, SDKMessage, UnstagedChangesResult } from '@canopy/shared'
import { CANOPY_DEFAULT_PERMISSION_MODE } from '@canopy/shared'
...
import {
  ...
  type AgentFocusStealSource,
} from '@/lib/agent-focus-steal'
...
export const agentSidePanelOpenMapAtom = atomWithStorage<Record<string, boolean>>(
  'canopy-agent-sidepanel-open-by-session',
  {},
  undefined,
  { getOnInit: true },
)
 ...
 * key 用我方历史键 canopy-agent-sidepanel-width（上游同位用的是它品牌下的 workspace-width 新键，
 * 我方用户从未写过那个键；跟上游换键会让存量用户的拖拽宽度全部回退默认）。
 */
const legacyAgentSidePanelWidthAtom = atomWithStorage<number>(
  'canopy-agent-sidepanel-width',
...
export const agentSidePanelLayoutMapAtom = atomWithStorage<Record<string, AgentSidePanelLayout>>(
  'canopy-agent-workspace-layout-by-session',
  {},
  undefined,
  { getOnInit: true },
)
...
export const agentFileSourceFilterMapAtom = atomWithStorage<Record<string, AgentFileSourceFilter>>(
  'canopy-agent-file-source-filter-map',
  {},
  undefined,
  { getOnInit: true },
)
...
export interface AgentExplorationBranchTab {
  /** Pi 原生 fork 生成的独立 Canopy session。 */
  sessionId: string
  /** 作为分叉锚点的 Canopy assistant message UUID。 */
...
// 入参与 isBrowserSidePanelTab 对齐：agentDiffPanelTabAtom 的值域含 'browser'，
// 调用方直接把 Map 里取出的标签传进来时不应被类型挡住。
export function getPreviewIdFromSidePanelTab(tab: AgentSidePanelTab | 'browser' | 'preview'): string | null {
...
export const agentSessionComponentOpenMapAtom = atomWithStorage<Record<string, WorkspaceComponentTab[]>>(
  'canopy-agent-session-component-tabs',
  {},
  undefined,
  { getOnInit: true },
)
...
/**
 * 右侧工作区标签的用户自定义顺序（按 Session）。
 * 只存 tabId 序列，运行期内存态——浏览器/终端等动态标签的 id 重启后即失效，
 * 与 splitMap 同理不做持久化；用户拖过之后当次会话内一直生效。
 */
export const agentRightWorkspaceTabOrderAtom = atom<Map<string, string[]>>(new Map())
...
export const agentSidePanelSplitRatioMapAtom = atomWithStorage<Record<string, number>>(
  'canopy-agent-workspace-split-ratio-by-session',
  {},
  undefined,
  { getOnInit: true },
)
...
export const openWorkspaceComponentAtom = atom(
  ...
  (get, set, component: WorkspaceComponentTab) => {
    ...
    // 用户主动打开项目组件：按手动表态锁死焦点
    set(pinSidePanelTabByUserAtom, { sessionId, tab: component })
...
export const revealChangedWorkspaceComponentAtom = atom(
  ...
  (get, set, { sessionId, component }: { sessionId: string; component: WorkspaceComponentTab }) => {
    ...
    // 早于通用判定的一条老规矩：正看着 Skills / 项目记忆时一律不抢（保留）
    const preservesUserFocus = panelOpen && isUserPriorityWorkspaceComponentTab(activeTab)
    if (preservesUserFocus) return

    // 其余交给统一判定：手点过就锁死、每 run 每类只抢一次
    set(tryStealSidePanelFocusAtom, { sessionId, source: 'workspace-component', tab: component })
  },
)
...
/**
 * 用户在该会话显式点过右侧工作区标签——点过之后 Agent 一次都不许抢焦点
 * （维护者 2026-09-20：「我选了改动，那就一直显示改动即可」）。判定见 `agent-focus-steal.ts`。
 * 只收用户的手动点击，Agent 自己切过去的不算，否则第一次被抢就把自己锁死了。
 */
export const agentUserPinnedSidePanelTabAtom = atom<Set<string>>(new Set<string>())

/**
 * 每个会话下「有动静但没抢焦点」的标签集合，用于在标签上标未读点。
 * 用户点进该标签即清除。
 */
export const agentUnreadSidePanelTabsAtom = atom<Map<string, Set<string>>>(new Map())

/** 记录某会话某来源在哪个 run 里已经抢过一次焦点，键由 `focusStealKey()` 生成。 */
export const agentFocusStolenRunAtom = atom<Map<string, string>>(new Map())

/**
 * Agent 侧的四条路径（浏览器 / 终端 / 项目组件 / 改动）唯一的「切标签」入口。
 *
 * 先问 `shouldStealSidePanelFocus`：准抢就展开面板并切过去、同时记账（本 run 该来源
 * 用掉一次机会）；不准抢就只给该标签打未读点。**不要再在别处直接 set
 * `agentDiffPanelTabAtom`**，否则又会出现绕过判定的第五条路径。
 *
 * @returns 是否真的抢到了焦点，调用方据此决定要不要做后续的聚焦动作。
 */
export const tryStealSidePanelFocusAtom = atom(
  null,
  (get, set, params: { sessionId: string; source: AgentFocusStealSource; tab: AgentSidePanelTab }): boolean => {
    const { sessionId, source, tab } = params
    const isCurrentSession = get(currentAgentSessionIdAtom) === sessionId
    // run 标识沿用渲染层既有口径：流式状态里的 startedAt
    const startedAt = get(agentSessionStreamingStateAtomFamily(sessionId))?.startedAt
    const runId = startedAt != null ? String(startedAt) : null
    const key = focusStealKey(sessionId, source)

    const stoleFocus = shouldStealSidePanelFocus({
      isCurrentSession,
      userPinnedTab: get(agentUserPinnedSidePanelTabAtom).has(sessionId),
      // 拿不到 runId（会话没在跑）时不按「已抢过」算，交给前两条判定把关
      alreadyStolenForRun: runId != null && get(agentFocusStolenRunAtom).get(key) === runId,
    })

    if (stoleFocus) {
      if (runId != null) {
        set(agentFocusStolenRunAtom, (previous) => {
          const next = new Map(previous)
          next.set(key, runId)
          return next
        })
    ...
    if (shouldMarkTabUnread({ stoleFocus })) {
      set(agentUnreadSidePanelTabsAtom, (previous) => {
        const current = previous.get(sessionId)
        if (current?.has(tab)) return previous
        const next = new Map(previous)
        next.set(sessionId, new Set(current ?? []).add(tab))
        return next
      })
    }
    return false
...
/**
 * 用户手点标签：锁死本会话的焦点，并清掉该标签的未读点。
 * tab 的值域与 `agentDiffPanelTabAtom` 对齐（含扁平化前遗留的 'browser' / 'preview'）。
 */
export const pinSidePanelTabByUserAtom = atom(
  null,
  (get, set, params: { sessionId: string; tab: AgentSidePanelTab | 'browser' | 'preview' }) => {
    const { sessionId, tab } = params
    set(agentUserPinnedSidePanelTabAtom, (previous) => {
    ...
    set(agentUnreadSidePanelTabsAtom, (previous) => {
      ...
      nextSet.delete(tab)
      const next = new Map(previous)
      if (nextSet.size === 0) next.delete(sessionId)
      else next.set(sessionId, nextSet)
      return next
    })
  },
)
...
/** 新会话默认权限模式 */
export const agentDefaultPermissionModeAtom = atom<AppPermissionMode>(CANOPY_DEFAULT_PERMISSION_MODE)

/** Per-session 权限模式 Map — sessionId → AppPermissionMode */
export const agentPermissionModeMapAtom = atom<Map<string, AppPermissionMode>>(new Map())

/**
 * 按 sessionId 派生该 session 的持久化权限模式。
 * 返回 `undefined`（session 不存在或未设置）或具体的 AppPermissionMode 字符串，
...
/** 上下文占用达到该比例时自动压缩；undefined = 用默认 0.8 */
export const agentAutoCompactionRatioAtom = atom<number | undefined>(undefined)

/** 提示缓存保留时长：'short' 5 分钟（默认）/ 'long' 1 小时；undefined = 未设置 = short */
export const agentPromptCacheRetentionAtom = atom<AgentPromptCacheRetention | undefined>(undefined)
