...
import { decodeDataUriToArrayBuffer } from '@/lib/audio-data-uri'

// UI SFX 采用 CC0 音频资产；每个 feel 只打包 Canopy 需要的四类语义 cue。
...
/** UI SFX 的 12 个音效 feel；每个 feel 对应四类 Canopy 通知场景。 */
 ...
 *
 * data: URI 直接在 JS 里解码：fetch/XHR 都受 CSP connect-src 管辖（白名单无
 * data:），构建产物若把小音频内联成 data: URI，走网络层必然双路被拦（0.17.43
 * 修复的「11 个小音效静默」）。根因侧 vite.config.ts 已禁止音频内联，此处兜底。
 */
async function loadAudioData(url: string): Promise<ArrayBuffer> {
  const inline = decodeDataUriToArrayBuffer(url)
