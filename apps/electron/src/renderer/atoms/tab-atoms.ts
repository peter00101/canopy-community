...
export const TUTORIAL_TAB_TITLE = `${CANOPY_BRAND.productName} 使用教程`
...
export const sidebarCollapsedAtom = atomWithStorage<boolean>(
  'canopy-sidebar-collapsed',
  false,
)

/**
 * 当前的收起是否由比例系统在紧凑档自动做的（持久化，与上面成对）。
 * 折叠态本身重载后还在，但「是谁折的」这个来历若不一起存，重载后会被当成用户折的：
 * 窗口拉宽时不再自动展开，右面板也会把 rail 当成用户让位而吃掉全部宽度（AppShell）。
 */
export const sidebarAutoCollapsedAtom = atomWithStorage<boolean>(
  'canopy-sidebar-auto-collapsed',
  false,
)
