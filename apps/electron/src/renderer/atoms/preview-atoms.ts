...
export interface PreviewFile {
  ...
  /** 候选基础目录（用于相对路径解析） */
  basePaths?: string[]
  /** 仅由用户在文件面板 / 聊天附件等可信 UI 中主动选择的外部文件可放宽预览范围。 */
...
/** 分栏比例（对话占比），持久化 */
export const previewSplitRatioAtom = atomWithStorage<number>('canopy-preview-split-ratio', 0.5, undefined, { getOnInit: true })

/** 代码预览换行偏好（默认不换行，保持现有横向滚动行为） */
export const previewCodeWrapAtom = atomWithStorage<boolean>(
  'canopy-preview-code-wrap',
  false,
  undefined,
  { getOnInit: true },
)
