 ...
 *
 * 按钮来源始终用于显示录音状态；只有本次输出应写回 Canopy 时，它才同时成为文本目标。
...
export function resolveVoiceDictationSessionInputIds(
  ...
  const targetInputId = routeToAppInput
...
export const VOICE_DICTATION_INSERT_EVENT = 'canopy:insert-voice-dictation-text'

/** 语音识别过程中的组合文本预览事件。 */
export const VOICE_DICTATION_PREVIEW_EVENT = 'canopy:preview-voice-dictation-text'

/** 取消语音输入时撤销组合文本预览。 */
export const VOICE_DICTATION_CLEAR_PREVIEW_EVENT = 'canopy:clear-voice-dictation-preview'

/** 底部输入工具栏显示的语音状态。 */
export const VOICE_DICTATION_STATUS_EVENT = 'canopy:voice-dictation-status'
