...
// Canopy
import AppLogo from '@/assets/models/canopy.png'
 ...
 */
export function resolveModelDisplayName(modelId: string, channels: import('@canopy/shared').Channel[]): string {
 ...
 */
export function resolveModelProvider(modelId: string, channels: import('@canopy/shared').Channel[]): ProviderType | undefined {
  for (const channel of channels) {
    for (const model of channel.models) {
      if (model.id === modelId) {
