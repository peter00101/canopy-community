...
describe('quoted selection XML', () => {
  ...
  test('Given Agent 引用与存量草稿引用 When 解析引用块 Then 旧 scratch-pad 来源回落 agent-history 且保留展示标签', () => {
    const content = [
    ...
    expect(parsed.quotes).toEqual([
      ...
      },
      {
        path: '草稿页',
        filename: '草稿页',
        // 上游 v0.19.1 随 Vault 移除 scratch-pad 来源类型；存量会话中的旧引用回落 agent-history，展示标签不变
        sourceType: 'agent-history',
        label: '草稿页',
      },
    ])
