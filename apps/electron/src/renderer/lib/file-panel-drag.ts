 ...
 *
 * 载荷结构：
 *   application/x-canopy-file-panel → JSON string of FilePanelDragItem[]
 */

export const FILE_PANEL_DRAG_MIME = 'application/x-canopy-file-panel'

/**
 * 三点菜单「引用到 Agent」触发的事件名。
 * detail 携带 FilePanelDragItem[]；AgentView 监听后调用 RichTextInput.insertFileMentions。
 */
export const INSERT_FILE_MENTION_EVENT = 'canopy:insert-file-mention'
