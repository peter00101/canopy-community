 ...
 * `localStorage.setItem('app-performance-debug', '1')`.
 * When enabled, `window.__appPerformance.snapshot()` exposes aggregate metrics
...
export function isPerformanceMonitoringEnabled(): boolean {
  ...
  return new URLSearchParams(window.location.search).get('perf') === '1'
    || window.localStorage.getItem('app-performance-debug') === '1'
}
