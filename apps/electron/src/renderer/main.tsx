...
import {
  ...
  agentMaxTurnsAtom,
  agentAutoCompactionRatioAtom,
  agentPromptCacheRetentionAtom,
...
import type { FeishuBotBridgeState, FeishuBridgeState, DingTalkBotBridgeState, DingTalkBridgeState } from '@canopy/shared'
import { Toaster } from './components/ui/sonner'
import { toast } from 'sonner'
import { diffCapabilities } from '@canopy/shared'
...
if (isMainWindow || isWorkspaceMemoryWindow) {
  document.documentElement.classList.add('canopy-main-window')
}
...
function ThemeInitializer(): null {
  ...
  useEffect(() => {
    ...
    // Windows：主题生效后把实际配色同步到系统标题栏按钮（WCO）
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [themeSignature])

  useEffect(() => {
    applyInterfaceVariantToDOM(interfaceVariant)
    // 经典/现代切换会改变标题栏背景（--sidebar-surface vs --background），同样要跟
  }, [interfaceVariant])
...
function AgentSettingsInitializer(): null {
  ...
  const setAutoCompactionRatio = useSetAtom(agentAutoCompactionRatioAtom)
  const setPromptCacheRetention = useSetAtom(agentPromptCacheRetentionAtom)
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (settings.agentAutoCompactionRatio != null) {
        setAutoCompactionRatio(settings.agentAutoCompactionRatio)
      }
      if (settings.agentPromptCacheRetention != null) {
        setPromptCacheRetention(settings.agentPromptCacheRetention)
      }
  ...
  }, [setAgentChannelId, setAgentModelId, setAgentWorkspaces, setCurrentWorkspaceId, setThinking, setEffort, setMaxBudget, setMaxTurns, setAutoCompactionRatio, setAutomationGroupOrder, setChannels, setChannelsLoaded, setAgentSettingsReady])
...
function UiPreferencesInitializer(): null {
  ...
  const setAgentAutoOpenChangesPanelEnabled = useSetAtom(agentAutoOpenChangesPanelEnabledAtom)
  ...
  useEffect(() => {
    initializeUiPreferences(
      ...
      setAgentAutoOpenChangesPanelEnabled,
      setProductivityTools,
    )
  }, [setLongTextPasteAsAttachmentEnabled, setRichTextRenderingEnabled, setSessionHoverPreviewEnabled, setAgentAutoOpenChangesPanelEnabled, setProductivityTools])
...
// 根级错误边界（RootErrorBoundary）：兜住局部边界之外的渲染错误，给恢复页而不是整窗白屏。
// 语音听写指示器、Agent 状态悬浮窗是透明小浮层，整页恢复页放不下也无意义，不包。
// ===== 快速任务窗口：轻量渲染 =====
if (isQuickTaskWindow) {
  import('./components/quick-task/QuickTaskApp').then(({ QuickTaskApp }) => {
    ReactDOM.createRoot(document.getElementById('root')!).render(
      <React.StrictMode>
        <RootErrorBoundary scope="quick-task">
          <ThemeInitializer />
          <QuickTaskApp />
        </RootErrorBoundary>
      </React.StrictMode>
  ...
  import('./components/diff/DetachedPreviewApp').then(({ DetachedPreviewApp }) => {
    ReactDOM.createRoot(document.getElementById('root')!).render(
      <React.StrictMode>
        <RootErrorBoundary scope="detached-preview">
          <ThemeInitializer />
  ...
  import('./components/agent-skills/WorkspaceMemoryWindowApp').then(({ WorkspaceMemoryWindowApp }) => {
    ReactDOM.createRoot(document.getElementById('root')!).render(
      <React.StrictMode>
        <RootErrorBoundary scope="workspace-memory" windowChrome={<WindowControls />}>
          <ThemeInitializer />
  ...
  // ===== 主窗口：完整渲染 =====
  // 初始化组件也放在边界内：它们渲染期抛错同样会卸载整棵树。主题 class 已写在 <html> 上，
  // 边界接管后恢复页仍沿用当前深浅主题；Windows 无边框窗口在恢复页里重新挂自绘窗口控件。
  ReactDOM.createRoot(document.getElementById('root')!).render(
    <React.StrictMode>
      <RootErrorBoundary scope="main" windowChrome={<WindowControls />}>
        <ThemeInitializer />
