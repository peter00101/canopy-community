...
import { RAIL_HUE_PALETTE, withRailHues } from '@/lib/rail-session-glyph'
import { AgentMascotIcon } from '@/components/ui/agent-mascot-icon'
...
import { paletteToneTextClass, paletteToneTextClassForId, type PaletteTone } from '@/lib/palette-tone'
...
import type { ConversationMeta, AgentSessionMeta, AgentWorkspace, WorkspaceCapabilities } from '@canopy/shared'
...
interface WorkspaceComponentSidebarEntryProps {
  ...
  /** 图标从主题色族里取哪一档（配色体系）：四个入口各占一色，同一调性、彼此可辨 */
  tone?: PaletteTone
}

/** 左侧项目级组件的单行入口；每项只打开一个对应的右侧 Tab。 */
function WorkspaceComponentSidebarEntry({ label, icon, active, onClick, badge, tone }: WorkspaceComponentSidebarEntryProps): React.ReactElement {
  return (
    <button
      type="button"
      aria-label={label}
    ...
    >
      <span className="flex min-w-0 items-center gap-3">
        <span className={cn('flex size-[18px] shrink-0 items-center justify-center', active ? 'text-accent-foreground' : tone ? paletteToneTextClass(tone) : 'text-[hsl(var(--sidebar-primary-foreground))]')}>
          {icon}
        </span>
...
const SESSION_QUICK_SWITCH_KEYDOWN_EVENT = 'canopy:session-quick-switch-keydown'
const SESSION_QUICK_SWITCH_KEYUP_EVENT = 'canopy:session-quick-switch-keyup'
...
/** 折叠栏最多同时显示的会话数：5 → 8（维护者 2026-08-30 定），恰等于色块调色板大小，可见范围内颜色严格不重复 */
const RAIL_RECENT_LIMIT = RAIL_HUE_PALETTE.length
...
// 侧栏顶缘的细拖拽条。mac 上侧栏已整体位于 MacTitleRow 之下，与 Windows 一样只留细条，
// 不再为红绿灯留白——红绿灯那一行与侧栏互不影响。
...
interface RailRecentItem {
  ...
  title: string
  type: SessionMiniMapType
  /** 会话创建时间：色块去重按它的稳定顺序进行，显示顺序怎么变颜色都不动 */
  createdAt: number
  /** 色块色相（度），由 rail-session-glyph 按会话 id 分配并在可见集合内去重 */
  hue: number
  active: boolean
...
function RailRecentButton({
  ...
  onTogglePin: (item: RailRecentItem) => void
  onToggleArchive: (item: RailRecentItem) => void
  onRequestDelete: (item: RailRecentItem) => void
  ...
  return (
    <ContextMenu>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            />
            {/* 固定图标 + 按会话着色的小色块（维护者的用户嫌首字母图标难看，2026-08-30 改）：
                Agent 用 Bot、Chat 用 MessageSquare；自动任务 / 委派会话沿用各自的图标，只换底色 */}
            <span
              className="rail-session-glyph flex size-7 items-center justify-center rounded-[9px]"
              data-active={item.active ? 'true' : undefined}
              style={{ '--rail-hue': item.hue } as React.CSSProperties}
            >
              {item.isAutomation
      ...
      {/* 折叠态右键菜单（维护者 2026-08-30 要求：收起状态也要能删会话）：置顶 / 归档 / 删除，
          与展开列表同一套处理函数；重命名需要行内编辑框，折叠态不提供 */}
      <ContextMenuContent className="w-36 z-[9999] min-w-0 p-0.5">
        <ContextMenuItem className="text-xs py-1 [&>svg]:size-3.5" onSelect={() => onTogglePin(item)}>
          {item.pinned ? <PinOff size={14} /> : <Pin size={14} />}
          {item.pinned ? '取消置顶' : '置顶'}
        </ContextMenuItem>
        <ContextMenuItem className="text-xs py-1 [&>svg]:size-3.5" onSelect={() => onToggleArchive(item)}>
          <Archive size={14} />
          归档
        </ContextMenuItem>
        <ContextMenuSeparator className="my-0.5" />
        <ContextMenuItem className="text-xs py-1 [&>svg]:size-3.5 text-destructive" onSelect={() => onRequestDelete(item)}>
          <Trash2 size={14} />
          {item.type === 'agent' ? '删除会话' : '删除对话'}
        </ContextMenuItem>
      </ContextMenuContent>
      <SessionMiniMapPopover
        target={{
...
export function LeftSidebar({ width, noTransition }: LeftSidebarProps): React.ReactElement {
  ...
  const cleanupMapAtoms = React.useCallback((id: string) => {
    ...
    // 受管浏览器 UI 状态：主进程的关闭事件可能因删除过程中的重渲染晚到，
    // 不能让右侧工作区继续渲染已删除会话的浏览器（收上游 #2078）
  ...
  const handleSelectAgentSession = React.useCallback((id: string, title: string): void => {
    ...
    if (selectedSession?.sourceDelegationId && selectedSession.parentSessionId) {
      ...
      if (parentSession) {
        ...
        // 用户从侧栏点开委派子会话：按手动表态锁死该父会话的焦点
        store.set(pinSidePanelTabByUserAtom, { sessionId: parentSession.id, tab: getDelegationSidePanelTab() })
  ...
  const railRecentItems = React.useMemo(() => {
    if (mode === 'chat') {
      return withRailHues(conversations
        ...
        // 上面的优先级排序只决定「谁进前 8」；显示顺序与展开列表一致按最近更新，
        // 点击切换不再把当前会话跳到最前（维护者 2026-08-30：跳位 + 变色让人找不到上一个会话）
    ...
    return withRailHues(agentSessions
      .filter((session) =>
        !session.archived
        && !session.isDraft
        && !draftSessionIds.has(session.id)
    ...
    >
      ...
      >
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            将删除「{pendingDeleteWorkspace?.name ?? '该项目'}」在 {CANOPY_BRAND.productName} 中保存的会话、自动任务、MCP、Skills 与 {CANOPY_BRAND.productName} 工作区配置；空白项目的 {CANOPY_BRAND.productName} 托管项目文件也会被删除。本地项目根目录、附加目录和附加文件只会移除关联，不会删除原始文件。删除后无法恢复。
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
              ...
              <div className="group/project relative flex items-center">
                ...
                >
                  {group.kind === 'automation' ? (
                    ...
                    <FolderOpen size={14} className={cn('flex-shrink-0', paletteToneTextClassForId(group.workspace?.id ?? group.id))} />
                  )}
      ...
      if (!collapsed) {
        if (visible.treeItems.length === 0) {
          rows.push({
            ...
            estimateSize: 28,
            content: <div className="ml-6 py-0.5 text-[12px] text-muted-foreground/75 select-none">暂无会话</div>,
          })
        } else {
  ...
  // ===== 折叠状态：精简图标视图 =====
  if (sidebarCollapsed) {
    // 折叠态 rail 与 Windows 同宽 60：红绿灯已住进上方独立的 MacTitleRow，
    // 不再需要那套「rail 装下灯组」的 96 = 18 + 60 + 18（该占位现在由标题行承担）。
    const collapsedRailWidth = 60
    ...
    return (
      <div
        ...
        style={{ width: collapsedRailWidth, flexShrink: 0 }}
      >
        <SidebarWindowDragStrip height={SIDEBAR_DRAG_STRIP_HEIGHT.collapsed} />

        {/* 顶部细留白，可拖拽。mac 不再为红绿灯留 40px——它们在上方的 MacTitleRow 里。
            现代风格下细留白隐藏、展开钮所在的 .sidebar-rail-top 撑成 48px 第二层顶栏（与展开态的 .sidebar-top-row 同高同色）。 */}
        <div className="sidebar-top-strip h-2 w-full flex-shrink-0 titlebar-drag-region" />

        {/* 折叠态将会话放在主路径：控件维持 40px 热区，但把视觉体积收至 32px。 */}
        <div className="flex flex-col items-center gap-0.5">
          <div className="sidebar-rail-top flex w-full flex-col items-center">
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                type="button"
          ...
          <div className="flex flex-col items-center gap-0.5 pb-2">
            {railRecentItems.map((item) => (
              <RailRecentButton
                ...
                onTogglePin={(target) => {
                  void (target.type === "agent" ? handleTogglePinAgent(target.id) : handleTogglePin(target.id));
                }}
                onToggleArchive={(target) => {
                  void (target.type === "agent" ? handleToggleArchiveAgent(target.id) : handleToggleArchive(target.id));
                }}
                onRequestDelete={(target) => handleRequestDelete(target.id)}
              />
            ))}
          </div>
        </div>
                ...
                <>
                  ...
                  >
                    ...
                    {(capabilities?.skills.filter((skill) => skill.hasUpdate)
                      .length ?? 0) > 0 && (
                      <span className="ml-auto size-2 rounded-full bg-info" />
                    )}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
  ...
  return (
    ...
    >
      ...
      {/* 顶部细留白，可拖拽。mac 不再为红绿灯留 40px——它们在上方的 MacTitleRow 里。
          现代风格下由 .sidebar-top-strip 规则隐藏：顶行整体并入 48px 的第二层顶栏（globals.css「两层顶栏」）。 */}
      <div className="sidebar-top-strip h-1 w-full flex-shrink-0 titlebar-drag-region" />

      {/* 模式切换器 + 折叠按钮；现代风格下 .sidebar-top-row 规则把它撑成 48px、居中、铺第二层底色并画底线 */}
      <div className="sidebar-top-row titlebar-drag-region flex items-start gap-1.5 px-3">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              ...
              {/* 240px 最窄侧栏 + 右侧快捷键徽标挤压时不换行（小屏实测「新建任/务」折行） */}
              <span className="whitespace-nowrap">{mode === 'agent' ? '新建任务' : '新建对话'}</span>
      ...
      <div className="space-y-0.5 px-3 pb-0.5">
        ...
        {mode === 'agent' && (
          <WorkspaceComponentSidebarEntry
            ...
            badge={(capabilities?.skills.filter((skill) => skill.hasUpdate).length ?? 0) > 0 ? <span className="size-2.5 rounded-full bg-info" /> : undefined}
          />
        )}
      </div>
...
const SESSION_ACCENT_ROW_CLASS: Record<SessionLeftAccent, string> = {
  orange: 'bg-warning-soft text-foreground font-medium',
    ...
    >
      ...
      <div className="group/project relative flex items-center">
        {renamingWorkspace ? (
          ...
          >
            <FolderOpen size={14} className={cn('flex-shrink-0', paletteToneTextClassForId(group.workspace.id))} />
            <input
