...
import { findBestSearchMatch } from '@canopy/shared'
...
function SearchResultIcon({ result }: { result: SearchResult }): React.ReactElement {
  return result.type === 'chat' ? (
    ...
    <Bot size={14} className="flex-shrink-0 text-info/70" />
  )
}
...
export function SearchDialog(): React.ReactElement {
  ...
  const handleAgentSearch = React.useCallback(async () => {
    ...
    const configDir = import.meta.env.DEV ? CANOPY_BRAND.configDirNameDev : CANOPY_BRAND.configDirName
    const prompt = `请帮我在 ${CANOPY_BRAND.productName} 的全部会话历史中搜索与以下描述相关的内容：
      ...
      >
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50">
          ...
          <button
            ...
            title={`适合在精准搜索找不到的情况下使用，Agent 会帮助你搜索整个 ${CANOPY_BRAND.productName} 会话空间`}
            className={cn(
              'flex shrink-0 items-center gap-1 px-2 py-1 rounded text-[12px] font-medium transition-colors',
        ...
        <div className="relative">
          ...
          {hasSearched && !loading && allResults.length === 0 && (
            <div className="py-8 flex flex-col items-center gap-3 text-[13px] text-foreground/40">
              ...
              <button
                onClick={() => void handleAgentSearch()}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[12px] font-medium bg-info-soft text-info hover:bg-info/20 transition-colors"
              >
                <Bot size={12} />
