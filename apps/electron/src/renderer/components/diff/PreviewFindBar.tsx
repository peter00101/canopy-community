...
import { buildPreviewFindMatcher, type PreviewFindOptions } from '@/lib/preview-find-matcher'

/**
 * 内容不在本页 DOM 里时的查找实现（PDF 在隔离 iframe 里、大表格是虚拟滚动只渲染可见行）。
 * 查找栏的界面、快捷键、计数与上下跳转不变，只把「搜索 / 定位 / 清除」交给提供者。
 */
export interface PreviewFindProvider {
  /** 执行搜索并返回命中总数；query 已去首尾空白且非空，正则已校验合法 */
  search: (query: string, options: PreviewFindOptions) => Promise<number>
  /** 把第 index 个命中滚到可见处并标为当前 */
...
interface PreviewFindBarProps {
  ...
  /** 提供时改走提供者搜索，不再在 rootRef 的 DOM 里插高亮 */
  provider?: PreviewFindProvider
  onOpenChange: (open: boolean) => void
}

type FindOptions = PreviewFindOptions

/** 提供者搜索（PDF 要先抽全文）比 DOM 高亮重，输入时稍等再搜 */
const PROVIDER_SEARCH_DEBOUNCE_MS = 180

const MATCH_SELECTOR = 'mark[data-canopy-find-match]'
const SHADOW_STYLE_ID = 'canopy-find-highlight-style'
...
const buildMatcher = buildPreviewFindMatcher
...
function injectShadowStyle(root: ParentNode): void {
  ...
  style.textContent = `
    ...
    ${MATCH_SELECTOR}[data-canopy-find-active="true"] {
...
function shouldSkipTextNode(node: Text): boolean {
  ...
  if (parent.closest('[data-canopy-find-ignore]')) return true
...
function markMatchesInTextNode(node: Text, matcher: RegExp): HTMLElement[] {
  ...
  for (const item of matches) {
    ...
    mark.dataset.appFindMatch = 'true'
...
function applyHighlights(container: HTMLElement, query: string, options: FindOptions): HTMLElement[] {
  ...
  marks.forEach((mark, index) => {
    mark.dataset.appFindIndex = String(index)
  })
  return marks
}
...
function setActiveMatch(marks: HTMLElement[], activeIndex: number): void {
  marks.forEach((mark, index) => {
    if (index === activeIndex) {
      mark.dataset.appFindActive = 'true'
      applyMatchStyle(mark, true)
      mark.scrollIntoView({ block: 'center', inline: 'nearest' })
    } else {
      delete mark.dataset.appFindActive
      applyMatchStyle(mark, false)
    }
  })
}

export function PreviewFindBar({ open, rootRef, contentKey, unsupportedReason, provider, onOpenChange }: PreviewFindBarProps): React.ReactElement | null {
  ...
  const usesProvider = Boolean(provider)
  /** 提供者搜索是异步的：只认最后一次发起的结果，旧结果回来直接丢掉 */
  const searchSeqRef = React.useRef(0)
  /** 提供者模式下最近一次已定位的命中，避免计数更新时重复滚动 */
  const activatedIndexRef = React.useRef(-1)
  ...
  const clearMarks = React.useCallback(() => {
    ...
    activatedIndexRef.current = -1
    setSearching(false)
    setMatchCount(0)
    setActiveIndex(-1)
  }, [rootRef, usesProvider, withObserverPaused])

  const runSearch = React.useCallback(() => {
    const root = rootRef.current
    if (!open || unsupportedReason) return
    if (!usesProvider && !root) return
    ...
    if (usesProvider) {
      ...
      setSearching(true)
      void current.search(trimmed, options).then((count) => {
        if (seq !== searchSeqRef.current) return
        const nextActiveIndex = count === 0
          ? -1
          : activeIndexRef.current >= 0
            ? Math.min(activeIndexRef.current, count - 1)
            : 0
        activeIndexRef.current = nextActiveIndex
        activatedIndexRef.current = nextActiveIndex
        setSearching(false)
        setMatchCount(count)
        setActiveIndex(nextActiveIndex)
        if (nextActiveIndex >= 0) current.activate(nextActiveIndex)
      }).catch((error: unknown) => {
        if (seq !== searchSeqRef.current) return
        console.warn('[preview-find] 提供者搜索失败:', error)
        setSearching(false)
        setMatchCount(0)
        setActiveIndex(-1)
      })
    ...
    setActiveIndex(nextActiveIndex)
  }, [clearMarks, open, options, query, rootRef, unsupportedReason, usesProvider, withObserverPaused])
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(runSearch, query.trim() ? PROVIDER_SEARCH_DEBOUNCE_MS : 0)
    return () => window.clearTimeout(timer)
  }, [open, runSearch, clearMarks, contentKey, unsupportedReason, usesProvider, query])

  React.useEffect(() => {
    if (!open || unsupportedReason || usesProvider) return
    const root = rootRef.current
    if (!root) return
  ...
  }, [open, rootRef, runSearch, unsupportedReason, usesProvider])
  ...
  React.useEffect(() => {
    if (usesProvider) {
      if (activeIndex < 0 || activeIndex >= matchCount || activeIndex === activatedIndexRef.current) return
      activeIndexRef.current = activeIndex
      activatedIndexRef.current = activeIndex
      providerRef.current?.activate(activeIndex)
      return
    }
  ...
  }, [activeIndex, matchCount, usesProvider, withObserverPaused])
