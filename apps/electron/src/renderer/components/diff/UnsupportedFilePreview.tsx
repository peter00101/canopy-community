...
import type { FileAccessOptions, FilePreviewMetadata } from '@canopy/shared'
import { buildUnsupportedPreviewHeading } from '@/lib/preview-load-state'
  ...
  return (
    <div className="flex h-full items-center justify-center px-6 py-10 text-center">
      <div className="w-full max-w-sm rounded-xl bg-muted/35 p-5 shadow-[0_1px_2px_rgba(0,0,0,0.06),0_8px_24px_rgba(0,0,0,0.05)]">
        ...
        {/* 标题跟着 reason 走：找不到文件不能说成「不安全」（0.18.16） */}
        <h2 className="mt-3 text-sm font-medium text-foreground text-balance">{buildUnsupportedPreviewHeading(reason)}</h2>
        {/* 「未找到文件」提示带换行与完整路径，需保留换行并允许长路径任意断行 */}
        <p className="mt-1.5 text-[13px] leading-5 text-muted-foreground text-pretty whitespace-pre-line [overflow-wrap:anywhere]">{reason}</p>
