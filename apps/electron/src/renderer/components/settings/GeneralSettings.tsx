...
import {
  ...
  agentAutoOpenChangesPanelEnabledAtom,
  ...
  updateAgentAutoOpenChangesPanelEnabled,
...
import { BROWSER_SEARCH_ENGINE_OPTIONS, DEFAULT_BROWSER_SEARCH_ENGINE, isBrowserSearchEngineId, type BrowserSearchEngineId } from '@canopy/shared'
...
export function GeneralSettings(): React.ReactElement {
  ...
  const [agentAutoOpenChangesPanelEnabled, setAgentAutoOpenChangesPanelEnabled] = useAtom(agentAutoOpenChangesPanelEnabledAtom)
  ...
  /** 受管浏览器默认搜索引擎：默认必应（国内可达），可切谷歌；Agent 搜索跟随同一选择 */
  const [browserSearchEngine, setBrowserSearchEngine] = React.useState<BrowserSearchEngineId>(DEFAULT_BROWSER_SEARCH_ENGINE)
  const [browserAutoApprove, setBrowserAutoApprove] = React.useState(DEFAULT_BROWSER_AUTO_APPROVE)
  ...
  React.useEffect(() => {
    window.electronAPI.getSettings().then((settings) => {
      ...
      setBrowserSearchEngine(isBrowserSearchEngineId(settings.browserSearchEngine) ? settings.browserSearchEngine : DEFAULT_BROWSER_SEARCH_ENGINE)
      setBrowserAutoApprove(settings.browserAutoApproveEnabled ?? DEFAULT_BROWSER_AUTO_APPROVE)
    }).catch(console.error)
  }, [])
  ...
  /** 切换受管浏览器操作免确认 */
  const handleBrowserAutoApproveChange = async (next: boolean): Promise<void> => {
    const previous = browserAutoApprove
    setBrowserAutoApprove(next)
    try {
      await window.electronAPI.updateSettings({ browserAutoApproveEnabled: next })
    } catch (error) {
      console.error('[通用设置] 更新浏览器免确认失败:', error)
      setBrowserAutoApprove(previous)
    }
  }

  /** 更新受管浏览器默认搜索引擎 */
  const handleBrowserSearchEngineChange = async (value: string): Promise<void> => {
    const next = isBrowserSearchEngineId(value) ? value : DEFAULT_BROWSER_SEARCH_ENGINE
    const previous = browserSearchEngine
    setBrowserSearchEngine(next)
    try {
      await window.electronAPI.updateSettings({ browserSearchEngine: next })
    } catch (error) {
      console.error('[通用设置] 更新浏览器搜索引擎失败:', error)
      setBrowserSearchEngine(previous)
    }
  }
    ...
    <div className="space-y-6">
      ...
      >
        <SettingsCard>
          ...
          <SettingsToggle
            label="Obsidian"
            description="在侧栏与菜单中显示 Obsidian 入口（关闭仅隐藏入口，Agent 仍可使用已配置的 Vault）"
          ...
          <div className="mx-4 mb-3 flex items-start justify-between gap-3 rounded-lg bg-muted/35 px-3 py-2.5 text-xs text-muted-foreground">
            <div className="flex min-w-0 flex-1 items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5">{CANOPY_BRAND.productName} 非常喜欢这个音效库，并特别还原了一部分 UI SFX 的设计风格，推荐大家访问和使用他们的产品。</p>
              </div>
            </div>
            <a
          ...
          <SettingsToggle
            label="浏览器操作免确认"
            description="关闭受管浏览器、把本机文件选进网页上传框时不再逐次询问。关掉后恢复每次确认；定时任务与子 Agent 始终不能关闭浏览器"
            checked={browserAutoApprove}
            onCheckedChange={(checked) => {
              void handleBrowserAutoApproveChange(checked)
            }}
          />
          <SettingsRow
            label="浏览器搜索引擎"
            description="受管浏览器新建空白标签页的首页与搜索词使用的引擎，Agent 搜索也跟随此设置"
          >
            <Select value={browserSearchEngine} onValueChange={handleBrowserSearchEngineChange}>
          ...
          <SettingsToggle
            label="改动文件时自动打开侧面板"
            description="Agent 写入或修改文件时自动弹出右侧面板并切到「改动」标签；关闭后改动仍会记录在「改动」标签里，只是不再自动弹出"
            checked={agentAutoOpenChangesPanelEnabled}
            onCheckedChange={(checked) => {
              setAgentAutoOpenChangesPanelEnabled(checked)
              updateAgentAutoOpenChangesPanelEnabled(checked)
            }}
          />
          {isMac && (
            <SettingsToggle
          ...
          <SettingsToggle
            label="Git/PR 标识"
            description="Agent 代你提交 commit 或创建 PR 时，可附加署名信息；可随时关闭"
