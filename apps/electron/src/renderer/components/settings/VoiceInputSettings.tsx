...
const OUTPUT_OPTIONS = [
  { value: 'auto', label: `自动：${CANOPY_BRAND.productName} 激活时写入对话框，否则写入当前光标` },
  { value: 'clipboard', label: '仅复制到剪贴板' },
  { value: 'app-input', label: '仅写入应用输入框' },
]
...
export function VoiceInputSettings(): React.ReactElement {
    ...
    try {
      ...
      if (result.status === 'granted') {
        ...
        if (result.platform === 'win32') {
          // Windows 无授权弹窗，request 已打开系统麦克风隐私设置页
          toast.info('已打开系统麦克风设置，请开启"麦克风访问"并允许桌面应用访问，然后回来重试')
        } else {
  ...
  return (
    <div className="space-y-6">
      <SettingsSection
        title="豆包流式语音输入"
        description={`通过全局快捷键唤起浮窗，实时识别语音，停止后写入 ${CANOPY_BRAND.productName} 输入框或当前光标位置。`}
        action={
          <Button
            variant="outline"
            size="sm"
      ...
      >
                ...
                <div>
                  ...
                  <span className="ml-2 text-muted-foreground">
                    {micPermission.status === 'granted'
                      ? '已授权，语音输入可正常使用'
                      : micPermission.status === 'denied'
                      ? micPermission.platform === 'win32'
                        ? '已被系统阻止，请在 Windows 设置中开启"麦克风访问"并允许桌面应用访问'
                        : `已被系统阻止，请在系统设置中允许 ${CANOPY_BRAND.productName} 访问麦克风`
                      : micPermission.status === 'not-determined'
                      ? '未授权，使用语音输入前需要先授权'
                      : '当前系统不支持预检，录音时将直接尝试采集麦克风'}
                  </span>
                </div>
              </div>
        ...
        <SettingsCard>
          ...
          <SettingsTextarea
            ...
            placeholder={`${CANOPY_BRAND.productName}\nJotai\nShadcnUI\nClaude Code`}
            minHeight={112}
          />
          <SettingsSelect
            label="输出方式"
            description={`默认写入当前光标位置；如果唤起时 ${CANOPY_BRAND.productName} 是当前激活窗口，会写入当前 Chat 或 Agent 输入框。自动粘贴失败时会保留到剪贴板。`}
