...
/**
 * 所有可选供应商。'qwen-anthropic' 仅为兼容存量 Anthropic 渠道保留，不出现在新建下拉（上游 #1766 主次对调）；
 * 'doubao-api' 属不采的上游 #1735 拆分，我方 'doubao' 就是 /api/v3 按量计费。
 */
const PROVIDER_OPTIONS: ProviderType[] = [
  'anthropic', 'anthropic-compatible', 'openai', 'openai-responses', 'openai-codex', 'xai', 'deepseek', 'google', 'kimi-api', 'kimi-coding', 'opencode-go-openai', 'zhipu', 'zhipu-coding', 'zhipu-coding-team', 'ark-coding-plan', 'minimax', 'doubao', 'qwen', 'qwen-token-plan', 'xiaomi', 'xiaomi-token-plan', 'custom',
]
...
export function ChannelForm({ channel, onSaved, onCancel }: ChannelFormProps): React.ReactElement {
  ...
  // 锁死供应商的 baseUrl 始终取锁定地址，忽略存储值
  const [baseUrl, setBaseUrl] = React.useState(() => {
    const initialProvider = channel?.provider ?? 'anthropic'
    return getLockedChannelBaseUrl(initialProvider)
      ?? channel?.baseUrl
      ?? PROVIDER_DEFAULT_URLS.anthropic
  })
  ...
  // 锁死供应商：Base URL 不可编辑，表单隐藏输入框
  const isLockedBaseUrlProvider = getLockedChannelBaseUrl(provider) !== null
  ...
  /** 添加模型（id/名称剥除不可见字符——用户从网页粘贴的 id 可能带零宽或 bidi 控制符） */
  const handleAddModel = (): void => {
    const id = stripInvisibleCharacters(newModelId)
    if (!id) return

    const name = stripInvisibleCharacters(newModelName)
  ...
  /**
   * 离开输入框时把通用兼容渠道的 Base URL 补成完整请求端点，并**回写输入框让用户看见**。
   *
   * `custom` / `anthropic-compatible` 的存储值是「完整请求地址，原样使用」语义（见
   * `url-utils.ts` 的刻意契约：中转站端点路径未必标准，解析期静默追加会把本来能用的渠道
   * 改坏）。但只填协议根同样是错的——0.18.67 维护者报障即此：填成裸域名后主对话仍能用
   * （Pi SDK 自己拼端点），自动命名却打到网关首页拿回 HTML，静默失败。
   *
   * 补全放在 blur 而不是解析期：用户打完字才动、结果可见、要用非标准路径改回去即可，
   * 逃生口与既有契约都不破。`migrateCompatibleChannelBaseUrl` 幂等，重复 blur 不会叠加后缀。
   */
  const handleBaseUrlBlur = (): void => {
    const completed = migrateCompatibleChannelBaseUrl(baseUrl, provider)
    if (completed !== baseUrl) {
      setBaseUrl(completed)
      toast.info('已补全为完整请求端点，如网关路径不同可直接改回', { id: 'base-url-completed' })
    }
    if (requiresBaseUrlRiskAcknowledgement) {
      ...
      <SettingsSection title="基本信息">
        <SettingsCard>
          ...
          <SettingsInput
            ...
            required
          />
          {/* 订阅 provider 的请求地址由 Pi SDK 内置管理；锁死供应商不可编辑地址 */}
          {!isSubscriptionProvider && !isLockedBaseUrlProvider && (
            <SettingsInput
          ...
          <div className="px-4 py-3 space-y-2">
            ...
            {isCodexProvider ? (
              <div className="space-y-2">
                ...
                ) : <div className="text-xs text-muted-foreground">{CANOPY_BRAND.productName} 会代理 token 请求；系统浏览器授权页仍需使用可访问 OpenAI 的网络。可改用设备码并在另一台设备完成授权。</div>}
              </div>
            ) : isXaiProvider ? (
              <div className="space-y-2">
                <Button
                ...
                ) : <div className="text-xs text-muted-foreground">{CANOPY_BRAND.productName} 会代理设备码与 token 请求；授权页由系统浏览器打开。若浏览器无可用代理，可扫码在另一台设备完成授权。</div>}
              </div>
            ) : isZhipuTeamProvider ? (
              <div className="space-y-2">
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2">
                ...
                <p>{CANOPY_BRAND.productName} 仅作为本地 Agent 执行环境：配置、会话等本地数据均存储在你的设备上，{CANOPY_BRAND.productName} 本身不会额外构成数据风险。</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
