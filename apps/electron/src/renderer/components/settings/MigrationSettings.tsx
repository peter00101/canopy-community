...
const ARCHIVE_MIGRATION_PROMPT = `请帮我创建一个可迁移的 ${CANOPY_BRAND.productName} 数据压缩包。

${CANOPY_BRAND.productName} 的本地数据通常存放在 ~/${CANOPY_BRAND.configDirName}。请按以下步骤处理：

1. 先确认当前 ${CANOPY_BRAND.productName} 数据文件夹的位置、计划生成的 ZIP 路径，以及压缩包是否可能包含会话记录、工作区配置和本地文件。
2. 在开始压缩前向我展示范围并征得确认；不要删除、移动或修改原始数据文件夹。
3. 将完整的 ${CANOPY_BRAND.configDirName} 数据文件夹压缩为一个 ZIP 文件，并告诉我生成路径和文件大小。
4. 提醒我将 ZIP 通过可信方式传输到新设备，并在新设备的 ${CANOPY_BRAND.productName} 对话中附上该 ZIP，执行恢复、项目路径分配和索引重建。
5. 不要尝试导出系统钥匙串、OAuth 登录或其他系统级凭据；这些内容需要在新设备上重新登录或配置。`

const RESTORE_MIGRATION_PROMPT = `我正在恢复来自另一台设备的 ${CANOPY_BRAND.productName} 数据，并已附上旧设备 ${CANOPY_BRAND.configDirName} 文件夹的 ZIP 压缩包。

请按以下步骤处理：

1. 先检查 ZIP 的内容，并说明将要写入的此设备 ${CANOPY_BRAND.productName} 数据目录以及可能覆盖的文件；在任何覆盖前征得我的确认，并为现有数据创建可恢复备份。
2. 将压缩包解压到此设备的 ${CANOPY_BRAND.productName} 数据目录，按当前版本的数据结构完成必要迁移。
...
export function MigrationSettings(): React.ReactElement {
  ...
  return (
    <div className="space-y-8">
      <SettingsSection
        title="迁移原理"
        description={`${CANOPY_BRAND.productName} 的本地数据通常存放在 ${CANOPY_BRAND.configDirName} 文件夹。迁移时先压缩该文件夹，再在新设备上由 ${CANOPY_BRAND.productName} 解压、分配项目路径并重建索引。`}
      >
        <ol className="space-y-3 text-sm leading-6 text-muted-foreground">
          <li className="flex gap-3">
            ...
            <span>在当前设备将完整的 {CANOPY_BRAND.configDirName} 数据文件夹压缩为 ZIP，原始数据保持不变。</span>
          </li>
          <li className="flex gap-3">
            ...
            <span>将 ZIP 通过可信方式传输到新设备，并附到任意一个 {CANOPY_BRAND.productName} 对话中。</span>
          </li>
          <li className="flex gap-3">
            ...
            <span>新设备上的 {CANOPY_BRAND.productName} 解压数据、重新分配本地项目目录，并重建会话、工作区和文件索引。</span>
          </li>
        </ol>
      </SettingsSection>
      ...
      <SettingsSection
        title="当前设备：创建迁移压缩包"
        description={`先打开 ${CANOPY_BRAND.productName} 数据文件夹；随后可将提示词粘贴到任意 ${CANOPY_BRAND.productName} 对话，由 Agent 协助创建 ZIP。`}
      >
      ...
      <SettingsSection
        title={`新设备：恢复 ${CANOPY_BRAND.productName} 数据`}
        description={`在新设备的任意 ${CANOPY_BRAND.productName} 对话中附上 ZIP，再粘贴以下提示词完成解压、项目分配和索引重建。`}
      >
      ...
      <div className="rounded-lg border border-warning/25 bg-warning-soft px-4 py-3 text-sm leading-6 text-warning dark:text-warning">
