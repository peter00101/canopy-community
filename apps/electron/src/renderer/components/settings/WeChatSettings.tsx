...
import type { WeChatBridgeStatus } from '@canopy/shared'
...
export function WeChatSettings(): React.ReactElement {
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsSection
        title="微信集成"
        description={`扫码登录微信，在微信中控制 ${CANOPY_BRAND.productName} Agent`}
      >
        <SettingsCard>
        ...
        {bridgeState.status === 'error' && bridgeState.errorMessage && (
          <div className="mt-2 px-3 py-2.5 rounded-lg bg-danger-soft text-danger dark:text-danger text-sm">
        ...
        {/* 连接成功提示 */}
        {isConnected && (
          <div className="mt-2 px-3 py-2.5 rounded-lg bg-success-soft text-success dark:text-success text-sm">
            微信已连接，消息将自动接收。
          </div>
        )}
      </SettingsSection>
          ...
          <div className="px-4 py-4 space-y-5 text-sm">
            ...
            <div className="space-y-1.5">
              ...
              <p className="pl-7 text-muted-foreground">
                点击上方「扫码登录」，用微信扫描二维码。
                这会将你的微信账号作为 Bot 接入 {CANOPY_BRAND.productName}。
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="pl-7 text-muted-foreground">
                扫码成功后，{CANOPY_BRAND.productName} 会自动建立长连接。
                凭证会加密保存，下次启动 {CANOPY_BRAND.productName} 时自动重连。
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="pl-7 text-muted-foreground">
                连接成功后，通过微信发送消息即可与 {CANOPY_BRAND.productName} Agent 交互。
                支持文本、图片、文件等消息类型。
              </p>
            </div>
