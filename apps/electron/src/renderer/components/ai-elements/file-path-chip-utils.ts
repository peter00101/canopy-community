/**
 * ⚠️ **本文件在我方已是死代码，零 import 站点（0.18.66 复核）。**
 *
 * 文件路径 chip 的纯逻辑早已下沉到 `@/lib/file-reference`（那边有 file-reference.test.ts），
 * `file-path-chip.tsx` / `message.tsx` 用的都是那一份。上游若再改这个文件（如 #2020 的
 * `isRelativeFilePath` 放宽与 `isLocalFileReference`），移植目标一律是 `@/lib/file-reference`，
 * 不要往这里搬——否则会造出两份口径不一致的判定。本文件可安全删除。
 */
