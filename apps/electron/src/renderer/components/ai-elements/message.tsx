 ...
 * AI Elements - 消息组件原语
 *
 * 简化迁移自 canopy-frontend 的 ai-elements/message.tsx，
...
import { getLocalImagePathCandidates } from '@/components/markdown/markdown-image-src'
import { resolveLocalImageUrl } from '@/components/markdown/local-image-url'
...
import { CodeBlock, HtmlPreviewBlock, MermaidBlock } from '@canopy/ui'
import { detectLanguage } from '@canopy/core'
import { FilePathChip, isAbsoluteFilePath, isImageFilePath, isLocalFileReference, isRelativeFilePath } from './file-path-chip'
import { BasePathsContext, BasePathsProvider } from './base-paths-context'
import { resolveInlineFileReference } from '@/lib/file-reference'
...
const MENTION_STYLES: Record<MentionType, { icon: typeof FileText; className: string }> = {
  ...
  todo: { icon: ListTodo, className: 'bg-warning-soft text-warning dark:text-warning' },
  calendar_event: { icon: CalendarDays, className: 'bg-info-soft text-info dark:text-info' },
...
// BasePathsContext / BasePathsProvider 已移到 ./base-paths-context（FilePathChip 也要读它，
// 留在本文件会与 message.tsx → file-path-chip.tsx 的既有 import 形成循环）。此处继续导出，
// 既有 import 站点（AgentMessages 等）不受影响。
export { BasePathsProvider }
...
/**
 * 会话级「文件名 → 绝对路径」映射上下文 — 由 AgentMessages 基于已落盘的全部消息提供。
 * 本轮映射只覆盖当前 turn；Agent 隔几轮再裸提早先写过的文件（典型：长期记忆目录里的
 * `xxx.md`）时本轮映射命不中，只能拿裸名去候选目录猜、猜不到就误报「文件不存在」
 * （上游 issue #1691）。查找顺序：本轮映射 → 会话映射 → 裸名交给主进程按候选目录解析。
 * 同名不同目录的文件同样从映射剔除，不比补全前更差。
 */
const SessionFileMapContext = React.createContext<Map<string, string> | undefined>(undefined)

/** 提供会话级文件名→绝对路径映射给所有内嵌的 MessageResponse */
export function SessionFileMapProvider({ map, children }: { map?: Map<string, string>; children: React.ReactNode }): React.ReactElement {
  return <SessionFileMapContext.Provider value={map}>{children}</SessionFileMapContext.Provider>
}
...
const MarkdownLink = React.memo(function MarkdownLink({
  ...
  // 相对链接（`[报告](workspace-files/plan/report.md)`）也要能点开预览，需要消息级候选根
  // 才拼得出绝对路径；主进程再按显式授权根校验（上游 #2020）。
  const contextBasePaths = React.useContext(BasePathsContext)
 ...
 * Agent 回复正文里的图片。
 *
 * 渲染层没有文件权限，`![](workspace-files/chart.png)` 这类本地 src 必须先经
 * `file:resolve-path` 换成 `canopy-file://` URL 才显示得出来。0.18.68 之前 markdown
 * 组件映射里根本没有 `img`，本地图片一律碎图（上游 #2030 同源缺陷，我方自修）。
 *
 * 授权边界仍由主进程按 sessionId + 候选目录判定（显式根集合），渲染层只递候选。
 * sessionId 与 FilePathChip 同源取自 `currentAgentSessionIdAtom`，不改 BasePathsProvider
 * 签名——那份上下文单独成文件是为打断循环 import（见 base-paths-context.tsx）。
 * 解析结果不跨组件缓存：`canopy-file://` 令牌 TTL 仅 1 小时，长开会话重绘时旧 URL 会 404。
 */
const MarkdownImage = React.memo(function MarkdownImage({
  ...
  const decodedSrc = typeof src === 'string' ? safeDecode(src) : ''
  // 本地形态判定与 LiveMarkdown 预览同一份真源：外链 / data / blob / canopy-file 直接交给 <img>
  const isLocalSource = decodedSrc.length > 0 && getLocalImagePathCandidates(decodedSrc).length > 0
  ...
  React.useEffect(() => {
    ...
    void resolveLocalImageUrl(decodedSrc, {
  ...
  }, [contextBasePaths, decodedSrc, isLocalSource, store])
  ...
  if (resolvedSrc) {
    return (
      <img
        ...
        alt={alt}
        {...imageProps}
        onError={() => { setResolvedSrc(null); setFailed(true) }}
      />
    )
  }
  return (
    <span
      className="inline-flex max-w-full items-center rounded bg-foreground/10 px-2 py-1 text-xs text-muted-foreground"
      role="img"
      aria-label={alt || decodedSrc}
    >
...
const MarkdownPre = React.memo(function MarkdownPre({
  ...
  if (codeChild) {
    ...
    // HTML / SVG 代码块：「源码/预览」一键切换（sandbox iframe 渲染）
    const artifactLang = /\blanguage-html\b/.test(className)
      ? 'html' as const
      : /\blanguage-svg\b/.test(className)
        ? 'svg' as const
        : null
    if (artifactLang) {
      const artifactCode = extractText(codeProps.children).replace(/\n$/, '')
      if (artifactCode.trim()) {
        return <HtmlPreviewBlock code={artifactCode} language={artifactLang} onCopy={copyTextToClipboard} />
      }
    }
...
const MarkdownInlineCode = React.memo(function MarkdownInlineCode({
  ...
  // 会话级映射：本轮命不中时兜底（跨轮次引用早先写过/读过的文件）
  const sessionFileMap = React.useContext(SessionFileMapContext)
  if (codeClassName) {
  ...
  if (text) {
    ...
    if (merged.length > 0 && isRelativeFilePath(text)) {
      // 命中本轮 / 会话实际触及文件的映射时，用绝对路径替换裸文件名（保留行号后缀），
      // 使内联引用与 footer chip 走同一条可靠解析；未命中则维持原样降级。
      const abs = resolveInlineFileReference(text, turnFileMap, sessionFileMap)
      if (abs) {
        return <FilePathChip filePath={abs} basePaths={merged} />
      }
...
export const MessageResponse = React.memo(
  function MessageResponse({ children, className, basePath, basePaths, remarkPlugins }: MessageResponseProps): React.ReactElement {
    ...
    const renderedMarkdown = (remarkPlugins?.includes(remarkMentions)
      ...
      : children
    ).replace(/<!--CANOPY_AUTOMATION:[\s\S]*?-->/g, '').trim()
...
/** 文件附件展示（标签样式，teal 色调）；点击解析本地路径并在预览面板打开 */
function MessageAttachmentFile({ attachment }: MessageAttachmentFileProps): React.ReactElement {
  ...
  const handleOpenPreview = React.useCallback((): void => {
    ...
    window.electronAPI.resolveAttachmentPath(attachment.localPath)
      .then((absolutePath) => {
        if (!absolutePath) {
          toast.warning(`附件文件已不存在：${attachment.filename}`)
          return
        }
        const lastSep = Math.max(absolutePath.lastIndexOf('/'), absolutePath.lastIndexOf('\\'))
        const parentPath = lastSep > 0 ? absolutePath.slice(0, lastSep) : undefined
        // 轻量对话没有右侧分屏宿主，强制 Tab 模式；scope 用会话/对话 ID
        openPreview(conversationId ?? 'chat-attachments', {
      ...
      .catch(() => toast.warning(`无法打开附件：${attachment.filename}`))
  }, [attachment.filename, attachment.localPath, conversationId, openPreview])

  const clickable = Boolean(attachment.localPath)

  return (
    <div
      className={cn(
        'flex items-center gap-2 rounded-lg bg-[#37a5aa]/10 border border-[#37a5aa]/20 px-3 py-1.5 text-[13px] text-[#37a5aa] shrink-0',
        clickable && 'cursor-pointer transition-colors hover:bg-[#37a5aa]/15',
      )}
      onClick={clickable ? handleOpenPreview : undefined}
      ...
      title={clickable ? `预览 ${attachment.filename}` : attachment.filename}
    >
