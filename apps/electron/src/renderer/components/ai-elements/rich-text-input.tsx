...
export const RichTextInput = forwardRef<RichTextInputHandle, RichTextInputProps>(function RichTextInput({
  ...
  // 手动高度：null = 自动模式（沿用三档 max-height）；拖拽/键盘调整后为固定像素高度
  ...
  // 是否显示折叠按钮：启用 collapsible 且内容已自动扩展（手动高度模式下隐藏，避免两套高度机制打架）
  const showCollapseToggle = collapsible && isExpanded && manualHeight === null

  // ===== 高度拖拽（参考 Cherry Studio Composer） =====

  const handleHeightDragStart = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    if (disabled) return
    const area = scrollAreaRef.current
    if (!area) return
    event.preventDefault()
    const startHeight = manualHeight ?? area.getBoundingClientRect().height
    heightDragRef.current = { pointerId: event.pointerId, startY: event.clientY, startHeight }
    event.currentTarget.setPointerCapture(event.pointerId)
    setIsDraggingHeight(true)
    setIsManuallyCollapsed(false)
  }, [disabled, manualHeight])

  const handleHeightDragMove = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    const drag = heightDragRef.current
    if (!drag || drag.pointerId !== event.pointerId) return
    setManualHeight(computeDragHeight(drag.startHeight, drag.startY, event.clientY, window.innerHeight))
  }, [])

  const handleHeightDragEnd = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    const drag = heightDragRef.current
    if (!drag || drag.pointerId !== event.pointerId) return
  ...
  const handleHeightKeyDown = useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    const area = scrollAreaRef.current
    if (!area) return
    if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
      event.preventDefault()
      const current = manualHeight ?? area.getBoundingClientRect().height
      setManualHeight(stepInputHeight(current, event.key === 'ArrowUp' ? 1 : -1, window.innerHeight))
      setIsManuallyCollapsed(false)
    } else if (event.key === 'Escape' && manualHeight !== null) {
      event.preventDefault()
  ...
  return (
    ...
    >
      {/* 高度拖拽条 — 悬浮在输入区顶部，不占布局空间；双击或 Esc 恢复自动高度 */}
      <div
        role="separator"
        aria-orientation="horizontal"
        aria-label="调整输入框高度"
        aria-valuemin={INPUT_HEIGHT_MIN}
        aria-valuemax={resolveInputHeightMax(window.innerHeight)}
        aria-valuenow={manualHeight ?? undefined}
        tabIndex={disabled ? -1 : 0}
        className={cn(
          'group absolute -top-[5px] left-0 right-0 z-20 h-[10px] cursor-row-resize touch-none',
      ...
      >
        <div
          className={cn(
            'mx-auto mt-[3.5px] h-[3px] w-10 rounded-full bg-muted-foreground/40 transition-opacity',
            isDraggingHeight ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100'
          )}
        />
      </div>
      <div
        ...
        className={cn(
          ...
          isDraggingHeight ? 'transition-none' : 'transition-[max-height,height] duration-200 ease-in-out',
          manualHeight === null && (
            isManuallyCollapsed
              ? 'max-h-[101px]'
        ...
        style={manualHeight !== null ? { height: manualHeight, maxHeight: manualHeight } : undefined}
        onMouseDown={(event) => {
          // 手动高度下内容不满会留白：点击空白区把光标落到文末
          if (event.target === scrollAreaRef.current && editor && !disabled) {
            event.preventDefault()
      ...
      <style>{`
        ...
        .ProseMirror p.is-editor-empty:first-child::before {
          ...
          /* 绝对定位替代 float+height:0：同样不占布局高度，且窄输入框时按单行省略截断，
             不再折成两行把首行提示顶乱（小屏实测）。宽度够时视觉与原先完全一致。 */
          position: absolute;
          left: 0;
          right: 0;
          top: 0;
