...
import { NATIVE_OVERLAY_ATTR } from '@/lib/dialog-safe-area'
...
export function BrowserSlot({ sessionId, tabId }: { sessionId: string; tabId: string }): React.ReactElement {
  ...
  // data-native-overlay：这个 div 的矩形就是原生视图的边界，居中弹窗据此让位（0.18.67）。
  return (
    <div
      ref={ref}
      className="flex-1 min-h-0 bg-muted/15 titlebar-no-drag"
      aria-label="受管浏览器页面"
      {...{ [NATIVE_OVERLAY_ATTR]: 'browser' }}
    />
  )
}
