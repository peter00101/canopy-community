...
import type { PlanningGroup, Todo } from '@canopy/shared'
    ...
    <section ref={todoWorkspaceRef} className="relative h-full min-h-[500px] overflow-hidden rounded-none border border-border/60 bg-card">
      <div className={cn(
        ...
        {selected && <PlanningFloatingInspector inline={layoutMode !== 'single-column'} fullWidth={layoutMode === 'single-column'} label="Todo 详情" onClose={() => { saveDetailNotes(); saveDetailTitle(); setSelectedId(null) }}><div className="space-y-4 p-4">
          {todoConflict && <div className="flex items-center justify-between gap-3 rounded-md border border-warning/35 bg-warning-soft px-3 py-2 text-xs text-warning dark:text-warning"><span>此 Todo 已在其他窗口更新，请重新加载后再编辑。</span><Button type="button" variant="link" size="sm" className="h-auto shrink-0 px-0 text-xs" onClick={reloadTodoDetail}>重新加载</Button></div>}
...
const TodoListItem = React.memo(function TodoListItem({
  ...
  const dueTone = isOverdue ? 'text-danger dark:text-danger' : todo.dueAt !== undefined && todo.dueAt <= todayEnd ? 'text-warning dark:text-warning' : 'text-muted-foreground'
...
function TodoMetaBadge({
  ...
  const toneClass = tone === 'overdue' ? 'bg-danger-soft text-danger dark:text-danger'
    : tone === 'today' ? 'bg-warning-soft text-warning dark:text-warning'
      : tone === 'high' ? 'bg-danger-soft text-danger dark:text-danger'
        : tone === 'medium' ? 'bg-warning-soft text-warning dark:text-warning'
