...
import { PLANNING_CONFLICT_ERROR, AUTOMATION_OCCURRENCE_SAMPLES_PER_DAY, getAutomationOccurrencesByDay } from '@canopy/shared'
import type { Automation, CalendarEvent, PlanningGroup, PlanningTag, Todo } from '@canopy/shared'
  ...
  return <div aria-hidden={!interactive} className={cn('grid h-full w-1/3 min-w-0 shrink-0 grid-cols-7 snap-start', !interactive && 'pointer-events-none')}>{week.days.map((day) => {
    ...
    return <div key={day} onPointerDown={interactive ? (event) => onPointerDown(event, day) : undefined} onPointerMove={interactive ? (event) => onPointerMove(event, day) : undefined} onPointerUp={interactive ? (event) => onPointerUp(event, day) : undefined} onPointerCancel={interactive ? (event) => onPointerCancel(event, day) : undefined} className={cn('relative min-w-0 border-r border-border/60', interactive && 'cursor-crosshair select-none')}>
      ...
      {segments.map((segment) => {
        ...
        if (segment.item.kind === 'todo') return <div key={`todo-${segment.item.id}`} className="absolute z-20" style={style}><TodoCalendarMarker todo={segment.item.todo} onSelect={() => onSelectTodo(segment.item.id)} className="h-full bg-warning/20 text-warning dark:text-warning" /></div>
        return <AutomationCalendarMarker key={`automation-${segment.item.id}`} automation={segment.item.automation} occurrenceCount={segment.item.count} showWindow={segment.item.isWindow} iconClassName="mt-0.5" className={cn('absolute z-20 flex items-start gap-1 overflow-hidden px-1 py-0.5 text-[10px] shadow-sm', segment.item.isWindow ? 'border-l-2 border-accent-2/70 bg-accent-2-soft text-accent-2 dark:text-accent-2' : 'bg-accent-2/20 text-accent-2 dark:text-accent-2')} style={style} />
      })}
