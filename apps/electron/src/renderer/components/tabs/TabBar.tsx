...
// mac 下 TabBar 住在 MacTitleRow（与红绿灯同行）、抬到 40px 填满行高；
// 34px 的 tab 项经 items-end 贴底对齐（Chrome 风格）。Windows 保持 34px。
import { getTabBarHeightClass } from '@/lib/window-titlebar-layout'
...
export function TabBar(): React.ReactElement {
  // 现代风格两层顶栏：标签栏撑到 48px 第二层（window-titlebar-layout.ts），经典保持 34px
  ...
  // 没有标签时的空占位：现代风格下由 .main-tabbar-empty 规则铺第二层顶栏底色，带才不会在中列断开
  if (tabs.length === 0) return <div className={cn('main-tabbar-empty', getTabBarHeightClass(detectIsMac(), interfaceVariant), 'titlebar-drag-region')} />
...
function TabBarInner({
  ...
  interfaceVariant: 'classic' | 'modern'
  tabs: TabItem[]
  ...
  // Chat 模式下右侧工作区整体不渲染（AppShell showRightPanel 绑 appMode），
  // 打开按钮必须同步隐藏——否则按钮跟进 Chat 界面、点击只翻转 atom 无任何可见效果（维护者报障）。
  const showOpenPanelButton = appMode === 'agent' && !isPanelOpen && activeTab?.type === 'agent'
  ...
  return (
    <div ref={barRef} className={cn('main-tabbar flex items-end tabbar-bg relative', getTabBarHeightClass(detectIsMac(), interfaceVariant))}>
