...
import type { BrowserStateChange, BrowserTabFocusChange } from '@canopy/shared'
...
export function MainArea(): React.ReactElement {
  ...
  // mac 的标签栏住在 MacTitleRow（与红绿灯同行），中列不再渲染；Agent 会话的头部同理由标题行承载
  const tabStripHost = React.useMemo(() => getTabStripHost(detectIsMac()), [])
  const showCenterTabBar = activeTab?.type !== 'agent' && tabStripHost === 'main-area'
  ...
  const publishBrowserState = React.useCallback((state: BrowserStateChange) => {
    ...
    if (shouldActivateAgentBrowserTab) {
      ...
      // 统一判定：用户手点过标签就不抢，Agent 连续导航也只抢本 run 第一次
      store.set(tryStealSidePanelFocusAtom, {
        sessionId: state.sessionId,
        source: 'browser',
        tab: getBrowserSidePanelTab(state.activeTabId),
      })
    }
  }, [activeTab, setBrowserOpenMap, setBrowserMinimizedMap, setBrowserStateMap, setPendingNavigationMap, store])
  ...
  const focusNativeBrowserTab = React.useCallback((change: BrowserTabFocusChange) => {
    ...
    store.set(tryStealSidePanelFocusAtom, {
      ...
      source: 'browser',
      tab: getBrowserSidePanelTab(change.tabId),
    })
  }, [activeTab, store])
    ...
    <Panel variant="grow" className={cn('bg-content-area', isClassic && 'rounded-2xl shadow-xl dark:shadow-sm')}>
      <div className="flex flex-1 min-h-0 overflow-hidden">
        <div className="flex flex-1 flex-col min-w-0 h-full">
          {/* 现代风格两层顶栏：占据主区的视图（规划 / 技能 / Vault）自带大标题、没有标签栏或会话标题条，
              这里补一段 48px 的空带（可拖拽），第二层才不会在中列断开；mac 由 MacTitleRow 承担，不重复 */}
          {!isClassic && tabStripHost === 'main-area' && (activeView === 'planning' || activeView === 'agent-skills' || activeView === 'vault') && (
            <div aria-hidden="true" className="main-view-topband titlebar-drag-region h-12 w-full shrink-0" />
          )}
          {activeView === 'planning' ? (
