...
import type { McpServerEntry, SkillMeta } from '@canopy/shared'
...
function buildSkillClassificationPrompt(input: {
  ...
  return `请帮我整理当前项目在 ${CANOPY_BRAND.productName} 工作区中保存的 Skills 的分组。
...
export function AgentSkillsView({
  ...
  const guideCatalogCli = React.useCallback(async (integration: CatalogCliIntegration): Promise<void> => {
    try {
      // “配置”是重新授予 Canopy 使用此 CLI 的入口，不会影响 CLI 自己的登录或授权。
  ...
  const disconnectCatalogCli = React.useCallback(async (integration: CatalogCliIntegration): Promise<void> => {
    try {
      ...
      toast.success(`已断开 ${integration.name}`, { description: `仅停止 ${CANOPY_BRAND.productName} 使用该 CLI，不会登出或撤销第三方授权。` })
    } catch {
  ...
  const handleClassifySkills = React.useCallback(async (): Promise<void> => {
    ...
    if (!data.skillsDir) {
      toast.error(`无法定位当前项目的 ${CANOPY_BRAND.productName} 工作区 Skills 目录`)
      return
    }
...
function SkillsTab({
  ...
  if (total === 0) {
    return <EmptyState icon={<Blocks className="size-8 text-foreground/30" />} title="暂无 Skill" hint={`可以在 Agent 模式下让 ${CANOPY_BRAND.productName} 帮你联网查找并安装 Skill，或从其他项目导入。`} />
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      {updateCount > 0 && (
        <div className="flex items-center gap-2 rounded-lg border border-info/20 bg-info-soft px-3 py-2 text-[13px] text-info dark:text-info">
      ...
      {builtinSkills.length > 0 && (
        <SkillSection title={`${CANOPY_BRAND.productName} 内置`} skills={builtinSkills} embedded={embedded} isBuiltin={isBuiltin} updatingSkill={updatingSkill} onOpen={onOpen} onToggle={onToggle} onUpdate={onUpdate} />
      )}
    </div>
  )
}
