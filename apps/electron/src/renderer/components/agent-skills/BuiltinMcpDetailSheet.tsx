/**
 * BuiltinMcpDetailSheet — Canopy 内置 MCP 托管详情
 */
...
import type { BuiltinMcpServerSummary } from '@canopy/shared'
...
function getConfigInfo(server: BuiltinMcpServerSummary): BuiltinMcpConfigInfo {
  ...
  return {
    source: `${CANOPY_BRAND.productName} 运行时`,
    description: `该内置能力由 ${CANOPY_BRAND.productName} 运行时托管。`,
  }
}
          ...
          <div className="shrink-0 border-b border-border/60 px-5 pb-4 pt-5">
            ...
            {server && (
              <div className="mt-4 flex items-start gap-3">
                <div className="rounded-xl bg-info-soft p-2 text-info shadow-sm shrink-0">
                  <Plug size={18} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="truncate text-base font-semibold text-foreground">{server.displayName}</h3>
                    <span className="shrink-0 rounded-md bg-info-soft px-1.5 py-0.5 text-[11px] font-medium text-info dark:text-info">
                      {CANOPY_BRAND.productName} 内置
                    </span>
                  </div>
