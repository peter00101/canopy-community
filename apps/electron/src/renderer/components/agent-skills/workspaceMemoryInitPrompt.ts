...
/** This block is project-portable policy, while exact Canopy paths remain in the workspace prompt. */
export const PROJECT_KNOWLEDGE_MAINTENANCE_BLOCK = `<!-- canopy:knowledge-maintenance:start -->
## 协作知识演进（${CANOPY_BRAND.productName} 维护）

- 保持本文件中的项目地图与已验证项目事实同步；命令、架构、边界和入口变化时做最小更新，不复制到协作记忆。
- ${CANOPY_BRAND.productName} 工作区的 \`memory/\` 是可扩展的长期协作知识库：\`MEMORY.md\` 只做主题索引和路由，按证据创建用户画像、协作偏好、纠错与经验、决策理由等主题文件；不要把临时过程或长篇证据写入其中。
...
<!-- canopy:knowledge-maintenance:end -->`
...
## 第一阶段：先建立可维护的项目地图
用户已授权你主动维护系统提示中给出的两个 \`AGENTS.md\`：项目根的 \`AGENTS.md\` 与 ${CANOPY_BRAND.productName} 工作区的 \`AGENTS.md\`。

1. 先读取两份现有文件（若存在），再用最小必要的项目证据核验：manifest、实际脚本、测试配置、目录入口、近期相关文档。不要只凭文件名或一般经验猜测。
2. 项目根 \`AGENTS.md\` 负责项目地图：架构、目录、命令、验证、项目边界和关键文档索引。${CANOPY_BRAND.productName} 工作区 \`AGENTS.md\` 只负责 ${CANOPY_BRAND.productName} 的执行环境、工作流和指向项目根规则的入口；不要复制项目事实，也不要枚举已安装 Skills——它们会动态注入系统提示词。
3. 缺失时创建简洁的最小索引；已有时只做可验证的增量更新。优先维护已有 \`<!-- canopy:... -->\` 区块；没有时只追加受管区块，绝不整体重写、删除或覆盖用户手写规则。
...
写入边界：
- 你可以基于已核验的项目事实，小幅维护系统提示中给出的项目根或 ${CANOPY_BRAND.productName} 工作区 \`AGENTS.md\`，但必须保留用户内容并遵守两份文件的职责边界。
