...
import type { SkillFileNode } from '@canopy/shared'
...
export function WorkspaceMemoryWindowApp(): React.ReactElement {
  ...
  React.useEffect(() => {
    document.title = `${CANOPY_BRAND.productName} · 工作区记忆`
  }, [])
  React.useEffect(() => {
      ...
      <div className={cn('flex h-screen min-h-0 flex-col overflow-hidden bg-content-area antialiased', getWindowTitlebarContentInsetClass(isWindows))}>
        ...
        <main className="grid min-h-0 flex-1 grid-cols-[240px_minmax(0,1fr)]">
          ...
          <section className="flex min-h-0 flex-col bg-background">
            ...
            {remoteChanged && <div className="flex shrink-0 items-center justify-between gap-3 border-b border-warning/30 bg-warning-soft px-4 py-2 text-xs text-warning dark:text-warning"><span>该文件已在其他位置更新。重新载入后再编辑，以免覆盖新内容。</span><Button size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={() => selectedPath && void openFile(selectedPath)}>重新载入</Button></div>}
