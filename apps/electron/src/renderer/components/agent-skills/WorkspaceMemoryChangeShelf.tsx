...
import type { WorkspaceMemoryFileChange } from '@canopy/shared'
    ...
    <article className="overflow-hidden rounded-xl border border-border/70 bg-background/40">
      ...
      {hasDiff ? (
        <pre className="overflow-x-auto whitespace-pre-wrap break-words p-4 font-mono text-xs leading-5">
          ...
          {change.diff?.removed.map((line, lineIndex) => <div key={`removed-${lineIndex}`} className="bg-danger-soft px-1 text-danger dark:text-danger">- {line || ' '}</div>)}
          {change.diff?.added.map((line, lineIndex) => <div key={`added-${lineIndex}`} className="bg-success-soft px-1 text-success dark:text-success">+ {line || ' '}</div>)}
