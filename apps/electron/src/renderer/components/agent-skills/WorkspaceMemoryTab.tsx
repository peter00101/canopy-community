...
import type { SkillFileNode, WorkspaceMemorySummary } from '@canopy/shared'
...
/** AGENTS.md 那一行在文件创建前就渲染：文件不在时打开它所在的文件夹，连文件夹都不在才提示。 */
function revealMemoryFile(absolutePath: string, label: string): void {
  window.electronAPI.showItemInFolder(absolutePath, undefined, { fallbackToParentFolder: true })
    .then((ok) => { if (!ok) toast.error(`未找到 ${label} 所在位置`) })
    .catch(() => toast.error(`未找到 ${label} 所在位置`))
}
        ...
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
              点击即授权 Agent 基于可验证证据维护项目根与 {CANOPY_BRAND.productName} 工作区的 AGENTS.md；随后在真实协作中逐步校准你的偏好。不会扫描历史会话。
            </div>
          </div>
      ...
      <div className={cn('grid min-h-[520px] gap-4 lg:grid-cols-[280px_minmax(0,1fr)]', embedded && 'min-h-0 flex-1 grid-cols-[180px_minmax(0,1fr)] gap-3')}>
          ...
          <div className="flex h-full min-h-0 flex-col">
            ...
            {migrationIssues.length > 0 && (
              <div
                ...
                className="mx-2 mt-2 flex items-center gap-1.5 rounded-md bg-warning-soft px-2 py-1.5 text-[11px] leading-relaxed text-warning dark:text-warning"
              >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-2">
              <FileButton
                ...
                label="AGENTS.md"
                meta={`${CANOPY_BRAND.productName} 工作区项目指令`}
                onClick={() => void openAgents(summary)}
                onReveal={() => revealMemoryFile(summary.agentsMd.path, 'AGENTS.md')}
              />
                ...
                {visibleAutoFiles.length === 0 ? (
                  ...
                  visibleAutoFiles.map((node) => (
                    <MemoryTreeNode
                      ...
                      onReveal={(path) => revealMemoryFile(autoMemoryPath(summary, path), path)}
                    />
                  ))
                )}
              </div>
        ...
        <SettingsCard divided={false} className="min-h-0 overflow-hidden">
          <div className="flex h-full min-h-0 flex-col">
            ...
            {saveConflict && (
              <div className="mx-4 mt-3 flex items-center justify-between gap-3 rounded-lg border border-warning/30 bg-warning-soft px-3 py-2 text-xs text-warning dark:text-warning">
