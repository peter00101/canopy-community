...
import { useDialogSafeAreaStyle } from "@/hooks/useDialogSafeAreaStyle"
...
>(({ className, children, hideClose, container, overlayClassName, style, ...props }, ref) => {
  // 开着内置浏览器时让开原生视图，否则弹窗右半既看不见也点不着（0.18.67）。
  // ⚠️ 仅在走窗口级 portal 时生效：传了 container 的弹窗，其 fixed 包含块可能是被
  // transform 过的祖先，按视口坐标算出的 left 会落错位置，此时保持原有的 left-[50%]。
  const safeAreaStyle = useDialogSafeAreaStyle()
  const positionStyle = container ? undefined : safeAreaStyle
  return (
