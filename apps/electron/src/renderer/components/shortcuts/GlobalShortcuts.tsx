...
export function GlobalShortcuts(): null {
  ...
  useShortcut(
    'clear-context',
    useCallback(() => {
      window.dispatchEvent(new CustomEvent('canopy:clear-context'))
    }, []),
  )
  ...
  useShortcut(
    'focus-input',
    useCallback(() => {
      window.dispatchEvent(new CustomEvent('canopy:focus-input'))
    }, []),
  )
  ...
  useShortcut(
    ...
    useCallback(() => {
      ...
      if (target) {
        window.dispatchEvent(new CustomEvent('canopy:stop-generation', { detail: target }))
      }
    ...
    const cleanup = window.electronAPI.onQuickTaskOpenSession(async (data) => {
      try {
        ...
        if (data.mode === 'agent') {
          ...
          const savedAttachments: import('@canopy/shared').FileAttachment[] = []
  ...
  useEffect(() => {
    ...
    const cleanup = window.electronAPI.onVoiceDictationInsertText((data) => {
      ...
      const insertedAtCursor = !window.dispatchEvent(new CustomEvent('canopy:insert-voice-dictation-text', {
        cancelable: true,
