    ...
    enter: ({ type, from, to }) => {
      if (type.name === 'SetextHeading2') {
        ...
        if (underline.to === state.doc.length && /^-\s*$/.test(underline.text)) {
          ...
          entries.push({ from: headingLine.from, to: headingLine.to, decoration: pendingListHeading })
        }
      }
      ...
      entries.push({ from, to: markerEnd, decoration: hiddenMarkdownSyntax })
    },
  })
  // 行内链接 `[text](dest)`：只藏 LinkMark 会把目标裸露在链接文字后面（0.18.55 修）。
  // 阅读态整段隐藏「( 之后到 ) 之前」（URL + 可选标题 + 中间空白），链接文字保留并加链接样式；
  // 光标进入该行时与其余语法一起恢复源码。
  for (const link of findMarkdownInlineLinks(state)) {
    if (link.textTo > link.textFrom) {
      entries.push({
        from: link.textFrom,
        to: link.textTo,
        decoration: Decoration.mark({
          class: 'live-markdown-link',
          attributes: { 'data-live-markdown-href': link.href, title: link.href },
        }),
      })
    }
    if (!activeLines.has(link.line) && link.destinationTo > link.destinationFrom) {
      entries.push({ from: link.destinationFrom, to: link.destinationTo, decoration: hiddenMarkdownSyntax })
    }
  }
  // 语法树遍历与链接扫描两路来源交错，RangeSetBuilder 要求按位置递增添加。
...
const markdownSyntaxVisibility: Extension[] = [
  ...
  EditorView.domEventHandlers({
    ...
    // Ctrl/Cmd+点击链接文字打开 http(s) 外链（走主进程 setWindowOpenHandler → shell.openExternal）；
    // 普通点击仍是放置光标进入编辑，与 Obsidian 的 live preview 习惯一致。
    click: (event) => {
      if (!(event.ctrlKey || event.metaKey)) return false
      const target = event.target as HTMLElement | null
      const href = target?.closest<HTMLElement>('[data-live-markdown-href]')?.dataset.liveMarkdownHref
      if (!isOpenableMarkdownLinkHref(href)) return false
