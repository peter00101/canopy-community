export type InlineLiveMarkdownPreview =
  | { kind: 'math' | 'autolink' | 'escape'; from: number; to: number; content: string }
...
/**
 * 整行 `$$公式$$` 的单行块级写法（Typora/Obsidian 常见）。
 * 内容里再出现 $ 属歧义写法，保守放弃交还原文显示。
 */
export function getSingleLineDisplayMath(line: string): string | null {
...
export function findInlineLiveMarkdownPreviews(line: string): InlineLiveMarkdownPreview[] {
  ...
  // $ 定界符按 Pandoc 惯例收紧：开界 $ 后与闭界 $ 前不许空白、闭界 $ 后不许紧跟数字，
  // 否则「卖 $100，那台卖 $200」这类美元价格会被配成一个公式。
  // 前置字符排除 $ 与 \，使 $$…$$ 不会被单 $ 模式啃出残留美元符。
  const mathPatterns = [
    { pattern: /(^|[^\\$])\$\$(?![\s$])([^$\n]*[^\s\\$])\$\$(?!\d)/g, contentIndex: 2, markerOffset: (match: RegExpExecArray) => match[1]!.length },
    { pattern: /(^|[^\\$])\$(?![\s$])([^$\n]*[^\s\\$])\$(?!\d)/g, contentIndex: 2, markerOffset: (match: RegExpExecArray) => match[1]!.length },
  ...
  while ((escapeMatch = escapePattern.exec(line)) !== null) {
    const from = escapeMatch.index
    const to = from + 2
    if (isEscaped(line, from) || overlapsInlineCode(codeRanges, from, to)) continue
    previews.push({ kind: 'escape', from, to, content: '$' })
  }
