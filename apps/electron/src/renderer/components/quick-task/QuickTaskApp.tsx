  ...
  async function loadModelInfo(): Promise<void> {
    try {
      ...
      if (mode === 'agent') {
        ...
        const raw = localStorage.getItem('canopy-selected-model')
        if (raw) {
          try {
      ...
      <div className={`quick-task-container flex w-full flex-col rounded-2xl bg-background transition-colors ${isDragOver ? 'ring-2 ring-primary/50' : ''}`}>
        ...
        <div className="flex-1 px-4 py-2">
          <textarea
            ...
            onPaste={handlePaste}
            placeholder={mode === 'agent' ? `向 ${CANOPY_BRAND.productName} 描述你的任务，Enter 发送...` : `向 ${CANOPY_BRAND.productName} 发送消息，Enter 发送...`}
