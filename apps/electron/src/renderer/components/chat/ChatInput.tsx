 ...
 * - 监听 canopy:clear-context 和 canopy:focus-input 自定义事件
 * - 卡片式容器样式
 */
...
export function ChatInput({ conversationId, streaming, pendingAttachments, onSetPendingAttachments, onSend, onStop, onClearContext }: ChatInputProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('canopy:clear-context', handler)
    return () => window.removeEventListener('canopy:clear-context', handler)
  }, [onClearContext])
