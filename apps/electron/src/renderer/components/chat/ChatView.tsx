...
import { TextSelectionCopyLayer } from '@/components/selection/TextSelectionCopyLayer'
...
function ChatViewInner({ conversationId }: ChatViewProps): React.ReactElement {
  ...
  /** Chat 消息区选区浮层的容器 */
  const chatSelectionRootRef = React.useRef<HTMLDivElement>(null)
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col h-full flex-1 min-w-0">
        ...
        <div ref={chatSelectionRootRef} className="flex flex-col flex-1 w-full max-w-[min(72rem,100%)] mx-auto overflow-hidden min-h-0">
          {/* 选中消息文本时弹出「复制文本」浮层（输入框等可编辑区在 hook 内自动排除） */}
          <TextSelectionCopyLayer rootRef={chatSelectionRootRef} />
          {/* 中间：消息区域 */}
          <ChatMessages
