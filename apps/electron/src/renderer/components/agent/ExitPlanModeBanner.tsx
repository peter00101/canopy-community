...
import { allPendingExitPlanRequestsAtom, agentStreamingStatesAtom, liveMessagesMapAtom } from '@/atoms/agent-atoms'
import type { ExitPlanModeAction, ExitPlanAllowedPrompt, SDKMessage } from '@canopy/shared'
...
export function ExitPlanModeBanner({ sessionId }: ExitPlanModeBannerProps): React.ReactElement | null {
  ...
  const handleAction = async (action: ExitPlanModeAction): Promise<void> => {
    ...
    setSubmitting(true)
    try {
      const trimmedFeedback = action === 'feedback' ? feedbackText.trim() : undefined
      // 反馈上屏消息的 uuid：渲染层生成、随响应传给主进程落盘同一 uuid，
      // 消息合并层按 uuid 去重，刷新重读后不会双显。
      const feedbackMessageUuid = action === 'feedback' ? crypto.randomUUID() : undefined
      const result = await window.electronAPI.respondExitPlanMode({
      ...
      if (result?.accepted === false) {
        // 审批已失效（run 已中止/会话已清理）——明确告知，绝不静默吞掉用户输入
        toast.error('该计划审批已失效', { description: '请直接在下方输入框重新发送你的意见。' })
      } else if (result?.planDocumentChanged === true) {
        // 批准被哈希复核否掉：计划文件在你看的这段时间里变了，批准作废。
        // 不提示的话表现就是「点了批准，横幅没了，什么也没发生」，几秒后又冒出一个新横幅。
        toast.warning('计划文档已变更，本次批准未生效', { description: '计划文件在审批期间被改动，请等待重新提交后再确认。' })
      } else if (action === 'feedback' && trimmedFeedback && feedbackMessageUuid) {
        // 反馈乐观上屏：用户要在会话里看到自己说了什么（主进程已用同 uuid 落盘）
        const feedbackMessage = {
          type: 'user',
          uuid: feedbackMessageUuid,
          message: { content: [{ type: 'text', text: trimmedFeedback }] },
        ...
        setLiveMessages((prev) => {
          ...
          map.set(sessionId, [...current, feedbackMessage])
          return map
        })
      }
      // 从队列移除
  ...
  /**
   * 在右侧工作区只读打开待审批的计划文档。
   * 审批到达时监听层已自动打开一次；这个按钮是用户关掉预览 Tab 后的回头路。
   */
  const planDocument = request?.planDocument
  ...
  }, [openPreview, planDocument, sessionId])
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {planDocument && (
            <button
              ...
              title={`在右侧查看 ${planDocument.displayName}`}
            >
