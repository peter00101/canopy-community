...
import { getTabStripHost } from '@/lib/window-titlebar-layout'
...
import type { AgentDeferredQueueMessageInput, AgentSendInput, AgentPendingFile, AgentThinkingLevel, FileDialogLargeFile, FileDialogResult, ModelOption, ReasoningCapability, SDKMessage, SDKUserMessage } from '@canopy/shared'
import { inferContextWindow, inferReasoningTransport, isCodexFastModeSupportedModel, MAX_ATTACHMENT_SIZE, normalizeReasoningCapabilityLevel, normalizeReasoningLevel, resolveReasoningCapability, resolveReasoningProfile } from '@canopy/shared'
...
import { finalizeStaleAgentRun } from '@/lib/agent-run-reconciler'
import {
...
interface AgentThinkingPopoverProps {
  agentThinking: import('@canopy/shared').ThinkingConfig | undefined
  onToggle: () => void
            ...
            <>
              <div className="space-y-2.5">
                ...
                {!thinkingLevels.includes('off') && (
                  // 档位表里没有 off 的模型（如 GLM-5.3）始终思考；点脑图标只会落到最低档而不是关闭，这里说清楚
                  <p className="text-[10px] leading-snug text-muted-foreground">该模型始终开启思考，不可关闭</p>
                )}
              </div>
            </>
          ) : (
...
export function AgentView({ sessionId, embedded = false }: AgentViewProps): React.ReactElement {
  ...
  const tabStripHost = React.useMemo(() => getTabStripHost(detectIsMac()), [])
  ...
  // 历史用量回填已随上游 fb62fbf1 下沉到 ContextUsageBadge：
  // 输入区的 stream state 现在只有 running/backgroundWaiting，AgentView 不再碰 usage
  // （这正是那次 perf 改动的目的），回填所需的历史消息以 persistedMessages 透传给徽标。
  ...
  // 取值逻辑抽到 useWorkspaceFilesPath，与 mac 标题行里的 AgentHeader 共用
  const workspaceFilesPath = useWorkspaceFilesPath(workspaceSlug, projectRootPath)
  ...
  const handleSend = React.useCallback(async (overrideText?: string, fromEditor = false): Promise<void> => {
    ...
    if (streaming) {
      // Agent 正在输出时，用户消息默认进入 Canopy 托管队列，不打断当前 turn。
  ...
  const handleStop = React.useCallback((): void => {
    ...
    // 例外：后端应答 hadActiveRun=false（根本没有 run 在跑，终态事件曾丢失）时，
    // STREAM_COMPLETE 永远不会来——立即结束本地运行态，否则停止按钮点了没反应。
    window.electronAPI.stopAgent(sessionId).then((result) => {
      if (result?.hadActiveRun === false) {
  ...
  const handleCompact = React.useCallback((): void => {
    ...
    const syntheticMsg: import('@canopy/shared').SDKMessage = {
      type: 'user',
      uuid: localUuid,
      message: {
    ...
    } as unknown as import('@canopy/shared').SDKMessage
  ...
  /** 快照回退：同一会话内回退到指定消息点，只截断对话（不修改工作区文件） */
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('canopy:stop-generation', handler)
    return () => window.removeEventListener('canopy:stop-generation', handler)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('canopy:focus-input', handler)
    return () => window.removeEventListener('canopy:focus-input', handler)
  }, [])
  ...
  const inputToolbarItems = React.useMemo<ToolbarItem[]>(() => [
    ...
    },
    {
      ...
      node: (
        <ContextUsageBadge
          ...
          persistedMessages={persistedSDKMessages}
        />
      ),
    },
  ], [
    ...
    // 切模型后进度环分母要跟着换：漏了它，徽标会一直按上一个模型的窗口算占用率与压缩阈值
    agentModelId,
    isCodexFastModeAvailable,
    ...
    handleCompact,
    agentVoiceInputId,
    // 回填的输入：漏了它，徽标会一直拿到消息加载前的空数组，历史会话永远显示「还没有占用」
    ...
    <SkillMentionNamesProvider workspaceSlug={workspaceSlug}>
      <div
        className={cn(
          ...
          // 与 Chat 视图对齐：Chat 右侧有常驻 40px 提示词条，内容区因此整体左移半个条宽。
          // Agent 无对应条，这里等宽右留白让两个模式的消息列（含 header 按钮）严格同轴（维护者要求）。
          !embedded && 'pr-10',
        )}
      ...
      >
        ...
        {/* mac 下会话头由 MacTitleRow 承载（与红绿灯同行），中列不再重复渲染 */}
        {!embedded && tabStripHost === 'main-area' && <AgentHeader sessionId={sessionId} projectFilesPath={workspaceFilesPath} />}

        <div className={cn(
          'flex min-h-0 flex-1 w-full flex-col overflow-hidden',
          // Agent 对话列按上游原版：最宽 72rem 居中——右面板开着时撑满主区、收起时居中（维护者 2026-09-10 定，靠左与按档位收窄都试过不好看）
        ...
        <div className="px-2.5 pb-2.5 md:px-[18px] md:pb-[18px]" data-input-mode="agent">
          ...
          >
            ...
            {isLegacyTranscript && (
              <div className="flex items-center justify-between gap-3 px-4 py-2 text-sm text-warning dark:text-warning">
            ...
            {(!agentChannelId || !hasAvailableModel) && (
              <div className="flex items-center gap-2 px-4 py-2 text-sm text-warning dark:text-warning">
                <Settings size={14} />
              ...
              <div className="flex flex-wrap gap-2 px-3 pt-2.5 pb-1.5">
                {pendingFiles.map((file) => (
                    <AttachmentPreviewItem
                      ...
                      onClick={file.sourcePath || file.filename.startsWith('clipboard-') ? () => handleClipboardPreview(file) : undefined}
