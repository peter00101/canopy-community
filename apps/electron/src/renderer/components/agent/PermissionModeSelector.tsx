...
import type { AppPermissionMode } from '@canopy/shared'
import { CANOPY_PERMISSION_MODE_CONFIG, CANOPY_PERMISSION_MODE_ORDER } from '@canopy/shared'
...
const MODE_ICONS: Record<AppPermissionMode, React.ComponentType<{ className?: string }>> = {
...
export function PermissionModeSelector({ sessionId }: PermissionModeSelectorProps): React.ReactElement | null {
  ...
  React.useEffect(() => {
    ...
    setModeMap((prev: Map<string, AppPermissionMode>) => {
  ...
  const cycleMode = React.useCallback(async () => {
    const currentIndex = CANOPY_PERMISSION_MODE_ORDER.indexOf(displayMode)
    const nextIndex = (currentIndex + 1) % CANOPY_PERMISSION_MODE_ORDER.length
    const nextMode = CANOPY_PERMISSION_MODE_ORDER[nextIndex]!
    const prevMode = mode
  ...
  const config = CANOPY_PERMISSION_MODE_CONFIG[displayMode]
