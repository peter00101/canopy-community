...
import { useSmoothStream } from '@canopy/ui'
import { getToolIcon, extractFilePath } from './tool-utils'
import { getToolPhrase, getToolResultSummary, shouldShowToolKindLabel, isPlanDecisionError } from './tool-phrase'
...
import { buildToolResultIndex, type SubAgentMeta, type ToolResultData, type ToolResultIndex } from '@/lib/tool-result-index'
import type {
  SDKContentBlock,
  SDKMessage,
  SDKTextBlock,
...
// ===== 工具结果索引 =====
//
// 索引构建是纯逻辑，抽在 @/lib/tool-result-index 便于测试；这里只保留 React 接线。
// AgentMessages 每次消息增量建一次索引经此 context 下发，工具块内 O(1) 查找。

export { buildToolResultIndex }
export type { ToolResultIndex }

export const ToolResultIndexContext = React.createContext<ToolResultIndex | null>(null)

/** 在 allMessages 中查找匹配 toolUseId 的工具结果（有索引走 O(1)，无 Provider 时兜底线性扫描） */
function useToolResult(toolUseId: string, allMessages: SDKMessage[]): ToolResultData | null {
  const index = React.useContext(ToolResultIndexContext)
  return React.useMemo(() => {
    if (index) return index.results.get(toolUseId) ?? null
    return buildToolResultIndex(allMessages).results.get(toolUseId) ?? null
  }, [toolUseId, allMessages, index])
}

// ===== useSubAgentMeta Hook =====

/** 从 allMessages 中查找匹配 toolUseId 的 task_notification 系统消息，提取用量数据（有索引走 O(1)） */
function useSubAgentMeta(toolUseId: string, allMessages: SDKMessage[]): SubAgentMeta | null {
  const index = React.useContext(ToolResultIndexContext)
  return React.useMemo(() => {
    if (index) return index.subAgentMeta.get(toolUseId) ?? null
    return buildToolResultIndex(allMessages).subAgentMeta.get(toolUseId) ?? null
  }, [toolUseId, allMessages, index])
}
...
function ToolUseBlock({ block, allMessages, animate = false, index = 0, dimmed = false, childBlocks, basePath, isStreaming }: ToolUseBlockProps): React.ReactElement {
  ...
  // ExitPlanMode 的 error = 用户决策（拒绝/提意见），不渲染成红色失败态
  const showErrorVisual = isError && !isPlanDecisionError(block.name)
        ...
        >
          ...
          {resultSummary && (
            <span className={cn(
              ...
              showErrorVisual ? 'text-destructive/70' : 'text-muted-foreground/55',
            )}>
              {resultSummary}
            </span>
          )}
          ...
          {phrase.diffStats && (isCompleted || !isStreaming) && (
            <span className="shrink-0 font-mono text-[12px] tabular-nums">
              {phrase.diffStats.additions > 0 && (
                <span className="text-success dark:text-success">+{phrase.diffStats.additions}</span>
              )}
              {phrase.diffStats.additions > 0 && phrase.diffStats.deletions > 0 && ' '}
              {phrase.diffStats.deletions > 0 && (
                <span className="text-danger dark:text-danger">-{phrase.diffStats.deletions}</span>
              )}
            </span>
          )}
