...
import type { AskUserQuestion } from '@canopy/shared'
...
export function AskUserBanner({ sessionId }: AskUserBannerProps): React.ReactElement | null {
  ...
  /** 单选即交的可改口窗口内为 true：底部提示改文案，避免「怎么自己就交了」的困惑 */
  ...
  // keydown handler 只在 requestId 变化时重建，经 ref 取最新实现
  const clearAutoAdvanceTimerRef = React.useRef(clearAutoAdvanceTimer)
  clearAutoAdvanceTimerRef.current = clearAutoAdvanceTimer
  ...
  React.useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent): void => {
      ...
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        e.preventDefault()
        // 键盘上下是「浏览」不是「定夺」：撤掉鼠标点选留下的自动提交计时，
        // 否则用户点一下再按 ↓ 翻看选项，400ms 后会被替他交掉
        clearAutoAdvanceTimerRef.current?.()
  ...
  const currentAnswer = getAnswer(activeTab)
  // 单选已选中（自动提交在路上）时才收起按钮；其余情形一律保留，避免把用户堵死
  const showConfirmButton = shouldShowAskUserConfirmButton({
    isLastTab,
    multiSelect: currentQuestion.multiSelect === true,
    showCustom: currentAnswer.showCustom,
    selectedCount: currentAnswer.selected.length,
  })
    ...
    <div className="ask-user-banner mx-4 mb-3 rounded-xl bg-card shadow-lg overflow-hidden animate-in slide-in-from-bottom-2 duration-200">
      ...
      <div className="px-4 pt-3 pb-2">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-foreground">{CANOPY_BRAND.productName} Agent 需要你的输入</span>
      ...
      <div className="px-4 pb-2">
        <QuestionCard
          ...
          onToggleOption={(label) => {
            ...
            clearAutoAdvanceTimer()
            // 点选项那一刻 toggleOptionByState 已把 showCustom 清成 false
            const decision = decideAskUserAdvance({
              isLastTab,
              multiSelect: currentQuestion.multiSelect === true,
              showCustom: false,
              selectedCount: 1,
      ...
      <div className="flex items-center justify-end gap-1.5 px-4 pb-3">
        <span className="text-[10px] text-muted-foreground/40 mr-auto">
          {autoSubmitPending ? '已选择 · 即将提交，改选可重选' : `↑↓ 选择 · Enter ${isLastTab ? '确认' : '下一个'}`}
        </span>
        {showConfirmButton && (
          <Button
            variant="default"
