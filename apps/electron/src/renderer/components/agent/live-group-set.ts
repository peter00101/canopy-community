import { getSDKCompactStatus, type SDKMessage, type SDKSystemMessage } from '@canopy/shared'
...
type RunScopedLiveMessage = SDKMessage & {
  _canopyLiveRunStartedAt?: number
}
...
export function buildLiveGroupSet({
  ...
  const activeLiveMessages = activeRunStartedAt == null
    ...
    : liveMessages.filter((message) => (
      (message as RunScopedLiveMessage)._canopyLiveRunStartedAt === activeRunStartedAt
    ))
