...
/**
 * 从 Canopy Skill 入口文件路径中提取 Skill 名称。
 *
...
export function getToolPhrase(toolName: string, input: Record<string, unknown>): ToolPhrase {
  ...
  switch (toolName) {
    ...
    case 'VisionRelay': {
      return phrase('视觉识别图片')
    }

    case 'VisionAugment': {
      return phrase('识别图片内容')
    }

    case 'VideoRelay': {
      return phrase('视频识别内容')
    }

    case 'VideoAugment': {
      return phrase('识别视频内容')
    }
...
/**
 * ExitPlanMode 的 error 结果不是工具失败：它只有「用户拒绝 / 用户提修改意见 / 中止」
 * 三种来源，全是正常的用户决策（反馈通过 deny message 通道传给模型）。
 * 显示成红色「失败」会让提意见的用户以为发送失败（维护者报障实锤）。
 */
export function isPlanDecisionError(toolName: string): boolean {
...
export function getToolResultSummary(toolName: string, result: string | undefined, isError = false): string | null {
  if (isError) return isPlanDecisionError(toolName) ? '计划未获批准' : '失败'
