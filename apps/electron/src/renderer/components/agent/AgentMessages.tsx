...
import { toTranscript } from '@canopy/session-core'
...
import { buildLongRunHint } from '@/lib/agent-long-run-hint'
import { getTaskProgressReserveClass } from '@/lib/task-progress-reserve'
import { isLiveMessageInRun, type LiveRunIdentity } from '@/lib/live-run-scope'
import { buildLiveGroupSet } from './live-group-set'
import { ToolResultIndexContext, buildToolResultIndex } from './ContentBlock'
...
import type { AgentEventUsage, RetryAttempt, SDKAssistantMessage, SDKMessage, SDKSystemMessage, SDKTextBlock, SDKThinkingBlock } from '@canopy/shared'
import { getSDKCompactStatus } from '@canopy/shared'
import { agentLiveMessagesAtomFamily, agentSessionStreamingStateAtomFamily, type AgentStreamState, type ToolActivity } from '@/atoms/agent-atoms'
...
/**
 * 汇总当前 run 全部 live turn 的任务工具活动，供底部任务进度浮层使用（收上游 #2076）。
 *
 * 压缩状态或后台唤醒会把一个 run 拆成多个 assistant-turn；只取最后一段的话，
 * 此前建好但还没再次更新的任务会从进度卡上消失。仍不从持久化历史恢复任务，
 * 也不收其他 run 的迟到消息，避免跨 run 显示旧进度。
 */
export function collectLiveRunTaskActivities(
  liveMessages: readonly SDKMessage[] | null | undefined,
  sessionModelId: string | undefined,
  run: LiveRunIdentity | null | undefined,
): ToolActivity[] {
  const currentRunMessages = (liveMessages ?? []).filter((message) => isLiveMessageInRun(message, run))
...
const AGENT_HISTORY_QUOTE_HIGHLIGHT_NAME = 'canopy-agent-history-quote'
...
function RetryingNotice({ retrying }: { retrying: NonNullable<AgentStreamState['retrying']> }): React.ReactElement {
  ...
  return (
    <div className="rounded-lg border border-warning/40 bg-warning-soft dark:border-warning/40 dark:bg-warning-soft p-3 mb-3">
      <button
        type="button"
      ...
      >
        {retrying.phase === 'succeeded' ? (
          ...
          retrying.phase === 'cancelled'
            ? <Ban className="size-4 text-warning dark:text-warning shrink-0" />
            : <AlertTriangle className="size-4 text-warning dark:text-warning shrink-0" />
        ) : (
          <RotateCw className="size-4 animate-spin text-warning dark:text-warning shrink-0" />
        )}
        <span className="text-sm text-warning dark:text-warning flex-1 tabular-nums">
          {statusText}
          {retrying.reason && ` · ${retrying.reason}`}
        </span>
        {expanded ? (
          <ChevronDown className="size-4 text-warning dark:text-warning shrink-0" />
        ) : (
          <ChevronRight className="size-4 text-warning dark:text-warning shrink-0" />
        )}
      </button>

      {expanded && (
        <div className="mt-3 space-y-3 border-t border-warning/40 dark:border-warning/40 pt-3">
          {retrying.maxTotalAttempts != null && (
            <div className="text-xs text-warning dark:text-warning tabular-nums">
              本轮已安排 {retrying.totalAttempt ?? 0}/{retrying.maxTotalAttempts} 次自动恢复
            </div>
          )}
          {retrying.history.length > 0 && (
            <>
              <div className="text-xs font-medium text-warning dark:text-warning">
                已执行的恢复记录：
              </div>
          ...
          {retrying.phase === 'scheduled' && (
            <div className="flex items-center gap-2 text-xs text-warning dark:text-warning pl-6 tabular-nums">
    ...
    <div className={cn('pl-6 space-y-2', isLatest && 'font-medium')}>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <div className="text-xs text-warning dark:text-warning font-mono break-words">
            {attempt.errorMessage}
          </div>

          {/* 环境信息 */}
          {attempt.environment && (
            <div className="text-[11px] text-warning dark:text-warning space-y-0.5">
          ...
          {attempt.stderr && (
            <div className="mt-2">
              <button
                type="button"
                className="text-[11px] text-warning dark:text-warning hover:underline flex items-center gap-1"
              ...
              {showStderr && (
                <pre className="mt-1 text-[10px] text-warning dark:text-warning bg-warning-soft dark:bg-warning-soft p-2 rounded overflow-x-auto max-h-[200px] overflow-y-auto">
                  {attempt.stderr}
                </pre>
              )}
            </div>
...
function AgentRunningIndicator({ startedAt }: { startedAt?: number }): React.ReactElement {
  ...
  // 长时间运行提示（第二层）：跑得久不等于在干活，给用户一个判断依据；文案是条件句，不误伤合法长任务
  const longRunHint = buildLongRunHint(elapsed)
  ...
  return (
    <div className="flex flex-col gap-1 min-h-[28px]">
      ...
      {longRunHint && (
        <div
          role="status"
          data-agent-long-run-hint={longRunHint.level}
          className={cn(
            'ml-6 max-w-[560px] rounded-md border px-2 py-1 text-[11px] leading-4',
            longRunHint.level === 'strong'
              ? 'border-warning/40 bg-warning-soft text-warning dark:text-warning'
...
export const AgentMessages = React.memo(function AgentMessages({
  ...
  // 任务进度浮层是绝对定位的，贴底时会压住尾部内容（长任务提示、最后一行回复）；
  // 由浮层上报占位状态，内容区据此在尾部预留净空。见 `task-progress-reserve.ts`。
  const [taskProgressOverlayShown, setTaskProgressOverlayShown] = React.useState(false)
  ...
  const allSDKMessages = React.useMemo(() => {
    ...
    const stampStableKey = (message: SDKMessage): SDKMessage => {
      ...
      if (record._appStableKey !== key) record._appStableKey = key
      return message
    }
    ...
    const upsert = (message: SDKMessage): void => {
      ...
      if (hasUuid(stamped)) {
        const key = (stamped as Record<string, unknown>)._appStableKey as string
  ...
  // 工具结果索引：每次消息增量只建一趟 O(N)，供全部工具块 O(1) 查找。
  // 没有它，每个工具块各自对全量消息线性扫描（O(M×N)），长会话流式期间主线程会被钉死。
  const toolResultIndex = React.useMemo(() => buildToolResultIndex(allSDKMessages), [allSDKMessages])

  const liveTaskActivities = React.useMemo(
    () => collectLiveRunTaskActivities(liveMessages, sessionModelId, { runGeneration: streamState?.runGeneration, startedAt }),
  ...
  // 结构派生（迷你地图、用户锚点、turn 编号）只关心 group 身份，不关心流式 token。
  ...
  // 会话级「文件名 → 绝对路径」映射：只基于已落盘消息构建（落盘变化才重算，流式帧不参与），
  // 供正文里跨轮次裸提的文件名补全绝对路径；当前 turn 由 AssistantTurnRenderer 的本轮映射优先覆盖。
  const sessionFileNameMap = React.useMemo(
    () => buildTurnFileNameMap(persistedSDKMessages ?? EMPTY_SDK_MESSAGES),
    [persistedSDKMessages],
  )
  ...
  return (
    ...
    <SessionFileMapProvider map={sessionFileNameMap}>
      ...
      `}</style>
        ...
        <ConversationContent
          className={cn(
            ...
            getTaskProgressReserveClass(taskProgressOverlayShown),
          )}
        >
        ...
        <TaskProgressOverlay
          ...
          onReserveSpaceChange={setTaskProgressOverlayShown}
        />
          </Conversation>
