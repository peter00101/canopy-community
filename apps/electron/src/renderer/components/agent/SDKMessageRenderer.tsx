...
import { ProcessBlockGroup, buildAssistantTurnRenderItems, hasNoExternalizedFinalOutput } from './ProcessBlockGroup'
...
// 会话转录的纯逻辑(Turn 分组 / 快照去重 / 预览)已下沉到 @canopy/session-core 作为唯一真源。
...
import {
  ...
  type MessageGroup,
  type AssistantTurn,
} from '@canopy/session-core'
export { groupIntoTurns, getGroupPreview, extractUserText } from '@canopy/session-core'
export type { MessageGroup, AssistantTurn } from '@canopy/session-core'
...
import type { AgentPendingFile } from '@canopy/shared'
...
function PermissionDeniedNotice({ message }: { message: SDKSystemMessage }): React.ReactElement {
  ...
  return (
    <div className="my-3 pl-[46px] pr-1">
      <div className="flex items-start gap-2.5 rounded-md border border-warning/20 bg-warning-soft px-3 py-2.5 text-xs text-foreground/80">
...
// extractMeta / MessageMeta 已迁移至 @canopy/session-core
...
// extractUserText 已迁移至 @canopy/session-core
...
// isUserInputMessage 已迁移至 @canopy/session-core
...
// AssistantTurn / MessageGroup 类型已迁移至 @canopy/session-core

// groupIntoTurns / mergeAdjacentSameModelTurns 已迁移至 @canopy/session-core
...
export function AssistantTurnRenderer({ turn, allMessages, basePath, onFork, onRewind, onCreateTodo, onRetry, onRetryInNewSession, onCompact, onRelinkProjectRoot, onRestoreProjectRoot, isStreaming, stoppedByUser, sessionModelId }: AssistantTurnRendererProps): React.ReactElement | null {
  ...
  // 回合零外显（模型以工具/thinking 收尾，全部内容折叠在过程组里）：补一行提示并保持过程组
  // 展开，避免用户看到「跑完了却什么都没有」（2026-08-31 用户反馈）。有错误横幅时不重复提示。
  const showNoFinalOutputHint = !isStreaming && !hasError && hasNoExternalizedFinalOutput(renderItems)
          ...
          {renderItems.map((item, itemIndex) => {
            ...
            return (
              <ProcessBlockGroup
                ...
                autoCollapseOnComplete={!showNoFinalOutputHint}
              />
            )
          })}
          {showNoFinalOutputHint && (
...
const SCHEDULED_RUN_MARKER = '<!--CANOPY_SCHEDULED_RUN-->'

// stripScheduledRunMarker 已迁移至 @canopy/session-core（本文件从该包 import 使用）
...
function ScheduledRunBadge(): React.ReactElement {
  ...
  return (
    <button
      ...
      title={`来自 ${CANOPY_BRAND.productName} 定时任务，点击查看设置`}
    >
      <Clock className="size-3" />
      <span>来自 {CANOPY_BRAND.productName} 定时任务</span>
    </button>
  )
}
...
export function getGroupId(group: MessageGroup): string {
  if (group.type === 'user') {
    ...
    const stableKey = (group.message as unknown as Record<string, unknown>)._appStableKey
  ...
  const stableKey = first ? (first as unknown as Record<string, unknown>)._appStableKey : undefined
  if (typeof stableKey === 'string') return stableKey
...
// getGroupPreview 已迁移至 @canopy/session-core（本文件从该包 import 并 re-export）
