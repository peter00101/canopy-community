...
import { PROCESS_GROUP_VIEWPORT_HEIGHT, getProgressViewportToggleLabel, shouldShowProgressViewportToggle } from './process-group-viewport'
...
interface ProcessBlockGroupProps {
  ...
  /** 回合零外显时传 false：完成后不自动折叠，让用户直接看到过程内容而不是一行空回复。 */
...
/**
 * 完成态下整个回合零外显（全部内容折叠在过程组里，无 trailing text 被外置为正式回答）。
 * 模型以工具调用或 thinking 收尾（提前 stop / 拖流被掐）时会出现——2026-08-31 用户
 * 反馈「跑了 15 分钟什么都没有」正是这个形态。调用方据此补提示行并保持过程组展开。
 */
export function hasNoExternalizedFinalOutput(items: AssistantTurnRenderItem[]): boolean {
  return items.length > 0 && items.every((item) => item.type === 'process-group')
}
...
export function ProcessBlockGroup({ blocks, isStreaming, renderChildren, isMessageTail = false, autoCollapseOnComplete = true }: ProcessBlockGroupProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    const shouldAutoCollapseAfterCompletion = wasStreamingRef.current && !userToggledRef.current && autoCollapseOnComplete
  ...
  // 「展开全部 / 收回小窗」：流式期间让用户随时脱离 480px 小视口看全量（维护者 2026-09-14 报「滑不到」）。
  // 展开时停止自动跟随，收回时恢复跟随并滚到最新。
  const toggleProgressViewport = React.useCallback((): void => {
    if (keepProgressViewport) {
      handleProgressScrollIntent()
      setKeepProgressViewport(false)
      return
    }
    followLatestRef.current = true
    setKeepProgressViewport(true)
  }, [keepProgressViewport, handleProgressScrollIntent])
  ...
  return (
    <div className="space-y-1.5">
      ...
      <button
        type="button"
        className={cn(
          'flex min-w-0 max-w-full items-center gap-2 py-0.5 text-left transition-opacity group',
          'hover:opacity-70',
        )}
        onClick={() => {
      ...
      {shouldShowProgressViewportToggle(!!isStreaming, shouldShowContent) && (
        <button
          type="button"
          className="shrink-0 rounded px-1.5 py-0.5 text-[12px] text-muted-foreground/60 transition-colors hover:bg-muted hover:text-muted-foreground"
          onClick={toggleProgressViewport}
          aria-pressed={!keepProgressViewport}
        >
          {getProgressViewportToggleLabel(keepProgressViewport)}
        </button>
      )}
      </div>
      ...
      {shouldRenderContent && (
        <div
          ...
          className={cn(
            ...
            keepProgressViewport && 'overflow-y-auto overscroll-contain scrollbar-thin',
          )}
