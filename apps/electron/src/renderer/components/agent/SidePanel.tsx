...
import { applyRightWorkspaceTabOrder, moveRightWorkspaceTab } from '@/lib/right-workspace-tab-order'
...
import {
  agentRightWorkspaceTabOrderAtom,
...
import type { FileEntry, AgentPendingFile, AgentSessionMeta, AttachedPathStatus, SDKMessage, WorktreeInfo } from '@canopy/shared'
...
export function SidePanel({ sessionId, sessionPath, activeTab, onTabChange, width = 460 }: SidePanelProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    // 记忆变更是第五条会抢焦点的路径，一并交给统一判定（面板展开也由它负责）
    tryStealSidePanelFocus({ sessionId, source: 'workspace-component', tab: 'memory' })
  }, [agentStreamState?.running, agentStreamState?.startedAt, effectiveActiveTab, isOpen, latestMemoryChange, sessionId, setMemoryNavigationRequest, setWorkspaceComponentTabs, tryStealSidePanelFocus, workspaceSlug])
  ...
  const handleOpenBrowserTab = React.useCallback(async () => {
    try {
      ...
      if (!state) return
      publishBrowserState(state)
      // 用户自己点「新建浏览器标签」：切过去，并按手动操作锁死焦点
      const tab = getBrowserSidePanelTab(state.activeTabId)
      pinSidePanelTabByUser({ sessionId, tab })
      handleWorkspaceTabChange(tab)
    } catch (error) {
      console.error('[SidePanel] 新建受管浏览器标签失败:', error)
    }
  }, [browserState, ensureBrowserOpen, handleWorkspaceTabChange, pinSidePanelTabByUser, publishBrowserState, sessionId])
  ...
  const handleOpenTerminal = React.useCallback((cwd?: string, title = '终端') => {
    ...
    // 用户自己点「新建终端」：同样算手动表态
    const tab = getTerminalSidePanelTab(terminalId)
    pinSidePanelTabByUser({ sessionId, tab })
    handleWorkspaceTabChange(tab)
  }, [handleWorkspaceTabChange, pinSidePanelTabByUser, sessionId, setTerminalTabsMap])
  ...
  // 用户拖动排过序则按其顺序渲染；新开的标签自动追加末尾（见 right-workspace-tab-order）
  const tabOrderMap = useAtomValue(agentRightWorkspaceTabOrderAtom)
  const setTabOrderMap = useSetAtom(agentRightWorkspaceTabOrderAtom)
  const orderedWorkspaceTabs = React.useMemo(
    () => applyRightWorkspaceTabOrder(workspaceTabs, tabOrderMap.get(sessionId)),
    [sessionId, tabOrderMap, workspaceTabs],
  )

  const handleReorderTab = React.useCallback((tabId: AgentSidePanelTab, toIndex: number) => {
    setTabOrderMap((previous) => {
      // 基准必须是「当前实际渲染的顺序」：保存的旧顺序里常残留已关闭标签的 id
      // （浏览器/终端标签开关频繁），拿它当基准会让新标签找不到自己而移动失效。
      const baseline = orderedWorkspaceTabs.map((tab) => tab.id)
      const next = moveRightWorkspaceTab(baseline, tabId, toIndex)
      const map = new Map(previous)
      map.set(sessionId, next)
      return map
    })
  }, [orderedWorkspaceTabs, sessionId, setTabOrderMap])
  workspaceTabsRef.current = orderedWorkspaceTabs
  ...
  return (
    <div
      className={cn(
        ...
        // 经典风靠圆角+阴影把面板托起来，用主体色即可；现代风是贴边平铺，
        // 若也用 --content-area 就与中间主区完全同色（实测两侧都是 rgb(245,248,250)），
        // 只剩一条 1px 淡线分隔，浅色主题下等于没有边界。改用侧栏那一档辅助色：
        // 侧栏与右面板同为辅助区，中间主区独占最亮的一档，三块自然分层。
        isClassic
          ? 'bg-content-area rounded-2xl shadow-xl dark:shadow-md'
 ...
 * 查询一组附加路径的即时可用状态。
 *
 * 取代历史上的「启动时按 existsSync 删配置」：路径暂时不可达（外接盘未插、
 * 网络盘未挂载、OneDrive 未同步、权限瞬时错误）时只标灰提示，配置永远保留，
 * 由用户自己决定要不要移除。与本地项目根的 projectRootStatus 是同一套哲学。
 */
function useAttachedPathStatuses(
  paths: readonly string[],
  refreshVersion: number,
): Map<string, AttachedPathStatus> {
  const [statuses, setStatuses] = React.useState<Map<string, AttachedPathStatus>>(new Map())
  // 数组每次渲染都是新引用，用内容做依赖键避免无限拉取。
  ...
  React.useEffect(() => {
    ...
    let cancelled = false
    window.electronAPI.getAttachedPathStatuses([...paths])
      .then((states) => {
        if (cancelled) return
        setStatuses(new Map(states.map((state) => [state.path, state.status])))
      })
      .catch(console.error)
    return () => { cancelled = true }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathsKey, refreshVersion])

  return statuses
}

/** 不可用附加路径的统一提示文案（悬停可见，不占列表空间）。 */
function describeAttachedPathStatus(status: AttachedPathStatus | undefined, path: string): string {
  if (status === 'missing') return `${path}\n\n当前找不到该路径（外接盘未连接、网络盘未挂载或已被移动）。配置已保留，可在菜单中手动移除。`
  if (status === 'unavailable') return `${path}\n\n该路径当前不可读（权限不足或被占用）。配置已保留，可在菜单中手动移除。`
  return path
}
...
interface AttachedFilesSectionProps {
  ...
  allowedPaths?: string[]
  sessionId: string
  /** 文件版本号，变化时重新查询附加路径可用状态 */
  refreshVersion?: number
}

function AttachedFilesSection({ title, scope = 'project', showSessionBadge = true, attachedFiles, onDetach, onAddToChat, onFilePreview, allowedPaths, sessionId, refreshVersion = 0 }: AttachedFilesSectionProps): React.ReactElement {
  const pathStatuses = useAttachedPathStatuses(attachedFiles, refreshVersion)
  return (
    <div className="pt-1 pb-0 flex-shrink-0">
      ...
      {attachedFiles.map((filePath) => {
        ...
        const status = pathStatuses.get(filePath)
        const isUnreachable = status === 'missing' || status === 'unavailable'
        return (
          <div
            key={filePath}
            className={cn(
              'flex items-center gap-1 py-1 pl-2 pr-2 text-sm cursor-pointer hover:bg-accent/50 group mx-2 rounded-lg',
              isUnreachable && 'opacity-45',
            )}
          ...
          >
            ...
            <span className="text-xs truncate flex-1" title={describeAttachedPathStatus(status, filePath)}>{name}</span>
            {isUnreachable && (
              <span className="flex-shrink-0 rounded-md bg-muted px-1.5 py-0.5 text-[10px] leading-none text-muted-foreground">未连接</span>
            )}
...
function AttachedDirsSection({ title, scope = 'project', showSessionBadge = true, attachedDirs, onDetach, refreshVersion, onAddToChat, onFilePreview, onOpenDirectoryTerminal, allowedPaths, sessionId }: AttachedDirsSectionProps): React.ReactElement {
  ...
  const pathStatuses = useAttachedPathStatuses(attachedDirs, refreshVersion)
  ...
  return (
    <div className="file-tree-guide-scope pt-1 pb-0 flex-shrink-0">
      ...
      {attachedDirs.map((dir) => {
        ...
        const status = pathStatuses.get(dir)
        const isUnreachable = status === 'missing' || status === 'unavailable'
        // 不可达目录只标灰 + 悬停解释，配置本身保留；移除仍走目录菜单里的「移除附加」。
        if (isUnreachable) {
          return (
            <div key={dir} className="opacity-45">
              <AttachedDirTree
                unavailableHint={describeAttachedPathStatus(status, dir)}
                dirPath={dir}
...
interface AttachedDirTreeProps {
  ...
  /** 自动定位脉冲时间戳，变化时重新触发 */
  revealTs?: number
  /** 非空表示该目录当前不可达：显示「未连接」并把说明挂到 title 上 */
  unavailableHint?: string
}

function AttachedDirTree({ dirPath, onDetach, selectedPaths, onSelect, refreshVersion, onAddToChat, onFilePreview, onOpenDirectoryTerminal, allowedPaths, sessionId, scope, showSessionBadge, revealTarget = null, revealTs = 0, unavailableHint }: AttachedDirTreeProps): React.ReactElement {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="relative z-10 text-xs truncate flex-1" title={unavailableHint || dirPath}>
          {dirName}
        </span>
        {unavailableHint && (
          <span className="relative z-10 flex-shrink-0 rounded-md bg-muted px-1.5 py-0.5 text-[10px] leading-none text-muted-foreground">未连接</span>
        )}
        {showSessionBadge && scope === 'session' && (
