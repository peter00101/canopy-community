...
import type { AgentSessionReferenceSearchResult, WorkspaceCapabilities } from '@canopy/shared'
  ...
  return createMentionSuggestion<SkillMentionItem>(
    {
      ...
      renderItem: (item) => (
        <>
          <Sparkles className="size-3.5 text-accent-2 flex-shrink-0" />
  ...
  return createMentionSuggestion<McpMentionItem>(
    {
      ...
      renderItem: (item) => (
        <>
          <Server className="size-3.5 text-success flex-shrink-0" />
          <span className="truncate font-medium flex-1 min-w-0">{item.name}</span>
  ...
  return createMentionSuggestion<SessionMentionItem>(
    {
      ...
      renderItem: (item) => (
        <>
          <MessageSquareText className="size-3.5 text-info flex-shrink-0" />
