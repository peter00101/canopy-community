  ...
  return (
    <div className="rounded-xl bg-foreground/[0.035] p-4 shadow-inner">
      <div className="flex flex-col gap-3">
        <div>
          <div className="text-[13px] font-semibold text-foreground">推荐：让 {CANOPY_BRAND.productName} Agent 创建</div>
          <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
            在左侧会话里说清目标，并明确表示要求创建定时任务，{CANOPY_BRAND.productName} Agent 会生成任务描述，并补全周期、项目和模型等配置，手动编辑更适合微调任务描述。
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
            例：检查 {CANOPY_BRAND.productName} 仓库新增 issue，主动回复问答类问题，不清楚的部分整理到项目级 Context 的 .context/issue-faq.md 文档；真正的 Bug 或请求罗列后发给我，不要记录任何重复的信息。
          </div>
        </div>
      </div>
    </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 flex flex-col gap-5">
          ...
          {isEdit && (
            <div className="rounded-lg bg-foreground/[0.03] p-3 flex flex-col gap-1.5 text-xs">
              ...
              {live?.completedAt && (
                <div className="flex items-center gap-1.5 pt-0.5 text-success dark:text-success">
          ...
          <div className="flex flex-col gap-2">
            <Label>项目</Label>
            {workspaces.length === 0 ? (
              <div className="flex items-center gap-2 rounded-md border border-warning/40 dark:border-warning/40 bg-warning-soft dark:bg-warning-soft px-3 py-2 text-sm text-warning dark:text-warning">
          ...
          <div className="flex gap-2 rounded-lg bg-warning-soft p-3 text-xs text-warning dark:text-warning">
