...
import { BookOpen, ChevronDown, ChevronRight, ChevronsUpDown, CircleHelp, Folder, FolderOpen, Loader2, PanelLeftClose, PanelLeftOpen, Plus, Trash2 } from 'lucide-react'
import { toast } from 'sonner'
import type { VaultCandidate, VaultFileEntry, VaultFocus, VaultReadResult, VaultSummary } from '@canopy/shared'
...
import type { LiveMarkdownEditorHandle } from '@/components/markdown/LiveMarkdownEditor'
...
const CANOPY_MANAGED_VAULT_DISPLAY_NAME = CANOPY_BRAND.vault.managedLabel
const CANOPY_SELF_MANAGED_VAULT_LABEL = CANOPY_BRAND.vault.selfManagedLabel
...
function getVaultCandidateDisplayName(candidate: VaultCandidate): string {
  return candidate.isCanopyManaged ? CANOPY_MANAGED_VAULT_DISPLAY_NAME : candidate.displayName
}
...
/** 主进程 vault-service 读文件时目标已不在磁盘上抛出的错误（文案见 `vault-service.ts`）。 */
...
function VaultMarkdownEditor({
  ...
  /** 文件树已收起：正文让出左侧 44px 页边给钉在视图左上角的侧栏开关，标题行上提 10px 与开关同轴。 */
  ...
  const bodyEditorRef = React.useRef<LiveMarkdownEditorHandle>(null)

  /** 标题里按回车：按常规 Markdown 编辑器习惯把光标落到正文首行行首。 */
  const focusBodyStart = React.useCallback(() => {
  ...
  return (
    ...
    >
      <div className={cn('mx-auto flex h-full w-full max-w-5xl flex-col px-5 py-5', sidebarCollapsed && 'pl-11 pt-2.5')}>
        ...
        <div className="min-h-0 flex-1">
          {/* 选区浮层由下方 SelectionActionPopover 统一承担（#1913 融合版已含「复制文本」按钮）；
              不再挂独立 TextSelectionCopyLayer——双浮层会同时弹出（维护者报障） */}
...
function VaultMarkdownPane({
  ...
  onOpenTutorial: () => void
  onOpenWikiLink: (target: string) => void
  /** 文件树已收起：空态与标题行都给钉在左上角的侧栏开关让出页边（开关本身由 VaultView 渲染，不经此处）。 */
  sidebarCollapsed?: boolean
}): React.ReactElement {
  if (loading || !readResult) {
    return (
      <section className="flex min-w-0 flex-1 flex-col bg-muted/25">
        <div className={cn('mx-auto flex h-full w-full max-w-5xl flex-col px-5 py-5', sidebarCollapsed && 'pl-11 pt-2.5')}>
          {/* 收起态提示行拉到与标题输入框同高（h-9），行中心落在开关同一轴线上 */}
          <div className={cn('titlebar-no-drag flex min-w-0 items-center gap-2', sidebarCollapsed && 'h-9')}>
...
export function VaultView({ embedded = false, sessionId }: { embedded?: boolean; sessionId?: string }): React.ReactElement {
  ...
  const [vaultSidebarOpen, setVaultSidebarOpen] = useAtom(vaultSidebarOpenAtom)
  const vaultSidebarId = React.useId()
  ...
  const refresh = React.useCallback(async ({ showLoading = false }: { showLoading?: boolean } = {}): Promise<void> => {
    ...
    try {
      ...
      // 上游 v0.19.26 起该 IPC 返回 VaultTreeEntry[]（文件 + 文件夹）；我方树由
      // buildVaultTree 从扁平文件列表自建，只取文件条目。
      const nextFiles = nextEntries.filter((entry): entry is VaultFileEntry => entry.kind === 'file')
  ...
  const openFile = React.useCallback(async (relativePath: string): Promise<void> => {
    ...
    try {
      ...
      if (requestId === readRequestRef.current) {
        setReadResult(null)
        // 树可能已过期（笔记在本渲染进程之外被删除/改名，双链仍按旧树解析到它）：
        // 刷新一次让失效条目消失。refresh 自带「已打开的笔记不存在」提示，这里不再叠加
        // toast.error，避免同一次点击连弹两条。
        if (isVaultFileNotFoundError(error)) void refresh()
        else toast.error(error instanceof Error ? error.message : '无法打开笔记')
      }
    } finally {
      if (requestId === readRequestRef.current) setFileLoading(false)
    }
  }, [refresh, selectFile, setReadResult])

  const openWikiLink = React.useCallback((target: string): void => {
    if (!readResult) return
    const path = resolveVaultWikiLink(target, readResult.relativePath, files.map((file) => file.relativePath))
    if (!path) {
      toast.error(`无法定位笔记“${target}”，请检查名称或使用完整的 ${VAULT_NAME} 内路径`)
      return
    }
    void openFile(path)
  }, [files, openFile, readResult])

  const selectVaultManually = async (): Promise<void> => {
    const selected = await window.electronAPI.selectVault({ inboxPath: CANOPY_BRAND.vault.inboxName, allowAgentWrites: false })
    if (!selected) return
    setConfig(selected)
    selectFile(null)
    setReadResult(null)
  ...
  const createCanopyVault = async (): Promise<void> => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : `无法创建 ${CANOPY_MANAGED_VAULT_DISPLAY_NAME}`)
    }
  }

  const connectDiscoveredVault = async (candidate: VaultCandidate): Promise<void> => {
    try {
      const selected = candidate.isCanopyManaged
        ? await window.electronAPI.selectDefaultVault()
        : await window.electronAPI.authorizeDiscoveredVault(candidate.path, { inboxPath: CANOPY_BRAND.vault.inboxName, allowAgentWrites: false })
      setConfig(selected)
      selectFile(null)
      setReadResult(null)
      setVaultSwitcherOpen(false)
    ...
    <>
      <main className={cn('flex h-full min-h-0 flex-col bg-muted/25', embedded && 'min-w-[360px] bg-content-area')}>
        ...
        <div className="relative flex min-h-0 flex-1">
          {/*
            侧栏开关：一颗按钮、一个 DOM 节点，钉在视图左上角 (12,14)，两态只换图标与文案。
            0.18.50 之前是两个节点（树头部一颗 / 笔记标题行一颗），坐标差 (−4, +10)，收合动画
            期间还会双钮重影；单节点后位置恒定、键盘焦点也不会随收合丢失。
            纵向 14px = 树头部 h-14 的行中心；收起态正文用 pt-2.5 把标题行上提到同一轴线。
          */}
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                type="button"
                aria-label={vaultSidebarOpen ? '收起笔记文件树' : '展开笔记文件树'}
                aria-expanded={vaultSidebarOpen}
                aria-controls={vaultSidebarId}
                onClick={() => setVaultSidebarOpen(!vaultSidebarOpen)}
                className="titlebar-no-drag absolute left-3 top-3.5 z-10 flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
              >
                {vaultSidebarOpen ? <PanelLeftClose size={15} /> : <PanelLeftOpen size={15} />}
              </button>
            </TooltipTrigger>
            <TooltipContent side="right">{vaultSidebarOpen ? '收起文件树' : '展开文件树'}</TooltipContent>
          </Tooltip>
          <aside
            id={vaultSidebarId}
            aria-hidden={!vaultSidebarOpen}
            className={cn(
              'relative flex shrink-0 flex-col overflow-hidden bg-muted/25 transition-[width] duration-200 ease-out',
              vaultSidebarOpen && 'border-r border-border/50',
            )}
            style={{ width: vaultSidebarOpen ? vaultSidebarWidth : 0 }}
          >
            {/* 内层定宽：收合动画期间内容整体滑出，而不是被挤压换行 */}
            <div className={cn('flex h-full min-h-0 flex-col', !vaultSidebarOpen && 'pointer-events-none')} style={{ width: vaultSidebarWidth }}>
              {/*
                左侧 48px 留给钉在容器左上角的侧栏开关（按钮 12~40 + gap 8），Vault 名仍从 x=48 起。
                这 40px 必须是一个 titlebar-no-drag 的占位元素，不能用 padding：主区独立视图里这个 header 是
                拖拽区，而 Electron 把 app-region 矩形按文档顺序依次做 union(drag) / difference(no-drag)——
                开关按钮在 DOM 里排在 header 之前，它自己的 no-drag 会先被减掉、再被 header 整块并回来，
                OS 层直接把点击当成标题栏拖拽，DOM 收不到 click（0.18.55 维护者报「Chat 模式下按钮点不动」，
                OS 级真实鼠标复现；CDP 合成事件绕过 OS hit-test 所以测不出来）。占位元素在 header 之后，
                difference 最后生效，才能真正给按钮留出洞。
              */}
              <header className={cn('flex h-14 items-center gap-2 pr-3', embedded ? 'titlebar-no-drag' : 'titlebar-drag-region')}>
                <div aria-hidden className="titlebar-no-drag h-full w-10 shrink-0" />
                ...
                >
                  ...
                  <PopoverContent side="top" align="start" className="w-72 p-1.5">
                      ...
                      {candidatesLoading ? (
                        ...
                        candidates.map((candidate) => (
                          ...
                          >
                            ...
                            {candidate.isCanopyManaged && <span className="shrink-0 text-[10px] text-muted-foreground">{CANOPY_SELF_MANAGED_VAULT_LABEL}</span>}
                          </button>
                        ))
                      ) : (
                    ...
                    <div className="mt-1 border-t border-border/60 pt-1">
                      <button
                        type="button"
                        onClick={() => { void createCanopyVault() }}
                      ...
                      >
                        ...
                        创建 {CANOPY_MANAGED_VAULT_DISPLAY_NAME}
                      </button>
                      <button
                        type="button"
          ...
          <VaultMarkdownPane
            ...
            sidebarCollapsed={!vaultSidebarOpen}
          />
        </div>
      </main>
      <ConfirmDialog
      ...
      <Dialog open={vaultHelpOpen} onOpenChange={setVaultHelpOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>在 {CANOPY_BRAND.productName} 中使用 {VAULT_NAME}</DialogTitle>
            <DialogDescription>{CANOPY_BRAND.productName} 直接读写本机已授权的 Markdown Vault；这些笔记也会继续保留在 {VAULT_NAME} 中。</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 text-sm leading-6 text-muted-foreground">
            <section>
              <p className="font-medium text-foreground">切换与管理 Vault</p>
              <p>点击左下角的 Vault 名称可切换已发现的本地 Vault，也可以创建 {CANOPY_MANAGED_VAULT_DISPLAY_NAME} 或打开本地仓库。云端 Vault 需先同步或挂载到本机。</p>
            </section>
            <section>
