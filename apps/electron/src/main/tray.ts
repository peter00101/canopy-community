...
function getTrayIconPath(): string {
  ...
  const resourcesDir = app.isPackaged
    ? join(process.resourcesPath, 'canopy-logos')
    : join(__dirname, 'resources/canopy-logos')
...
function buildTrayMenu(actions: TrayActions): Menu {
  ...
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    { type: 'separator' },
    {
      label: `打开 ${CANOPY_BRAND.productName}`,
      click: () => actions.showMainWindow(),
    },
    { type: 'separator' },
    {
      label: `退出 ${CANOPY_BRAND.productName}`,
      click: () => {
        app.quit()
      },
    },
...
export function createTray(actionsInput?: Partial<TrayActions>): Tray | null {
  ...
  try {
    ...
    // Windows 置空 tooltip：避免系统气泡与自绘托盘悬浮层（Agent 状态通知）叠加
    tray.setToolTip(process.platform === 'win32' ? '' : CANOPY_BRAND.productName)
