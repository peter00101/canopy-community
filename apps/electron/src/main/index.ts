...
import { installAppCrashListeners, installCrashGuard, logCrashEvent } from './lib/crash-guard'
import { forwardRootErrorBoundaryLogs } from './lib/renderer-error-log'
import { OFFICECLI_CHILD_ENV } from './lib/officecli-manager'
import { warmupPiRuntimeModule } from '@canopy/runtime-pi'

// 崩溃防护必须最先安装：Agent runtime / Bridge / MCP 都在主进程，任何未捕获异步异常
// 此前都会直接终结整个应用（用户表现为闪退）且不留日志。日志在 ~/.canopy/logs/crash-*.log。
installCrashGuard()
installAppCrashListeners(app)

// 把 OfficeCLI 的自更新抑制开关设进主进程环境，一次覆盖所有后代进程。
//
// 只在 runBundledOfficeCli 里带是不够的：Agent 的可见终端（utilityProcess + node-pty）
// 与 MCP 子进程都直接继承 process.env，Agent 或用户在终端里跑一次包内的 officecli
// 就会让它联网把自己替换掉——之后完整性自检发现哈希不符，Office 预览与三个 Office
// 工具全部停用且无恢复路径，只能重装。三个变量对其他程序无副作用。
Object.assign(process.env, OFFICECLI_CHILD_ENV)

// Dev 与正式版使用独立的 userData 目录，避免共享 Chromium SingletonLock 导致 dev 启动被静默退出
// 必须在任何会读取 userData 路径的模块加载之前执行
const userDataDirBase = CANOPY_BRAND.userDataDirName ?? '@canopy/electron'
if (!app.isPackaged) {
  // 多个 worktree 可显式隔离开发实例，避免其中一个分支抢走另一分支的 Chromium SingletonLock。
  const instance = process.env.CANOPY_DEV_INSTANCE?.replace(/[^a-zA-Z0-9_-]/g, '')
  if (instance) app.setName(`${CANOPY_BRAND.productName}-${instance}`)
  app.setPath('userData', join(app.getPath('appData'), instance ? `${userDataDirBase}-dev-${instance}` : `${userDataDirBase}-dev`))
} else if (CANOPY_BRAND.userDataDirName) {
  // 与其它发行版并存：独立 userData = 独立的单实例锁与 safeStorage 密钥。
  // userDataDirName 为 null 时保持 Electron 默认目录，存量用户的密钥不受影响。
  app.setPath('userData', join(app.getPath('appData'), CANOPY_BRAND.userDataDirName))
}
...
if (!app.requestSingleInstanceLock()) {
  console.warn(
    `[启动] 已有 ${CANOPY_BRAND.productName} 进程持有单实例锁，本次启动将退出。\n` +
      `  如果窗口未出现，可能旧进程已卡死。请运行 \`killall ${CANOPY_BRAND.productName}\` 后重试。`,
  )
  app.quit()
} else {
  // 主流程：正常启动（单实例锁已获取）
...
function registerProtocolsAndHandlers(): void {
  ...
  // 用于内联预览本地文件（renderer 用 iframe 加载 canopy-file:// 资源）
  protocol.registerSchemesAsPrivileged([
    { scheme: 'canopy-file', privileges: { standard: true, secure: true, supportFetchAPI: true, corsEnabled: true, stream: true } },
  ])
...
import { handleAppFileRequest } from './lib/local-file-protocol'
...
import { attachTextContextMenu } from './lib/text-context-menu'
...
import { startScheduler, stopScheduler } from './lib/automation-service'
...
import { setAppVersion } from '@canopy/core'
...
/** Pi Agent SDK 预热延迟：等主窗口渲染与启动 I/O 静默后再后台加载，避免与启动争抢磁盘。 */
const PI_SDK_WARMUP_DELAY_MS = 5_000
 ...
 *
 * 打包后 electron-builder 排除了 dist/resources/（见 electron-builder.yml 的
 * files 段），运行时图标由 extraResources 提供的 resources/icon.png 承担；
 * .ico / .icns 仅用于安装包与可执行文件本身，不进运行时资源目录。
 */
function getIconPath(): string {
  ...
  // dev：resources 在 build:resources 阶段被复制到 dist/ 下，与 main.cjs 同级
...
function createWindow(): void {
  ...
  const titleBarOptions = isMac
    ? {
        ...
        // y=11 让 14px 高的红绿灯垂直居中于 36px 的 MacTitleRow（MAC_TITLEBAR_HEIGHT_PX）；
        // x=18 与标题行左侧 96px 红绿灯占位（MAC_TRAFFIC_LIGHTS_INSET_PX = 18 + 60 + 18）配套，
        // 改一处要同步另一处。侧栏已整体位于标题行之下，不再参与避让。
    ...
    : isWindows
      ? {
          // 0.18.18 撤 WCO 换回自绘按钮（维护者定：系统原生按钮观感差）；hit-test
          // 重叠问题改由渲染层拖拽区/按钮区分矩形隔离解决（上游 #1900 方案）。
  ...
  // 渲染层根级错误边界的日志转进 crash 日志（白屏时唯一线索）
  forwardRootErrorBoundaryLogs(mainWindow.webContents, 'main')
  ...
  const showRendererFailure = (reason: string): void => {
    // webContents 可先于 BrowserWindow 销毁（关窗竞态）；缺一查都会让下方 loadURL 抛
    // 「Object has been destroyed」成为 unhandledRejection（2026-08-31 用户 crash 日志实例）。
    ...
    const escapeHtml = (value: string): string => value
      ...
      .replace(/"/g, '&quot;')
      .replace(/'/g, '&#39;')
    const page = `<!doctype html><html><head><meta charset="utf-8"><title>${CANOPY_BRAND.productName} 无法加载</title><style>body{font:14px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding:48px;color:#222;background:#fff}main{max-width:680px;margin:auto}h1{font-size:20px;font-weight:600}pre{white-space:pre-wrap;background:#f3f3f3;padding:16px;border-radius:8px}a{display:inline-block;margin-top:12px;padding:9px 14px;border-radius:6px;background:#222;color:#fff;text-decoration:none}</style></head><body><main><h1>${CANOPY_BRAND.productName} 无法加载主界面</h1><pre>${escapeHtml(reason)}</pre><a href="${escapeHtml(rendererEntryUrl)}">重新加载主界面</a></main></body></html>`
    try {
      mainWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(page)}`).catch((error) => {
        console.error('[启动] 降级错误页加载失败:', error)
        try { mainWindow?.show() } catch { /* 窗口已销毁 */ }
      })
    } catch (error) {
      // loadURL 在 webContents 销毁竞态下会同步抛而非返回 rejected promise
      console.error('[启动] 降级错误页加载失败（窗口销毁竞态）:', error)
    }
  }
  ...
  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    ...
    logCrashEvent('render-process-gone', `reason=${details.reason} exitCode=${details.exitCode} url=${rendererUrl ?? ''}`)
  ...
  // dev 不再自动弹 DevTools（需要时 F12/Ctrl+Shift+I 手动开）：附着状态下 Chromium 会在
  // 每次窗口 resize 时弹「宽×高px」视口角标，挡住右上角按钮数秒（维护者报障，
  // 曾被误判为 Win11 Snap Layouts）。正式包 isDev=false 本就不开。
  void loadMainRenderer().catch((error) => {
...
async function bootstrap(): Promise<void> {
  ...
  // 同步默认 Skills 模板到 ~/.canopy/default-skills/
  ...
  // Island 服务未初始化的平台（Linux）注册 no-op handler，避免渲染层调用时报
  // "No handler registered"。darwin / win32 的真 handler 由 initAgentIslandService
  // 注册（agent-island-service.ts），这里再注册会因同通道二次注册炸掉 bootstrap
  //（0.17.6 上游把 Island 状态机扩到 Windows 后曾实际发生，触发降级模式）。
  if (!isAgentIslandServiceSupported()) {
    ipcMain.handle('agent-island:mark-session-viewed', () => {})
  }
  ...
  safeRun('registerGlobalShortcut:voice-dictation', () =>
    registerGlobalShortcut('voice-dictation', () => {
      toggleVoiceDictationWindow({ targetIsApp: mainWindow?.isFocused() === true })
    }),
  )
  ...
  // 预热 Pi Agent SDK：约 36MB 模块图仅在首条 Agent 消息时动态 import
  //（agent-orchestrator.ts / pi-agent-adapter.ts），冷启动叠加杀毒实时扫描
  // 首次加载可达数十秒，表现为"第一次对话几十秒无响应"。提前到启动空闲期
  // 后台加载；失败静默，sendMessage 内的动态 import 是天然兜底。
  setTimeout(() => {
    const warmupStart = Date.now()
    void warmupPiRuntimeModule()
      .then(() => {
        console.log(`[启动] Pi Agent SDK 预热完成（耗时 ${Date.now() - warmupStart}ms）`)
      })
      .catch((err) => {
        console.warn('[启动] Pi Agent SDK 预热失败（首条消息时将重试加载）:', err)
      })
  }, PI_SDK_WARMUP_DELAY_MS)
...
function handleBootstrapFailure(err: unknown): void {
  ...
  try {
    ...
    dialog.showErrorBox(
      `${CANOPY_BRAND.productName} 启动遇到错误`,
      `部分功能可能不可用：\n\n${message}\n\n` +
        `日志位置：${app.getPath('logs')}\n\n` +
        `常见原因与排查：\n` +
        `1. 旧版 ${CANOPY_BRAND.productName} 进程未退出（终端运行 killall ${CANOPY_BRAND.productName} 后重试）\n` +
        `2. ~/${CANOPY_BRAND.configDirName}/ 配置损坏（重命名 ~/${CANOPY_BRAND.configDirName} 后重启）\n` +
        `3. 系统 Keychain 无法解密保存的凭证（删除 ~/${CANOPY_BRAND.configDirName}/feishu.json 等后重新登录）\n\n` +
