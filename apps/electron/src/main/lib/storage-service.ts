...
import { rmSyncWithRetry } from '@canopy/kernel'
...
async function calcTempFilesCategory(): Promise<StorageCategory> {
  const previewDir = join(tmpdir(), 'canopy-preview')
  const installerDir = join(app.getPath('temp'), 'canopy-installers')
...
async function cleanupOrphanAgentSessions(): Promise<CleanupResult> {
  ...
  try {
    const files = await fsPromises.readdir(dir)
    for (const file of files) {
      // 会话正文 {id}.jsonl 与 Skill 激活 sidecar {id}.skill-activations.json 同属一个会话
      const id = file.endsWith('.jsonl')
        ? basename(file, '.jsonl')
        : file.endsWith('.skill-activations.json')
          ? basename(file, '.skill-activations.json')
          : null
