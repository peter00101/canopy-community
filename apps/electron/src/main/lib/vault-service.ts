...
import { readJsonFileSafe, writeJsonFileAtomic, writeTextFileAtomic } from '@canopy/kernel'
...
export function createVaultFileSystem(rootPath: string): VaultFileSystem {
  ...
  const writeFile = (input: VaultWriteInput): VaultWriteResult => {
    ...
    writeTextFileAtomic(revalidated.absolutePath, input.content, true) // skipBackup: Vault 是用户可见目录(Obsidian/文件浏览器),不留 .bak 污染
...
function configureVaultAt(rootPath: string, configPath: string, options: { inboxPath?: string; allowAgentWrites?: boolean } = {}): VaultSummary {
  const root = assertVaultRoot(rootPath)
  const inboxPath = options.inboxPath?.trim() || CANOPY_BRAND.vault.inboxName
  const normalizedInboxPath = normalizeRelativeMarkdownPath(join(inboxPath, 'placeholder.md')).replace(/\/placeholder\.md$/, '')
  ...
  const config: VaultConfig = {
    rootPath: root,
    displayName: isManagedRoot ? CANOPY_BRAND.vault.managedLabel : basename(root) || 'Vault',
...
export function selectDefaultVault(): VaultSummary {
  return configureVault(getDefaultVaultDir(), { inboxPath: CANOPY_BRAND.vault.inboxName, allowAgentWrites: false })
}
...
export function discoverVaultCandidates(): VaultCandidate[] {
  ...
  const candidates: VaultCandidate[] = managedRoot
    ? [{ path: managedRoot, displayName: CANOPY_BRAND.vault.managedLabel, isObsidianVault: existsSync(join(managedRoot, '.obsidian')), isCanopyManaged: true }]
    : []
 ...
 * Obsidian's registry is the source of truth; the selected Canopy Vault is also
