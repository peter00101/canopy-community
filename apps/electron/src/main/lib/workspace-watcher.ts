 ...
 * 工作区文件监听器
 *
 * 使用 fs.watch 递归监听 ~/.canopy/agent-workspaces/ 目录，
 * 根据变化的文件路径区分事件类型：
...
import { classifyWorkspaceWatchFilename, isHighNoisePath, normalizeWatchFilename, shouldFlushPendingWatchEvents, shouldNotifyForWatchFilename } from './workspace-watcher-utils'
 ...
 * debounce 的兜底上限（ms）。
 *
 * 纯 debounce 在**持续**写入下会饿死：每个事件都重置定时器，只要写入间隔一直
 * 小于 DEBOUNCE_MS 就永远不 flush，待发集合无界增长（Agent 跑批量下载脚本写几千
 * 个文件即触发）。这里保证从第一个待发事件起最多攒这么久就必须发一次。
 */
const MAX_DEBOUNCE_WAIT_MS = 2000

/** 单批待发路径上限，超过立即 flush，避免一次性向渲染层倾泻上万条路径。 */
...
/** 附加目录本批第一个待发事件的时间，用于兜底 flush；无待发时为 null。 */
let firstPendingAttachedAt: number | null = null
...
function notifyWorkspaceFilesChanged(changedPath?: string): void {
  ...
  if (firstPendingAttachedAt === null) firstPendingAttachedAt = Date.now()

  const flushAttached = (): void => {
    if (attachedFilesTimer) {
      clearTimeout(attachedFilesTimer)
      attachedFilesTimer = null
    }
    firstPendingAttachedAt = null
    const changedPaths = [...attachedChangedPaths]
    attachedChangedPaths.clear()
    if (mainWin && !mainWin.isDestroyed()) {
      // 保留删除路径（上游 #1886），使当前预览也能从旧缓存切换到「文件不存在」状态；
      // renderer 在记录会话文件改动前会自行确认路径仍是实际文件。
      mainWin.webContents.send(AGENT_IPC_CHANNELS.WORKSPACE_FILES_CHANGED, changedPaths)
    }
  }

  // 与主监听同样的兜底：附加目录常是用户的真实项目目录，批量写入更密集
  if (shouldFlushPendingWatchEvents({
    pendingCount: attachedChangedPaths.size,
    firstPendingAt: firstPendingAttachedAt,
    now: Date.now(),
    maxPending: MAX_PENDING_PATHS,
    maxWaitMs: MAX_DEBOUNCE_WAIT_MS,
  })) {
    flushAttached()
    return
  }
  if (attachedFilesTimer) clearTimeout(attachedFilesTimer)
  attachedFilesTimer = setTimeout(flushAttached, DEBOUNCE_MS)
}
...
export function startWorkspaceWatcher(win: BrowserWindow): void {
  ...
  /** 本批第一个待发事件的时间，用于兜底 flush；无待发时为 null。 */
  let firstPendingFileAt: number | null = null
  ...
  try {
    watcher = watch(watchDir, { recursive: true }, (_eventType, filename) => {
      ...
      // 仅工作区顶层的 mcp.json / skills/ / skills-inactive/ 才是能力变更；
      // 顶层 config.json 与所有噪声路径直接丢弃，其余一律按用户文件处理。
      ...
      if (changeType === 'capabilities') {
        ...
        if (firstPendingFileAt === null) firstPendingFileAt = Date.now()
        ...
        // 攒太多或等太久都立刻发一批，避免持续写入把 debounce 饿死
        if (shouldFlushPendingWatchEvents({
          pendingCount: changedFilePaths.size,
          firstPendingAt: firstPendingFileAt,
          now: Date.now(),
          maxPending: MAX_PENDING_PATHS,
          maxWaitMs: MAX_DEBOUNCE_WAIT_MS,
        })) {
          ...
          filesTimer = setTimeout(flushFiles, DEBOUNCE_MS)
        }
      }
    })
