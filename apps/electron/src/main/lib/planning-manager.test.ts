...
/**
 * Electron 可执行文件路径。刻意不走 `require('electron')`：成套混跑时，别的测试文件的
 * `mock.module('electron', …)` 是进程级全局的，这里会拿到一个桩对象而不是路径字符串，
 * `spawnSync` 直接抛「file 参数必须是字符串」。改为照 electron 包自己的算法
 * 读 `path.txt`——只 resolve 不加载，任何模块 mock 都碰不到。
 */
function resolveElectronBinary(): string {
  ...
  return join(process.env.ELECTRON_OVERRIDE_DIST_PATH ?? join(electronDir, 'dist'), executable)
}
...
test('Given a fresh planning database When planning data changes Then isolation, transactions, reminders and optimistic versions stay correct', async () => {
  const home = mkdtempSync(join(tmpdir(), 'canopy-planning-'))
  ...
  const source = `
    ...
    const configDir = join(process.env.HOME, CANOPY_BRAND.configDirNameDev)
  ...
  try {
    ...
    const result = spawnSync(electronBinary, [outputPath], {
      cwd: repoRoot,
      env: { ...process.env, ELECTRON_RUN_AS_NODE: '1', HOME: home, CANOPY_DEV: '1' },
      encoding: 'utf8',
    })
