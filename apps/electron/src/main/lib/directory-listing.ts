...
import { isTransientFileName } from './file-visibility'
...
/** stat 单个条目时可容忍的错误：条目照常列出，只是拿不到大小。 */
const STAT_TOLERATED_CODES = ['EBUSY', 'EPERM', 'EACCES'] as const
...
export function listShallowDirectory(
  ...
  for (const item of items) {
    ...
    // 临时 / 锁文件几秒内出现又消失，展示出来会让下方各行反复上下跳动（口径见 file-visibility.ts）。
    if (!isDirectory && isTransientFileName(item.name)) continue
    ...
    if (!isDirectory) {
      try {
        ...
        // 被独占打开 / 无权读属性的单个文件（如盘根的 pagefile.sys 报 EBUSY）照常列出、只是没有大小；
        // 不能让一个条目拖垮整个目录，否则文件树会被整棵换成一行错误。
        if (!STAT_TOLERATED_CODES.some((code) => hasErrorCode(error, code))) throw error
      }
    }
