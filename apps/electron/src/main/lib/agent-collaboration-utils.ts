...
const PERMISSION_RANK: Record<AppPermissionMode, number> = {
  plan: 0,
  bypassPermissions: 1,
}
 ...
 * 会话中的工具调用 ID 无限累积。
 */
// ⚠️ 上游 #1770 在扩展委派等待时长时顺手删掉了本函数唯一的测试（agent-collaboration-utils.test.ts），
// 而两个委派工具仍靠它防重放——该删除不采，测试已保留。
...
export function resolveDelegationPermissionMode(
  _parentMode: AppPermissionMode | undefined,
  _requestedMode: AppPermissionMode | undefined,
): AppPermissionMode {
  // Pi 子会话目前固定直接执行。
...
export function buildRecoveredDelegationState(input: {
  ...
  fallbackPermissionMode?: AppPermissionMode
  ...
  return {
    ...
    permissionMode: input.session.permissionMode ?? input.fallbackPermissionMode ?? CANOPY_DEFAULT_PERMISSION_MODE,
    status,
...
export function buildDelegationPrompt(input: {
  ...
  return `你是 ${CANOPY_BRAND.productName} 协作子 Agent。你由父 Agent 会话 ${input.parentSessionId} 委派创建，委派 ID 为 ${input.delegationId}。
