...
import { forwardRootErrorBoundaryLogs } from './renderer-error-log'

const DEFAULT_WIDTH = 980
const DEFAULT_HEIGHT = 720
const MIN_WIDTH = 680
const MIN_HEIGHT = 480
const MEMORY_WINDOW_TITLE = `${CANOPY_BRAND.productName} · 工作区记忆`
...
function createWorkspaceMemoryWindow(workspaceSlug: string, relativePath?: string): BrowserWindow {
  ...
  const titleBarOptions = isMac
    ...
    : isWindows
      ? {
          // 与主窗口一致：0.18.18 撤 WCO 换回自绘按钮（hit-test 由渲染层分矩形隔离）
  ...
  forwardRootErrorBoundaryLogs(win.webContents, 'workspace-memory')
