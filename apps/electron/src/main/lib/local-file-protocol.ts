 ...
 *
 * The renderer never receives raw canopy-file:// absolute paths. Main process
...
function registerEntry(path: string, isDirectory: boolean): string {
  ...
  return `canopy-file://${token}`
}

export function registerAppFilePath(path: string): string {
  return registerEntry(path, false)
}

export function registerAppDirectoryPath(path: string): string {
  return registerEntry(path, true)
}

export function handleAppFileRequest(request: Request): Promise<Response> | Response {
  let url: URL
  try {
