 ...
 *
 * 管理应用设置（主题模式等）的读写。
 * 存储在 ~/.canopy/settings.json
 */

import { readFileSync, writeFileSync, existsSync } from 'node:fs'
import { getSettingsPath } from './config-paths'
import { DEFAULT_INTERFACE_VARIANT, DEFAULT_THEME_MODE, DEFAULT_THEME_STYLE, normalizeProductivityToolsSettings } from '../../types'
import type { AgentIslandSettings, AppSettings } from '../../types'
import { getTerminalProfilesForPlatform, isTerminalProfile } from '@canopy/shared'
...
export function getSettings(): AppSettings {
  ...
  if (!existsSync(filePath)) {
    return {
      ...
      richTextRenderingEnabled: true,
      feishuSessionMirror: { mode: 'off' },
      visionRelay: { enabled: false },
      videoRelay: { enabled: false },
  ...
  try {
    ...
    return {
      ...
      // 只有 themeMode === 'special' 时 themeStyle 才生效，所以显式选过 light/dark 的老用户
      // 即便这里回落到门面主题也不会被改变观感
      ...
      richTextRenderingEnabled: data.richTextRenderingEnabled ?? true,
      feishuSessionMirror: data.feishuSessionMirror ?? { mode: 'off' },
      visionRelay: data.visionRelay ?? { enabled: false },
      videoRelay: data.videoRelay ?? { enabled: false },
