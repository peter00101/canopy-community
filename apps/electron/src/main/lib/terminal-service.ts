...
import { TerminalOutputAckFallback } from './terminal-output-ack-fallback'
...
// 渲染层的终端标签是唯一的 ACK 发起方；它没挂载时由主进程代 ACK，回放缓冲与退出事件才不会卡在第一块。
const ackFallback = new TerminalOutputAckFallback({ acknowledge: (input) => terminalRuntimeClient.acknowledgeOutput(input) })
...
export async function createTerminal(
  ...
  terminalOutputBuffers.set(input.terminalId, new TerminalOutputBuffer(MAX_REPLAY_CHARS))
...
export function getTerminalSnapshot(terminalId: string): TerminalSnapshot {
  ...
  const buffer = terminalOutputBuffers.get(terminalId)
  return { state, output: buffer?.text() ?? '', sequence: buffer?.sequence ?? 0 }
}
...
export function acknowledgeTerminalOutput(input: TerminalOutputAck): void {
  ...
  if (!terminals.has(input.terminalId)) return
  ackFallback.observeRendererAck(input.terminalId, input.sequence)
...
function appendOutputBuffer(event: { terminalId: string; sequence: number; data: string }): void {
  let buffer = terminalOutputBuffers.get(event.terminalId)
  if (!buffer) {
    buffer = new TerminalOutputBuffer(MAX_REPLAY_CHARS)
    terminalOutputBuffers.set(event.terminalId, buffer)
  }
  buffer.append(event)
}
