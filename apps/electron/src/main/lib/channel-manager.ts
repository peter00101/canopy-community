...
import { refreshCodexOAuth, refreshXaiOAuth, refreshXaiOAuthCredentialsSerial, rememberXaiOAuthCredentials, listCodexModels, listXaiModels } from '@canopy/runtime-pi'
...
import { normalizeHttpResponse, normalizeModelsProbeResponse, normalizeRequestError } from './channel-test-error'
import { writeJsonFileAtomic } from '@canopy/kernel'
...
/** 合并预设模型时按 id 去重；先出现的优先，保留我方已实测过的展示名与启用状态。 */
function dedupeModelsById(models: readonly ChannelModel[]): ChannelModel[] {
...
/**
 * 火山方舟 Coding Plan 预设模型 = 我方已实测可用的三个 + 上游 #1767 的套餐支持清单，按 id 去重。
 * `/models` 返回的是方舟通用目录（不等于套餐授权清单），所以这里不走接口拉取。
 */
const ARK_CODING_PLAN_MODELS: ChannelModel[] = dedupeModelsById([
...
/**
 * 一次性模型候选更新（上游 #1999 起改为逐条登记）。每条更新有独立 ID，新增模型时只追加新条目，
 * 不会把用户已删除的旧候选重新塞回去；ID 不依赖 config schema version，更高版本配置也能安全收到。
 * 候选默认禁用，不在每次启动时按完整预设列表补齐，以尊重用户主动删除过的模型。
 *
 * 上游 #1999 还带了一条「显示名纠错」更新（把曾发布过的错误默认名 GPT-6-Astra 改回 GPT-6 Astra）；
 * 我方从未发过错误名，该条连同 nameCorrections 机制一并不收。
 */
/** 一次性改名：供应商废弃了旧模型 ID 时，把用户渠道里的旧条目原地换成新 ID（见 applyPresetModelRename）。 */
export interface PresetModelRename {
  fromId: string
  /** 旧条目的默认显示名；用户自己改过名的条目保留其自定义名。 */
  fromName: string
  toId: string
  toName: string
}
...
/**
 * DeepSeek 2026-09 起 V4.1 Flash 的现行 ID 是 deepseek-flash（官方定价页上没有 deepseek-v4.1-flash 这个 ID）。
 * 0.18.77~0.18.82 随上游 #2036 追加进用户渠道的 deepseek-v4.1-flash 候选在 API 上无效，
 * 这里原地改名而不是留着一条废 ID（上游 #2042 只改了候选 ID，老用户渠道里的旧条目会一直留着）。
 */
const DEEPSEEK_FLASH_RENAME: PresetModelRename = {
  fromId: 'deepseek-v4.1-flash',
  fromName: 'DeepSeek V4.1 Flash',
  toId: 'deepseek-flash',
...
const PRESET_MODEL_CANDIDATE_UPDATES: readonly {
  ...
  /** 先于 candidates 应用：改名保留启用状态；新 ID 已存在时直接删掉旧条目。 */
  renames?: Partial<Record<ProviderType, readonly PresetModelRename[]>>
}[] = [
  {
    id: 'deepseek-flash-v1',
    renames: {
      deepseek: [DEEPSEEK_FLASH_RENAME],
      'ark-coding-plan': [DEEPSEEK_FLASH_RENAME],
    },
    candidates: {
      deepseek: [
  ...
  {
    // 已应用 model-candidates-v3 的存量智谱渠道也要收到 FlashX（收上游 #2077）。
  ...
  },
  {
    ...
    candidates: {
      // Codex 登录 / 模型拉取会启用全部精选模型；存量 Codex 渠道的迁移保持同一语义（enabled: true）。
      'openai-codex': [
 ...
 *
 * v2 → v3：使用默认 DashScope Anthropic 兼容端点的通义千问渠道切换为 OpenAI 兼容端点（上游 #1766）。
 * 上游的 v2→v3 / v3→v4（火山方舟改名、豆包 API → 火山引擎 API）属不采的 #1735，我方版本号不跟随上游。
 *
...
function migrateConfig(config: ChannelsConfig): { config: ChannelsConfig; changed: boolean } {
  ...
  const channels = config.channels.map((channel) => {
    ...
    // v1 → v2：仅 custom / anthropic-compatible 的 baseUrl 语义变更。
    if (version < 2 && (migratedChannel.provider === 'custom' || migratedChannel.provider === 'anthropic-compatible')) {
      const migratedUrl = migrateCompatibleChannelBaseUrl(migratedChannel.baseUrl, migratedChannel.provider)
      if (migratedUrl !== migratedChannel.baseUrl) {
        console.log(
          `[渠道管理] v${version}→v2 迁移渠道 ${migratedChannel.name} (${migratedChannel.provider}) Base URL: ${migratedChannel.baseUrl} → ${migratedUrl}`,
        )
        migratedChannel = { ...migratedChannel, baseUrl: migratedUrl }
      }
    }

    // v2 → v3：通义千问默认 DashScope Anthropic 兼容端点拉不到模型列表，切到 OpenAI 兼容端点（上游 #1766）。
    ...
    if (version < 3
      ...
      console.log(
        `[渠道管理] v${version}→v3 迁移渠道 ${migratedChannel.name}：通义千问 Anthropic 兼容端点 → OpenAI 兼容端点`,
      )
      migratedChannel = {
        ...migratedChannel,
        provider: 'qwen',
...
/**
 * 应用尚未执行过的一次性预设模型更新。更新 ID 不依赖 schema version，以兼容由其他版本写入的更高配置版本；
 * 已应用过的条目跳过，所以老用户只会收到新增的那几条。
 */
/**
 * 原地改名：旧 ID 不存在则返回 undefined（无事发生）；新 ID 已存在则删掉旧条目（用户自己加过新 ID，保留用户那条）；
 * 否则把旧条目换成新 ID，启用状态原样保留，显示名只在仍是旧默认名时才跟着换（用户改过的名字不动）。
 */
/**
 * 预设迁移判断「模型是否已存在」的口径：忽略大小写与首尾空白，与 Pi 运行时查模型的口径一致，
 * 用户配置里 `GLM-5.3-FlashX` / 带空格的旧条目不会被当成缺失而重复追加（收上游 #2077，我方改名迁移同样适用）。
 */
function normalizePresetModelId(modelId: string): string {
  return modelId.trim().toLowerCase()
}

/** 导出供测试直接钉住改名边界（大小写 / 空白变体、新旧同名）。 */
export function applyPresetModelRename(models: readonly ChannelModel[], rename: PresetModelRename): ChannelModel[] | undefined {
  const fromId = normalizePresetModelId(rename.fromId)
  const toId = normalizePresetModelId(rename.toId)
  // 归一化后新旧同名（只改大小写）不算改名；否则下面「新 ID 已存在」会命中旧条目自身把它删掉。
  if (fromId === toId) return undefined
  const fromIndex = models.findIndex((model) => normalizePresetModelId(model.id) === fromId)
  if (fromIndex === -1) return undefined
  if (models.some((model) => normalizePresetModelId(model.id) === toId)) {
    return models.filter((model) => normalizePresetModelId(model.id) !== fromId)
  }
  return models.map((model, index) => (
    index === fromIndex
      ? { ...model, id: rename.toId, name: model.name === rename.fromName ? rename.toName : model.name }
      : model
  ))
}
...
function applyPresetModelCandidateUpdates(config: ChannelsConfig): { config: ChannelsConfig; changed: boolean } {
  ...
  for (const update of PRESET_MODEL_CANDIDATE_UPDATES) {
    ...
    channels = channels.map((channel) => {
      const renames = update.renames?.[channel.provider] ?? []
      const candidates = update.candidates[channel.provider]
      if (renames.length === 0 && !candidates) return channel

      let models: ChannelModel[] = channel.models
      let touched = false
      for (const rename of renames) {
        const renamed = applyPresetModelRename(models, rename)
        if (!renamed) continue
        models = renamed
        touched = true
        console.log(
          `[渠道管理] 预设更新 ${update.id} 把渠道 ${channel.name} (${channel.provider}) 的 ${rename.fromId} 改为 ${rename.toId}`,
        )
      }

      if (candidates) {
        const existingModelIds = new Set(models.map((model) => normalizePresetModelId(model.id)))
        const missingCandidates = candidates.filter((model) => !existingModelIds.has(normalizePresetModelId(model.id)))
        if (missingCandidates.length > 0) {
          console.log(
            `[渠道管理] 预设更新 ${update.id} 为渠道 ${channel.name} (${channel.provider}) 添加 ${missingCandidates.length} 个候选模型`,
          )
          models = [...models, ...cloneModels(missingCandidates)]
          touched = true
        }
      }

      return touched ? { ...channel, models } : channel
    })
...
/**
 * 火山方舟 Coding Plan 曾经的接入地址（上游 preset 引入时的值，官方现已切换为 /api/coding）。
 */
const ARK_CODING_PLAN_LEGACY_BASE_URL = 'https://ark.cn-beijing.volces.com/api/plan'

/**
 * 纠正火山方舟 Coding Plan 的过时接入地址。
 *
 * 官方现行文档的 Anthropic 协议地址为 /api/coding；旧地址 /api/plan 不消耗
 * Coding Plan 套餐额度且可能按普通 API 额外计费。存量渠道多为预设默认值
 * 原样落盘，读取时幂等纠正（含从旧版本备份恢复配置的场景）。
 */
function migrateArkCodingPlanBaseUrls(config: ChannelsConfig): { config: ChannelsConfig; changed: boolean } {
  let mutated = false
  const channels = config.channels.map((channel) => {
    ...
    if (normalized !== ARK_CODING_PLAN_LEGACY_BASE_URL) {
      return channel
    }
    mutated = true
    const nextBaseUrl = PROVIDER_DEFAULT_URLS['ark-coding-plan']
    console.log(
      `[渠道管理] 渠道 ${channel.name} (ark-coding-plan) Base URL 已由旧地址迁移: ${channel.baseUrl} → ${nextBaseUrl}`,
    )
    return { ...channel, baseUrl: nextBaseUrl }
  })

  if (!mutated) {
    return { config, changed: false }
  }
  return { config: { ...config, channels }, changed: true }
}

/**
 * 去掉渠道里官方已下线的模型条目（名单与判定在 @canopy/shared 的 retired-models）。
 * 不分渠道类型：官方下线后 Codex 订阅、官方 API、中转网关都不会再有它。无可删时返回原数组。
 */
function dropRetiredModels(models: ChannelModel[]): ChannelModel[] {
  if (!models.some((model) => isRetiredModelId(model.id))) {
    return models
  }
  return models.filter((model) => !isRetiredModelId(model.id))
}

/**
 * 清掉渠道配置里官方已下线的模型（上游 #2061 只过滤了 Codex 模型目录）。
 *
 * 老版本「拉取模型」会把 gpt-5.3-codex-spark 以 fetched + enabled 写进 channels.json，
 * 光过滤目录清不掉这些存量条目。这里与锁死 Base URL 同法，读取时幂等纠正：
 * 下线名单以后再加模型也不用新登记迁移 ID，从旧版本备份恢复的配置同样会被清掉。
 * 引用该模型的会话 / 默认模型 / 定时任务不在这里改写——发送时会明确报「已下线，请换一个模型」，不静默换模型。
 */
function pruneRetiredModels(config: ChannelsConfig): { config: ChannelsConfig; changed: boolean } {
  let mutated = false
  const channels = config.channels.map((channel) => {
    const models = dropRetiredModels(channel.models)
    if (models === channel.models) {
      return channel
    }
    mutated = true
    console.log(
      `[渠道管理] 渠道 ${channel.name} (${channel.provider}) 已移除 ${channel.models.length - models.length} 个官方已下线的模型`,
    )
 ...
 * 强制覆写锁死供应商的 Base URL。
 *
 * 锁死供应商的地址由产品统一托管：即使配置文件被手工修改，
 * 或历史版本存了旧地址，读取时也会在此纠正并回写。
 */
function enforceLockedBaseUrls(config: ChannelsConfig): { config: ChannelsConfig; changed: boolean } {
  let mutated = false
  const channels = config.channels.map((channel) => {
    const locked = getLockedChannelBaseUrl(channel.provider)
    if (locked === null || channel.baseUrl === locked) {
      return channel
    }
    mutated = true
    console.log(
      `[渠道管理] 渠道 ${channel.name} (${channel.provider}) Base URL 已重置为锁定地址`,
    )
    return { ...channel, baseUrl: locked }
  })
...
function readConfig(): ChannelsConfig {
  ...
  try {
    ...
    const arkMigrated = migrateArkCodingPlanBaseUrls(schemaMigration.config)
    const presetModelUpdate = applyPresetModelCandidateUpdates(arkMigrated.config)
    // 排在预设更新之后：任何预设迁移都不能把官方已下线的模型塞回去
    const retiredPruned = pruneRetiredModels(presetModelUpdate.config)
    // 锁死供应商的 Base URL（红线）：最后一步统一纠正，任何迁移都不能把它改坏
    const enforced = enforceLockedBaseUrls(retiredPruned.config)
    if (schemaMigration.changed || arkMigrated.changed || presetModelUpdate.changed || retiredPruned.changed || enforced.changed) {
      writeConfig(enforced.config)
...
export function createChannel(input: ChannelCreateInput): Channel {
  ...
  const channel: Channel = {
    ...
    // 锁死供应商忽略传入值，始终使用锁定地址
    baseUrl: getLockedChannelBaseUrl(input.provider) ?? input.baseUrl,
    apiKey: encryptApiKey(input.apiKey),
    // 与读取时纠正同一规则：官方已下线的模型不落盘，返回值与下次读取保持一致
    models: dropRetiredModels(input.models),
...
export function updateChannel(id: string, input: ChannelUpdateInput): Channel {
  ...
  const updated: Channel = {
    ...
    // 锁死供应商忽略传入值，始终使用锁定地址
    baseUrl: getLockedChannelBaseUrl(nextProvider) ?? input.baseUrl ?? existing.baseUrl,
    apiKey: input.apiKey ? encryptApiKey(input.apiKey) : existing.apiKey,
    models: dropRetiredModels(input.models ?? existing.models),
...
/** 保存 Pi 或 Canopy 刷新后的完整 Codex OAuth 凭据。 */
...
export async function resolveCodexOAuthCredentials(channelId: string): Promise<CodexOAuthCredentials> {
  ...
  const refreshPromise = (async (): Promise<CodexOAuthCredentials> => {
    try {
      const refreshed = await refreshCodexOAuth(getEffectiveProxyUrl, credentials.refresh)
      const merged = {
        ...refreshed,
...
/** 保存 Pi 或 Canopy 刷新后的完整 xAI OAuth 凭据。 */
...
export async function resolveXaiOAuthCredentials(channelId: string): Promise<XaiOAuthCredentials> {
  ...
  const refreshed = await refreshXaiOAuthCredentialsSerial(
    channelId,
    credentials,
    (current) => refreshXaiOAuth(getEffectiveProxyUrl, current.refresh),
  )
...
export async function testChannel(channelId: string): Promise<ChannelTestResult> {
  ...
  try {
    switch (provider) {
      ...
      default:
        return { success: false, message: `不支持的供应商: ${provider}。该渠道可能由旧版本创建，请删除后重新添加。` }
    }
  } catch (error) {
 ...
 * Coding Plan 渠道必须发送 Canopy User-Agent，否则返回 403。
 */
async function testAnthropicCompatible(
  ...
  // 2xx 返回网页 = Base URL 指向站点首页而非 API（0.18.67）
  return normalizeModelsProbeResponse(response)
}
...
async function fetchZhipuTeamQuota(
  credentials: import('@canopy/shared').ZhipuTeamCredentials,
  proxyUrl?: string,
): Promise<ZhipuQuotaResponse | { error: string }> {
  const fetchFn = getFetchFn(proxyUrl)
  const headers: Record<string, string> = {
 ...
 *
 * 总出口统一剥除模型 id / 显示名里的不可见字符（bidi 控制符、零宽字符等）——
 * new-api 系服务端的 anthropic `/v1/models` 会把上游渠道的脏 id 原样透传，
 * 内嵌 U+202E 时 UI 渲染成「镜像文字」、拿去发 messages 请求必 404。
 * 在总出口做而不是各分支里做，保证覆盖全部 provider。
 */
export async function fetchModels(input: FetchModelsInput): Promise<FetchModelsResult> {
  ...
  const sanitized = sanitizeChannelModels(result.models)
  // 网关的 /models 可能仍列着官方已下线的模型，拉取时一并去掉，免得用户再把它加回渠道
  const available = dropRetiredModels(sanitized)
  const notes: string[] = []
  if (sanitized.length !== result.models.length) {
    // 清洗后条目变少（id 全为控制符被丢弃 / 洗后同 id 去重），把差额告知用户
    notes.push(`已过滤 ${result.models.length - sanitized.length} 个异常模型条目`)
  }
  if (available.length !== sanitized.length) {
    notes.push(`已过滤 ${sanitized.length - available.length} 个官方已下线的模型`)
  }
  if (notes.length === 0) {
    return { ...result, models: available }
  }
  return {
    ...result,
    models: available,
    message: `${result.message}（${notes.join('；')}）`,
  }
}

async function fetchModelsFromProvider(input: FetchModelsInput): Promise<FetchModelsResult> {
  const proxyUrl = await getEffectiveProxyUrl()
  const provider = inferProviderFromBaseUrl(input.provider, input.baseUrl)
  ...
  try {
    switch (provider) {
      ...
      case 'xai':
        ...
        if (provider === 'ark-coding-plan') {
          // 上游 #1767：Coding Plan 的 /models 是方舟通用目录，不等于套餐授权清单，只能用预设白名单。
 ...
 * Coding Plan 渠道必须发送 Canopy User-Agent。
