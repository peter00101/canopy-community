...
function createRepository(prefix: string, fileName: string): string {
  ...
  execFileSync('git', ['config', 'user.email', 'test@canopy.local'], { cwd: path, stdio: 'pipe' })
  execFileSync('git', ['config', 'user.name', 'Canopy Test'], { cwd: path, stdio: 'pipe' })
...
beforeEach(() => {
  repoPath = createRepository('canopy-git-diff-', 'tracked.txt')
})
...
describe('git diff scan cache', () => {
  ...
  test('refreshes a linked worktree when its symbolic HEAD ref changes outside the watcher', async () => {
    const linkedParentPath = mkdtempSync(join(tmpdir(), 'canopy-git-diff-linked-parent-'))
  ...
  test('keeps another repository cache valid after targeted invalidation', async () => {
    const secondRepo = createRepository('canopy-git-diff-second-', 'other.txt')
    try {
