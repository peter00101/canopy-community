...
async function cleanupItem(item: PlanningSyncCleanupItem): Promise<void> {
  await removePlanningNativeSyncItem(item.entity, { targetId: item.targetId, identity: item.appEntityId, calendarItemIdentifier: item.calendarItemIdentifier, startAt: item.nativeStartAt })
...
async function syncNativeItem(item: PlanningNativeOutboxItem): Promise<void> {
  if (item.operation === 'hide') {
    ...
    if (item.connection.canWrite) await removePlanningNativeSyncItem(item.connection.entity, { targetId: item.connection.targetId, identity: item.appEntityId, calendarItemIdentifier: item.calendarItemIdentifier })
  ...
  if (item.connection.entity === 'calendar') {
    const event = getCalendarEvent(item.appEntityId)
    if (!event) { completePlanningNativeOutbox({ ...item, operation: 'hide' }); return }
    const identifiers = await upsertPlanningNativeSyncItem('calendar', { targetId: item.connection.targetId, identity: item.appEntityId, calendarItemIdentifier: item.calendarItemIdentifier, allowRecreate: item.recreatePending, title: event.title, notes: event.notes, startAt: event.startAt, endAt: event.endAt, allDay: event.allDay })
    completePlanningNativeOutbox(item, identifiers)
    return
  } else {
    const todo = getTodo(item.appEntityId)
    if (!todo) { completePlanningNativeOutbox({ ...item, operation: 'hide' }); return }
    const identifiers = await upsertPlanningNativeSyncItem('reminder', { targetId: item.connection.targetId, identity: item.appEntityId, calendarItemIdentifier: item.calendarItemIdentifier, allowRecreate: item.recreatePending, title: todo.title, notes: todo.notes, dueAt: todo.dueAt, dueDateOnly: item.dueDateOnly ?? isTodoDueDateOnly(todo.dueAt), priority: todo.priority, completed: todo.status === 'completed', completedAt: todo.completedAt })
    completePlanningNativeOutbox(item, identifiers)
  }
}
...
async function syncItem(item: PlanningSyncOutboxItem): Promise<void> {
  ...
  if (item.operation === 'delete') {
    await removePlanningNativeSyncItem(entity, { targetId: item.profile.targetId, identity: item.appEntityId, calendarItemIdentifier: item.calendarItemIdentifier, startAt: item.nativeStartAt })
