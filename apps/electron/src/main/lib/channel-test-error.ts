...
import type { ChannelTestErrorType, ChannelTestResult } from '@canopy/shared'
...
/**
 * 2xx 响应体是否明显是网页而不是 API 应答。
 *
 * 背景（0.18.67 维护者报障）：中转站域名常对**任意路径**都返回 200 + 站点首页 HTML。
 * 渠道「测试」只探 `/models` 且只看状态码，于是 Base URL 填成协议根（缺 `/v1`）时测试
 * 照样绿灯，而真正发请求的链路（标题生成走 `/v1/messages`）早已 404 —— 用户拿不到
 * 任何信号。实测：`https://<网关>/models` → 200 HTML，`https://<网关>/v1/models` → 401。
 *
 * 只在 2xx 上判定；4xx/5xx 交给既有的状态码归类，它们本来就会返回 HTML 错误页。
 */
export function looksLikeHtmlPage(contentType: string | null | undefined, bodyText: string): boolean {
...
/**
 * 归一化「GET 模型列表」这类探测请求的响应。
 *
 * 与 `normalizeHttpResponse` 的唯一差别：2xx 但响应体是网页时判失败，并明确指向 Base URL
 * 填写错误，而不是报「连接成功」。
 */
export async function normalizeModelsProbeResponse(response: Response): Promise<ChannelTestResult> {
  if (!response.ok) return normalizeHttpResponse(response)

  const contentType = response.headers.get('content-type')
  const bodyText = await readBoundedText(response)
  if (looksLikeHtmlPage(contentType, bodyText)) {
    return buildFailure(
      'not_found',
      '该地址返回的是网页而不是 API 应答，请检查 Base URL 是否填成了站点首页（通常应填到 /v1 或完整请求端点）',
      response.status,
    )
  }
  return { success: true, message: '连接成功' }
