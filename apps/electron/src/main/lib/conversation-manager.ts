 ...
 *
 * 负责对话的 CRUD 操作和消息持久化。
 * - 对话索引：~/.canopy/conversations.json（轻量元数据）
 * - 消息存储：~/.canopy/conversations/{id}.jsonl（JSONL 格式，逐行追加）
 */

import { readFileSync, writeFileSync, appendFileSync, existsSync, unlinkSync, createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { createInterface } from 'node:readline'
import { writeJsonFileAtomic, readJsonFileSafe } from '@canopy/kernel'
...
import type { ConversationMeta, ChatMessage, RecentMessagesResult, MessageSearchResult, FileAttachment, SessionMessageSearchResponse } from '@canopy/shared'
import {
  createSearchSnippet,
...
/**
 * 更新单条消息的附件列表（用于视觉补齐描述缓存写回）
 *
 * 找不到消息时静默返回（不影响主流程）。
 * 调用方需保证此刻无并发写者（chat-service 在 assistant 消息落盘前调用）。
 *
 * @param id 对话 ID
...
export function updateMessageAttachments(
  ...
  if (index === -1) {
    console.warn(`[对话管理] 更新附件失败，消息不存在 (${id}/${messageId})`)
    return
  }
  messages[index] = { ...messages[index] as ChatMessage, attachments }
