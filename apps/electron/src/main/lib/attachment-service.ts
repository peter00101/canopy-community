 ...
 *
 * 负责文件附件的本地存储、读取和删除。
 * 存储路径：~/.canopy/attachments/{conversationId}/{uuid}.ext
 *
 * - 保存：base64 解码 → 写入文件
...
import { MAX_ATTACHMENT_SIZE } from '@canopy/shared'
...
/** 视频 MIME 白名单（与视频助手一致，Moonshot 官方九种格式） */
 ...
 * 判断是否为视频附件（视频助手支持的格式）
 */
export function isVideoAttachment(mediaType: string): boolean {
  return VIDEO_MIME_TYPES.has(mediaType)
}
 ...
 * ~/.canopy/attachments/{conversationId}/{uuid}.ext
 *
 * @param input 保存附件参数
 ...
 *
 * 支持两种路径格式：
 * 1. 相对路径 {conversationId}/{uuid}.ext → 解析到 ~/.canopy/attachments/
 * 2. 绝对路径（Agent 工作区附件）→ 需在 ~/.canopy/ 目录下，直接读取
 *
 ...
 * 解析附件的绝对路径（不读取内容）
 *
 * 与 readAttachmentAsBase64 使用同一套路径规则与安全校验，
 * 供消息附件点击后交给文件预览面板加载。
 */
export function resolveAttachmentAbsolutePath(localPath: string): string {
  let fullPath: string
 ...
 * 删除对话的全部附件
 *
 * 删除整个 ~/.canopy/attachments/{conversationId}/ 目录。
 *
