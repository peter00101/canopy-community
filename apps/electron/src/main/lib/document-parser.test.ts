...
describe('document-parser / 旧版 Excel、PowerPoint 附件', () => {
  test('Given .xls / .xlt / .ppt / .pps / .pot When 判断 Then 都登记为可解析（此前落到「未知格式按文本读」）', () => {
    for (const ext of ['.xls', '.XLT', '.ppt', '.pps', '.pot']) expect(isSupportedDocumentExtension(ext)).toBe(true)
  })

  test('Given 真实的 BIFF8 .xls When 提取 Then 得到按工作表分段的表格文字，而不是二进制乱码', async () => {
    ...
    const text = await extractTextFromFile(path)
    expect(text).toContain('## 工作表：明细（2 行 × 2 列）')
    expect(text).toContain('北京\t12.5')
    expect(text).not.toContain(String.fromCharCode(0xfffd))
  })

  test('Given 损坏的 .ppt When 提取 Then 抛出可理解的错误（附件链路会写成「提取失败」而不是塞乱码）', async () => {
    const path = join(dir, 'broken.ppt')
    writeFileSync(path, 'not a presentation')
    await expect(extractTextFromFile(path)).rejects.toThrow(/无法解析这个 PowerPoint 97-2003 文件/)
  })
})
