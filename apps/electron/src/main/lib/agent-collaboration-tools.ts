 ...
 * Agent 内置协作会话工具
 *
 * 通过 Pi custom tools 暴露 Canopy Agent 子会话委派能力。
...
export function registerCollaborationEventBus(eventBus: import('./agent-event-bus').AgentEventBus): void {
  ...
  eventBus.on((sessionId: string, payload: AgentStreamPayload) => {
    ...
    if (payload.kind !== 'app_event') return
    ...
    if (event.type === 'ask_user_request') {
      ...
      eventBus.emit(record.parentSessionId, {
        ...
        } as import('@canopy/shared').AppEvent,
      })
    }
...
interface StartDelegationResult {
  ...
  effectivePermissionMode: AppPermissionMode
...
function recoverDelegationRecordFromSession(
  ...
  fallbackPermissionMode: AppPermissionMode | undefined,
...
function getCurrentParentPermissionMode(
  ...
  fallback: AppPermissionMode | undefined,
): AppPermissionMode | undefined {
...
/**
 * 构建协作会话工具定义。
 * 复用同一份内部状态（delegations Map、blocked events 等）；返回值是中性的
 * `KernelToolDefinition`，与具体 Agent Runtime 无关——把它转成 Pi 认识的
 * `sdk.defineTool()` 对象是 `@canopy/runtime-pi` 的 `adaptToolsForPi()` 的事
 * （在 `pi-builtin-tools.ts` 拿到全部内置工具后统一转换）。
 */
export function buildPiCollaborationTools(
  ...
  // 与 pi-builtin-tools 的 jsonToolResult 同口径：内核 JSON 安全化 + 紧凑文本（收上游 #2081）
  function piJsonResult(payload: unknown): KernelToolResult {
  ...
  return [
    ...
    },
    {
      ...
      label: '委派子 Agent',
      description: `创建一个真实可见的 ${CANOPY_BRAND.productName} 协作子 Agent 会话来并行处理独立子任务。只用于长耗时、可并行、需要追踪的任务。可选 thinkingLevel 指定子会话首轮思考强度；模型不支持时运行时会安全归一化。返回中的 configuredThinkingLevel/thinkingLevel 表示配置/请求值，不代表模型 capability normalization 后的实际运行档位。委派只表示子会话已启动，不表示任务完成或结果已回传：若本轮回复、下一步决策或交付依赖该子任务，主会话必须在回复前用返回的 delegationId 调用 wait_for_delegations 收敛结果；只有仍有完全独立的工作时才可先继续推进。`,
      parameters: Type.Object({
        title: Type.Optional(Type.String({ description: '子会话标题' })),
        role: roleType,
    ...
    },
    {
      ...
      label: '批量委派子 Agent',
      description: `批量创建多个真实可见的 ${CANOPY_BRAND.productName} 协作子 Agent 会话。适合把同一大任务拆成多片并行处理。每个 item 可独立指定首轮 thinkingLevel；模型不支持时运行时会安全归一化。返回中的 configuredThinkingLevels/delegations[].thinkingLevel 表示各项配置/请求值，不代表模型 capability normalization 后的实际运行档位。创建成功只表示各子会话已启动，不表示批次已完成：若本轮需要基于任一或全部子任务交付、判断或回复，主会话必须在回复前用返回的 delegationIds 调用 wait_for_delegations（需要完整结论时用 mode=all）；仅可在等待前推进完全独立的主线。`,
      parameters: Type.Object({
    ...
    },
    {
      ...
      label: '等待子会话完成',
      description: `父会话用于收敛子会话结果的等待屏障：等待指定的 ${CANOPY_BRAND.productName} 协作子会话完成，并返回结构化结果摘要。只要本轮回复、决策或交付依赖已委派任务，主会话必须在回复前调用本工具，不能只因 delegate_agent/delegate_agents 已返回就宣称完成；需要全部结果时传入所有 delegationIds 并使用 mode=all，需要部分早期结果时才使用 mode=any。若返回 timeout 或仍有 running 委派，必须如实说明未收敛状态，不能把未完成任务当作已有结果。`,
      parameters: Type.Object({
    ...
    },
    {
      ...
      label: '列出协作子会话',
      description: `列出当前父会话创建的 ${CANOPY_BRAND.productName} 协作子会话及状态。返回中的 thinkingLevel 表示配置/请求值，不代表模型 capability normalization 后的实际运行档位。`,
      parameters: Type.Object({
    ...
    },
    {
      ...
      label: '读取子会话结果',
      description: `按委派 ID 读取一个或多个 ${CANOPY_BRAND.productName} 协作子会话的结果摘要。返回中的 thinkingLevel 表示配置/请求值，不代表模型 capability normalization 后的实际运行档位。`,
      parameters: Type.Object({
    ...
    },
    {
      ...
      label: '停止子会话',
      description: `停止一个正在运行的 ${CANOPY_BRAND.productName} 协作子会话。`,
      parameters: Type.Object({
    ...
    },
    {
      ...
      label: '批量停止子会话',
      description: `批量停止多个正在运行的 ${CANOPY_BRAND.productName} 协作子会话。`,
      parameters: Type.Object({
    ...
    },
    {
      ...
      async execute(_toolCallId: string, params: unknown) {
        ...
        if (blocked.type === 'permission' && blocked.permissionRequestId) {
          const { permissionService } = await import('@canopy/kernel')
