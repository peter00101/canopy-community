...
describe('looksLikeHtmlPage - 识别「网页而非 API 应答」', () => {
  test('Given content-type 是 text/html, When 判定, Then 命中', () => {
    expect(looksLikeHtmlPage('text/html; charset=utf-8', '{}')).toBe(true)
  })

  test('Given 响应体以 doctype 开头（大小写与前导空白都要容忍）, When 判定, Then 命中', () => {
    expect(looksLikeHtmlPage('text/plain', '\n  <!DOCTYPE HTML><html>')).toBe(true)
    expect(looksLikeHtmlPage(null, '<html lang="zh">')).toBe(true)
  })

  test('Given 正常的 JSON 模型列表, When 判定, Then 不命中', () => {
    expect(looksLikeHtmlPage('application/json', '{"data":[{"id":"claude-opus-5"}]}')).toBe(false)
  })

  test('Given 缺 content-type 的 JSON, When 判定, Then 不命中（只按内容判，不误伤）', () => {
    expect(looksLikeHtmlPage(undefined, '{"models":[]}')).toBe(false)
  })
})

describe('normalizeModelsProbeResponse - 中转站 catch-all 首页不再算测试通过（0.18.67）', () => {
  test('Given 网关对任意路径返回 200 + 首页 HTML, When 探测模型列表, Then 判失败并提示 Base URL 填错', async () => {
    const response = new Response('<!doctype html><html><body>首页</body></html>', {
      status: 200,
      headers: { 'content-type': 'text/html' },
    })
    const result = await normalizeModelsProbeResponse(response)
    expect(result.success).toBe(false)
    expect(result.errorType).toBe('not_found')
    expect(result.message).toContain('Base URL')
  })

  test('Given 真实模型列表 JSON, When 探测, Then 成功（既有行为不变）', async () => {
  ...
  test('Given 401, When 探测, Then 走既有状态码归类（未认证不是「地址错」）', async () => {
    const result = await normalizeModelsProbeResponse(new Response('unauthorized', { status: 401 }))
    expect(result.success).toBe(false)
    expect(result.errorType).toBe('auth')
  })

  test('Given 4xx 且返回 HTML 错误页, When 探测, Then 仍按状态码归类而不是报「地址错」', async () => {
    const response = new Response('<html>404</html>', {
      status: 404,
      headers: { 'content-type': 'text/html' },
    })
    const result = await normalizeModelsProbeResponse(response)
