 ...
 * 配置路径工具
 *
 * 管理 Canopy 应用的本地配置文件路径。
 * 所有用户配置存储在 ~/.canopy/ 目录下。
 */
...
import { rmSyncWithRetry, readJsonFileSafe, writeJsonFileAtomic } from '@canopy/kernel'

/**
 * 获取配置目录名称
 *
 * 开发模式下返回 '.canopy-dev'，正式版本返回 '.canopy'。
 *
 * 检测优先级：
 * 1. CANOPY_DEV=1 环境变量（显式覆盖）
...
export function getConfigDirName(): string {
  if (_configDirName === undefined) {
    if (process.env.CANOPY_DEV === '1') {
      _configDirName = CANOPY_BRAND.configDirNameDev
    } else {
      try {
        const { app } = require('electron')
        _configDirName = app.isPackaged ? CANOPY_BRAND.configDirName : CANOPY_BRAND.configDirNameDev
      } catch {
        _configDirName = CANOPY_BRAND.configDirName
      }
    }
    const mode = _configDirName === CANOPY_BRAND.configDirNameDev ? '开发模式' : '正式版本'
 ...
 * 获取配置目录路径
 *
 * 开发模式返回 ~/.canopy-dev/，正式版本返回 ~/.canopy/。
 * 如果目录不存在则自动创建。
 */
 ...
 * 获取对话索引文件路径
 *
 * @returns ~/.canopy/conversations.json
 */
 ...
 *
 * @param id 对话 ID
 * @returns ~/.canopy/conversations/{id}.jsonl
 */
 ...
 * @returns ~/.canopy/attachments/{conversationId}/
 */
 ...
 * @returns 完整路径 ~/.canopy/attachments/{conversationId}/{uuid}.ext
 */
...
/**
 * 解析 Canopy 管理的默认 Markdown Vault 目录。
 */
...
/**
 * 获取 Canopy 管理的默认 Markdown Vault 目录，并在首次使用时创建。
 *
 * @returns 正式版本 ~/.canopy/vault/，开发模式 ~/.canopy-dev/vault/
 */
 ...
 * 获取系统默认 App 探测缓存路径
 *
 * @returns ~/.canopy/default-apps.json
 */
 ...
 * 获取用户档案文件路径
 *
 * @returns ~/.canopy/user-profile.json
 */
 ...
 * 获取代理配置文件路径
 *
 * @returns ~/.canopy/proxy-settings.json
 */
 ...
 * 获取系统提示词配置文件路径
 *
 * @returns ~/.canopy/system-prompts.json
 */
 ...
 * 获取 Chat 工具配置文件路径
 *
 * @returns ~/.canopy/chat-tools.json
 */
 ...
 * 获取 Agent 会话索引文件路径
 *
 * @returns ~/.canopy/agent-sessions.json
 */
 ...
 *
 * @param id 会话 ID
 * @returns ~/.canopy/agent-sessions/{id}.jsonl
 */
...
/**
 * 指定 Agent 会话的 Skill 激活元信息 sidecar 路径（0.17.24 起）
 *
 * 之前 Skill 激活信息内联在 JSONL 的 user 消息里，每次激活都要把整个会话正文读出来重写一遍
 * （审查报告 N-1）。现在写这个小文件，读取时按 user 消息 uuid 合并回去；旧会话内联的仍然认。
 *
 * @returns ~/.canopy/agent-sessions/{id}.skill-activations.json
 */
export function getAgentSessionSkillActivationsPath(id: string): string {
  return join(getAgentSessionsDir(), `${id}.skill-activations.json`)
}

/**
 * 获取 Agent 工作区索引文件路径
 *
 * @returns ~/.canopy/agent-workspaces.json
 */
 ...
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.canopy/agent-workspaces/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.canopy/agent-workspaces/{slug}/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.canopy/agent-workspaces/{slug}/mcp.json
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.canopy/agent-workspaces/{slug}/skills/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.canopy/agent-workspaces/{slug}/workspace-files/
 */
 ...
 * @returns ~/.canopy/agent-workspaces/{slug}/{sessionId}/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.canopy/agent-workspaces/{slug}/skills-inactive/
 */
...
export function getBundledCliPath(): string | undefined {
  ...
  const binName = process.platform === 'win32' ? 'canopy.exe' : 'canopy'
...
/** 清理 ~/.canopy/default-skills/ 中已退役的内置 Skill 缓存。 */
...
/**
 * 从 app bundle 同步默认 Skills 到 ~/.canopy/default-skills/
 *
 ...
 * 获取微信长轮询同步游标路径
 *
 * @returns ~/.canopy/wechat-sync.json
 */
 ...
 * 获取微信聊天绑定持久化路径
 *
 * @returns ~/.canopy/wechat-bindings.json
 */
 ...
 *
 * @returns ~/.canopy/dingtalk-bindings-{botId}.json
 */
 ...
 * 获取飞书聊天绑定持久化路径
 *
 * @returns ~/.canopy/feishu-bindings.json
 */
 ...
 *
 * @returns ~/.canopy/feishu-bindings-{botId}.json
 */
 ...
 *
 * @returns ~/.canopy/feishu-metadata-{botId}.json
 */
 ...
 * 而不是用户的 ~/.claude.json，实现 Canopy 与 Claude Code CLI 的配置隔离。
 *
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.canopy/sdk-config/
 */
 ...
 * 获取 Scratch Pad 文件路径。
 *
 * 保留原始旧版 scratch-pad.md，并按其内容指纹仅复制到 Canopy 管理的默认 Vault 一次。
 * 已存在的 Vault 内容绝不覆盖；旧文件优先复制为草稿.md，重名时使用草稿 N.md。
 * 内容复制成功后以崩溃安全的状态文件记录指纹；重启或重复调用只返回 canonical Vault 路径。
 * 迁移失败时继续使用旧路径，以便下次安全重试。
 *
 * @returns 正式版本 ~/.canopy/vault/scratch-pad.md，开发模式 ~/.canopy-dev/vault/scratch-pad.md
 */
 ...
 *
 * @returns ~/.canopy/automations.json
 */
