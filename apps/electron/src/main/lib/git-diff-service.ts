...
import type { ChangedFileEntry, UnstagedChangesResult, UntrackedFileEntry } from '@canopy/shared'
import { normalizePathForCompare } from '@canopy/shared'
import type { ChangeSource, ChangedFileStatus } from '@canopy/shared'
 ...
 *
 * 用于安全校验：worktree 常被放在主仓库之外（如 ~/canopy-dev/worktrees/xxx），
 ...
 */
export async function listWorktrees(repoPath: string): Promise<import('@canopy/shared').WorktreeInfo[]> {
  const roots = await findAllGitRoots(repoPath)
  const worktreesByPath = new Map<string, import('@canopy/shared').WorktreeInfo>()
...
export async function getWorktreeChanges(
...
): Promise<import('@canopy/shared').UnstagedChangesResult> {
  ...
  const allFiles: import('@canopy/shared').ChangedFileEntry[] = []
  const fileMap = new Map<string, import('@canopy/shared').ChangedFileEntry>()
  ...
  if (committedStatus) {
    for (const line of committedStatus.split('\n').filter(Boolean)) {
      ...
      let status: import('@canopy/shared').ChangedFileStatus
      let filePath: string
      ...
      const entry: import('@canopy/shared').ChangedFileEntry = {
        filePath,
        status,
