 ...
 * - XLS/PPT：旧版 Excel/PowerPoint，后台线程内置解析（legacy-office-parser）
...
/**
 * 旧版 Excel / PowerPoint 97-2003 二进制格式：交给后台线程的内置解析器（legacy-office-parser）。
 * 此前没有登记，落到「未知格式按文本读」，把几十万字符的二进制乱码塞给模型。
 */
const LEGACY_SPREADSHEET_PRESENTATION_EXTENSIONS = new Set([
  '.xls', '.xlt',
  '.ppt', '.pps', '.pot',
])

/** 旧版表格附件每个工作表最多列出的行数，与整段文字的字符上限（防止一个附件撑爆上下文） */
const LEGACY_ATTACHMENT_MAX_ROWS_PER_SHEET = 2000
const LEGACY_ATTACHMENT_MAX_CHARS = 200_000
/** 与 Office 工具、预览同一档的文件体积上限 */
const LEGACY_ATTACHMENT_MAX_BYTES = 50 * 1024 * 1024
...
const SUPPORTED_DOCUMENT_EXTENSIONS = new Set([
  ...
  ...LEGACY_SPREADSHEET_PRESENTATION_EXTENSIONS,
...
/**
 * 根据 MIME 类型判断是否为可解析文档（非图片、非视频附件）
 *
 * 排除图片与视频类型（各走视觉/视频助手链路），其余尝试按扩展名判断。
 */
...
export async function extractTextFromFile(filePath: string): Promise<string> {
  ...
  // 旧版 Excel / PowerPoint 文件
  if (LEGACY_SPREADSHEET_PRESENTATION_EXTENSIONS.has(ext)) {
    return extractLegacySpreadsheetOrPresentation(filePath)
  }
...
/**
 * 提取旧版 Excel / PowerPoint 文本：表格按工作表输出制表符分隔的行，演示文稿按页输出文字。
 * 解析在后台线程里做（8MB 的 .xls 要 1 秒左右），不卡主进程。
 */
async function extractLegacySpreadsheetOrPresentation(filePath: string): Promise<string> {
  const { size } = statSync(filePath)
  if (size > LEGACY_ATTACHMENT_MAX_BYTES) {
    throw new Error(`文件超过 ${LEGACY_ATTACHMENT_MAX_BYTES / 1024 / 1024}MB，无法提取内容`)
  }
  const text = await runLegacyOfficeTaskOffThread({
    type: 'plain-text',
    filePath,
    maxRowsPerSheet: LEGACY_ATTACHMENT_MAX_ROWS_PER_SHEET,
    maxChars: LEGACY_ATTACHMENT_MAX_CHARS,
  })
  console.log(`[文档解析] 旧版 Excel/PowerPoint 提取完成: ${text.length} 字符`)
  return text
}
