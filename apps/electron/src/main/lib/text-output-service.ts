 ...
 * 文本输出服务
 *
 * 语音输入完成后优先写入 Canopy 输入框，否则尝试写入当前光标位置。
 */
...
/** 在显示语音浮窗前记录目标是否为 Canopy 主窗口。 */
export function captureVoiceDictationTarget(forceAppInput?: boolean): boolean {
  const mainWindow = getMainWindow()
  targetWasAppInput = forceAppInput ?? BrowserWindow.getFocusedWindow() === mainWindow
  return targetWasAppInput
}
...
function shouldWriteToAppInput(settings: VoiceDictationSettings): boolean {
  return settings.outputMode === 'app-input' ||
    (settings.outputMode === 'auto' && targetWasAppInput)
}
...
function resolveVoiceDictationOutputContext(
  ...
  return {
    routeToAppInput: shouldWriteToAppInput(settings),
...
export function previewVoiceDictationText(
  ...
  if (!text || !outputContext?.routeToAppInput) return
...
/** 取消录音时撤销尚未提交到 Canopy 输入框的临时组合文本。 */
...
export async function commitVoiceDictationText(
  ...
  try {
    ...
    const shouldSendToAppInput = hasActivePreview || outputContext.routeToAppInput
    if (shouldSendToAppInput) {
      ...
      if (delivered) {
        return { mode: 'app-input', success: true, message: '已写入输入框' }
      }

      clipboard.writeText(trimmed)
      return { mode: 'clipboard', success: true, message: `${CANOPY_BRAND.productName} 输入框未确认接收，已复制到剪贴板` }
    }
