...
function handleAgentEvent(sessionId: string, payload: AgentStreamPayload): void {
  if (payload.kind === 'app_event') {
    handleAppEvent(sessionId, payload.event)
...
function handleAppEvent(sessionId: string, event: import('@canopy/shared').AppEvent): void {
  switch (event.type) {
...
function handleSdkMessage(sessionId: string, message: import('@canopy/shared').SDKMessage): void {
  switch (message.type) {
    case 'assistant': {
      const aMsg = message as import('@canopy/shared').SDKAssistantMessage
    ...
    case 'user': {
      const uMsg = message as import('@canopy/shared').SDKUserMessage
      const session = sessions.get(sessionId)
      if (!session) break
    ...
    case 'result': {
      const rMsg = message as import('@canopy/shared').SDKResultMessage
      const session = ensureSession(sessionId)
      const isChildSession = isDelegatedChildSession(sessionId)
    ...
    case 'system': {
      const sMsg = message as import('@canopy/shared').SDKSystemMessage
      const session = ensureSession(sessionId)
      switch (sMsg.subtype) {
        case 'task_started': {
          session.phase = 'running'
...
function requiresImmediateAgentIslandPush(payload: AgentStreamPayload): boolean {
  ...
  return message.type === 'result' || (message.type === 'assistant' && Boolean((message as import('@canopy/shared').SDKAssistantMessage).error))
}
