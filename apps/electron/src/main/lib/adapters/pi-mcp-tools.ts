 ...
 * mcpServers 参数，因此 Canopy 在主进程连接用户配置的 MCP server，并把 MCP tools
 * 映射成 Pi customTools。
 */
...
import { sanitizeToolResultImageContent } from '@canopy/runtime-pi'
...
class PiMcpClientManager {
  ...
  private async createConnection(
    ...
    const client = new Client({ name: 'canopy-pi-agent-mcp-bridge', version: '0.1.0' }, { capabilities: {} })
    try {
...
/**
 * 将 Canopy 已构建的 MCP server 配置转换为 Pi customTools。
 *
