...
import { AGENT_IPC_CHANNELS, MAX_ATTACHMENT_SIZE } from '@canopy/shared'
...
import { createPiRuntimePlugin } from '@canopy/runtime-pi/utility-runtime'
import { AgentEventBus } from './agent-event-bus'
import { AgentOrchestrator } from './agent-orchestrator'
import { memoryRefreshLifecycleHook } from './agent-memory-refresh-service'
...
import { getAgentSessionMeta, updateAgentSessionMeta, appendSDKMessages, markAgentSessionDeleting } from './agent-session-manager'
...
const adapter = createPiRuntimePlugin().createAdapter()
const orchestrator = new AgentOrchestrator(adapter, eventBus, [memoryRefreshLifecycleHook])
...
// ExitPlanMode 审批在横幅之外被 resolve（abort / 会话清理 / IPC 响应）时广播给所有窗口，
// 渲染层据此兜底清横幅；否则残留横幅里发的反馈会被静默吞掉（permission_resolved 同族）。
exitPlanService.setResolvedNotifier((sessionId, requestId) => {
  eventBus.emit(sessionId, {
    kind: 'app_event',
    event: { type: 'exit_plan_mode_resolved', requestId },
  })
})
...
export interface AgentRunExtensions {
  /** 中性工具定义（如飞书群聊工具）；由 orchestrator 在构建 Pi query 时经 adaptToolsForPi 转换。 */
  customTools?: KernelToolDefinition[]
}
 ...
 * 中止指定会话的 Agent 执行
 *
 * @returns 是否真的有活跃 run 被中止（false 时渲染层须自行结束本地运行态）
 */
export function stopAgent(sessionId: string): boolean {
  // SEND_MESSAGE 预留会话槽位后、异步桥接尚未到达 orchestrator 的窗口内收到的
  // stop 必须被记住，否则随后的 run 会创建出无法取消的 adapter query。
...
/**
 * DELETE_SESSION 专用：先写入删除墓碑，再等待已发出的 utility abort 收束，
 * 之后调用方才能安全移除会话文件（收上游 #2074）。
 */
...
): Promise<import('@canopy/shared').RewindSessionResult> {
...
/** 当前所有仍在执行的会话 ID（渲染层看门狗对账用） */
export function listActiveAgentSessionIds(): string[] {
  return orchestrator.listActiveSessionIds()
}
 ...
 * 运行中动态切换会话的权限模式
 *
 * 同时更新 Canopy 侧（canUseTool 动态读取）和 SDK 侧（query.setPermissionMode）。
 */
export async function updateAgentPermissionMode(sessionId: string, mode: AppPermissionMode): Promise<void> {
...
export async function queueAgentMessage(
  rawInput: AgentQueueMessageInput,
  _webContents: WebContents,
): Promise<string> {
  // 进程边界：畸形入参（如缺 userMessage）在这里给出明确错误，而不是在编排层深处炸 TypeError
  const input = validateAgentQueueMessageInput(rawInput)
...
export async function submitOrEnqueueAgentMessage(
  ...
  // ExitPlanMode 审批挂起时，主输入框的消息作为对该审批的反馈送给模型：
  // 不能打断（打断会让模型重新提审批、用户越催越循环），也不能排队（审批挂着 turn
  // 永远不结束，排队消息永远等不到消费）。忽略 interrupt 标志，反馈会让 run 自然继续。
  const rawText = input.rawUserMessage ?? input.userMessage
  const pendingExitPlan = exitPlanService.getPendingRequests().find((r) => r.sessionId === input.sessionId)
  if (pendingExitPlan && shouldRouteAsExitPlanFeedback({
    dispatch: input.dispatch,
    sessionActive: orchestrator.isActive(input.sessionId),
    hasPendingExitPlan: true,
    rawText,
  })) {
    // 先落盘用户消息再 respond（Pi 还挂在审批上，零落盘竞争）；uuid 与渲染层
    // 乐观上屏的 queueMessageId 一致，消息合并层按 uuid 去重、刷新不双显。
    try {
      appendSDKMessages(input.sessionId, [{
        type: 'user',
        uuid: input.queueMessageId || randomUUID(),
        message: { content: [{ type: 'text', text: rawText }] },
        parent_tool_use_id: null,
        _createdAt: Date.now(),
      } as unknown as SDKMessage])
    } catch (error) {
      console.warn(`[Agent 服务] 计划审批反馈落盘失败: sessionId=${input.sessionId}`, error)
    }
    exitPlanService.respondToExitPlanMode({
      requestId: pendingExitPlan.requestId,
      action: 'feedback',
      feedback: input.userMessage,
    })
    console.log(`[Agent 服务] 消息已作为计划审批反馈送入: sessionId=${input.sessionId}, requestId=${pendingExitPlan.requestId}`)
 ...
 * 保存文件到项目文件根目录
 *
 * 空白项目写入 Canopy 托管的 workspace-files/；本地目录项目直接写入用户选择的原始目录。
 */
