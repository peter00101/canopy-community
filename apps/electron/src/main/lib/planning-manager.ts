...
import { PLANNING_CONFLICT_ERROR } from '@canopy/shared'
...
type SyncOutboxRow = { id: string; profile_id: string; target_id: string | null; operation: 'upsert' | 'delete'; app_entity_id: string; native_start_at: number | null; attempts: number; next_attempt_at: number; last_error: string | null; revision: number; created_at: number; updated_at: number }
type SyncBindingRow = { profile_id: string; target_id: string | null; app_entity_id: string; calendar_item_identifier: string | null; calendar_item_external_identifier: string | null; last_synced_hash: string | null; last_synced_at: number | null }
type SyncProfileConflictRow = { id: string; profile_id: string; app_entity_id: string; kind: 'changed' | 'deleted'; native_item_json: string | null; detected_at: number }
type SyncCleanupRow = { id: string; entity: 'calendar' | 'reminder'; target_id: string; app_entity_id: string; calendar_item_identifier: string | null; native_start_at: number | null; attempts: number; next_attempt_at: number; last_error: string | null; created_at: number; updated_at: number }
...
type NativeBindingRow = { connection_id: string; app_entity_id: string; calendar_item_identifier: string; due_date_only: number; recreate_pending: number; last_native_hash: string | null; last_synced_at: number | null }
type NativeOutboxRow = { id: string; connection_id: string; operation: 'upsert' | 'hide'; app_entity_id: string; attempts: number; next_attempt_at: number; revision: number }
type NativeConflictRow = { id: string; connection_id: string; entity: 'calendar' | 'reminder'; app_entity_id: string; kind: 'changed' | 'deleted'; native_item_json: string | null; detected_at: number; resolved_at: number | null }
...
export interface PlanningNativeExternalItem {
  ...
  /** 仅由 addon 从 Canopy 自己写入的 URL marker 解析，绝不透出任意用户 URL。 */
  appIdentity?: string
  title: string
  notes?: string
  startAt?: number
...
function migrateDatabase(db: SqliteDatabase): void {
  ...
  if (version > PLANNING_SCHEMA_VERSION) throw new Error(`任务/日程数据版本高于当前 ${CANOPY_BRAND.productName}，无法安全打开`)
    ...
    if (version < 2) {
      db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS planning_sync_bindings (
          ...
          PRIMARY KEY(profile_id, app_entity_id)
        );
        CREATE TABLE IF NOT EXISTS planning_sync_outbox (
          ...
          created_at INTEGER NOT NULL,
          updated_at INTEGER NOT NULL,
          UNIQUE(profile_id, app_entity_id)
        );
    ...
    if (version < 4) {
      ...
      db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS planning_sync_cleanup (
          ...
          created_at INTEGER NOT NULL,
          updated_at INTEGER NOT NULL,
          UNIQUE(entity, target_id, app_entity_id)
        );
    ...
    if (version < 5) {
      db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS planning_native_bindings (
          ...
          last_synced_at INTEGER,
          PRIMARY KEY(connection_id, app_entity_id),
        ...
        CREATE TABLE IF NOT EXISTS planning_native_outbox (
          ...
          created_at INTEGER NOT NULL,
          updated_at INTEGER NOT NULL,
          UNIQUE(connection_id, app_entity_id)
        );
...
export function connectPlanningNativeConnection(input: ConnectPlanningNativeConnectionInput): PlanningNativeConnection {
  ...
  if (getDatabase().prepare('SELECT id FROM planning_sync_profiles WHERE entity=:entity AND target_id=:targetId').get({ entity: input.entity, targetId: target.id })) throw new Error(`${CANOPY_BRAND.productName} 受管目标不能同时作为外部连接`)
...
/** 断开只移除 Canopy 投影与映射，绝不删除用户的系统原始事项。 */
function nativeConflictFromRow(row: NativeConflictRow): PlanningNativeSyncConflict {
  ...
  const local = getDatabase().prepare(`SELECT title FROM ${table} WHERE id=:id`).get({ id: row.app_entity_id }) as { title?: string } | undefined
  return { id: row.id, connectionId: row.connection_id, entity: row.entity, appEntityId: row.app_entity_id, title: local?.title ?? connection?.target_title ?? '系统事项', kind: row.kind, detectedAt: row.detected_at }
}
...
function managedProfileConflictFromRow(row: SyncProfileConflictRow): PlanningNativeSyncConflict {
  ...
  const local = getDatabase().prepare('SELECT title FROM calendar_events WHERE id=:id').get({ id: row.app_entity_id }) as { title?: string } | undefined
  return { id: row.id, profileId: row.profile_id, entity: 'calendar', appEntityId: row.app_entity_id, title: local?.title ?? profile?.target_title ?? `${CANOPY_BRAND.productName} 日程`, kind: row.kind, detectedAt: row.detected_at }
}
...
/** 冲突必须显式选择；保留系统会回流，保留 Canopy 才会继续/重建其出站版本。 */
export function resolvePlanningNativeSyncConflict(input: ResolvePlanningNativeSyncConflictInput): boolean {
  ...
  withPlanningTransaction(() => {
    if (input.resolution === 'keep_app') {
      if (conflict.kind === 'changed' && conflict.native_item_json) {
        // 记录用户已看过的系统版本，强制 reconcile 不会在 outbox 写回前马上重新创建同一冲突。
        getDatabase().prepare('UPDATE planning_native_bindings SET last_native_hash=:hash,last_synced_at=:now WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ hash: conflict.native_item_json, now: Date.now(), connectionId: conflict.connection_id, appEntityId: conflict.app_entity_id })
      } else if (conflict.kind === 'deleted') {
        // locator 已失效但仍须保留 binding 供 outbox 取目标；写回成功后会原子更新为新 locator。
        getDatabase().prepare('UPDATE planning_native_bindings SET recreate_pending=1 WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ connectionId: conflict.connection_id, appEntityId: conflict.app_entity_id })
      }
    }
    if (input.resolution === 'keep_system') {
      getDatabase().prepare('DELETE FROM planning_native_outbox WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ connectionId: conflict.connection_id, appEntityId: conflict.app_entity_id })
      if (conflict.kind === 'deleted') {
        const table = connection.entity === 'reminder' ? 'todos' : 'calendar_events'
        getDatabase().prepare(`DELETE FROM ${table} WHERE id=:id AND native_connection_id=:connectionId`).run({ id: conflict.app_entity_id, connectionId: conflict.connection_id })
        getDatabase().prepare('DELETE FROM planning_native_bindings WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ connectionId: conflict.connection_id, appEntityId: conflict.app_entity_id })
      }
    }
  ...
  withPlanningTransaction(() => {
    if (input.resolution === 'keep_app') {
      if (conflict.kind === 'changed' && conflict.native_item_json) {
        ...
        const currentBinding = getDatabase().prepare('SELECT calendar_item_identifier FROM planning_sync_bindings WHERE profile_id=:profileId AND app_entity_id=:appEntityId').get({ profileId: profile.id, appEntityId: conflict.app_entity_id }) as { calendar_item_identifier?: string | null } | undefined
        if (currentBinding?.calendar_item_identifier === native.calendarItemIdentifier) {
          getDatabase().prepare('UPDATE planning_sync_bindings SET last_synced_hash=:hash,last_synced_at=:now WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ hash: planningNativeCalendarHash(native), now: Date.now(), profileId: profile.id, appEntityId: conflict.app_entity_id })
        } else {
          // 这是 locator recovery 冲突：不能将新系统 locator 自动认领；用户选 Canopy 后按新项目发布。
          getDatabase().prepare('UPDATE planning_sync_bindings SET calendar_item_identifier=NULL,last_synced_hash=NULL WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId: profile.id, appEntityId: conflict.app_entity_id })
        }
        enqueuePlanningSync('calendar_event', conflict.app_entity_id, 'upsert')
      } else {
        // locator 已失效；用户已选择保留 Canopy，重置 locator 并确保下一轮有明确的出站操作。
        getDatabase().prepare('UPDATE planning_sync_bindings SET calendar_item_identifier=NULL,last_synced_hash=NULL WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId: profile.id, appEntityId: conflict.app_entity_id })
        enqueuePlanningSync('calendar_event', conflict.app_entity_id, 'upsert')
      }
    } else {
      getDatabase().prepare('DELETE FROM planning_sync_outbox WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId: profile.id, appEntityId: conflict.app_entity_id })
      if (conflict.kind === 'changed' && conflict.native_item_json) {
        const native = JSON.parse(conflict.native_item_json) as PlanningNativeExternalItem
        // 用户明确保留系统版本后才接受新的 locator；随后回流会更新日程内容和 stable hash。
        getDatabase().prepare('UPDATE planning_sync_bindings SET calendar_item_identifier=:calendarItemIdentifier,calendar_item_external_identifier=:calendarItemExternalIdentifier WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId: profile.id, appEntityId: conflict.app_entity_id, calendarItemIdentifier: native.calendarItemIdentifier, calendarItemExternalIdentifier: native.calendarItemExternalIdentifier ?? null })
      }
      if (conflict.kind === 'deleted') {
        getDatabase().prepare("DELETE FROM planning_reminders WHERE target_type='calendar_event' AND target_id=:id").run({ id: conflict.app_entity_id })
        getDatabase().prepare('DELETE FROM calendar_events WHERE id=:id').run({ id: conflict.app_entity_id })
        getDatabase().prepare('DELETE FROM planning_sync_bindings WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId: profile.id, appEntityId: conflict.app_entity_id })
      }
    }
...
function enqueuePlanningSync(targetType: PlanningReminderTargetType, appEntityId: string, operation: 'upsert' | 'delete', now = Date.now(), nativeStartAt?: number): void {
  const entity = syncEntityForPlanningTarget(targetType)
  const external = getDatabase().prepare('SELECT bindings.connection_id, connections.can_write FROM planning_native_bindings AS bindings JOIN planning_native_connections AS connections ON connections.id=bindings.connection_id WHERE bindings.app_entity_id=:appEntityId AND connections.entity=:entity').get({ appEntityId, entity }) as { connection_id: string; can_write: number } | undefined
  if (external) {
    // 用户明确连接的可写 Calendar / Reminder 在 Canopy 删除时都写穿 EventKit；只读集合一律不能本地假成功。
    if (external.can_write !== 1) throw new Error(`该系统集合为只读，不能在 ${CANOPY_BRAND.productName} 中修改或删除`)
    // native outbox 用 hide 表示“本地投影已删除”；coordinator 根据实体执行受限 EventKit remove。
    const externalOperation = operation === 'delete' ? 'hide' : 'upsert'
    getDatabase().prepare(`INSERT INTO planning_native_outbox (id,connection_id,operation,app_entity_id,attempts,next_attempt_at,created_at,updated_at) VALUES (:id,:connectionId,:operation,:appEntityId,0,:now,:now,:now) ON CONFLICT(connection_id,app_entity_id) DO UPDATE SET operation=excluded.operation,attempts=0,next_attempt_at=excluded.next_attempt_at,last_error=NULL,revision=planning_native_outbox.revision+1,updated_at=excluded.updated_at`).run({ id: randomUUID(), connectionId: external.connection_id, operation: externalOperation, appEntityId, now })
    return
  }
  ...
  for (const profile of profiles) {
    getDatabase().prepare(`
      INSERT INTO planning_sync_outbox (id, profile_id, target_id, operation, app_entity_id, native_start_at, attempts, next_attempt_at, created_at, updated_at)
      VALUES (:id, :profileId, :targetId, :operation, :appEntityId, :nativeStartAt, 0, :now, :now, :now)
      ON CONFLICT(profile_id, app_entity_id) DO UPDATE SET
    ...
    `).run({ id: randomUUID(), profileId: profile.id, targetId: profile.target_id, operation, appEntityId, nativeStartAt: nativeStartAt ?? null, now })
  }
}

/** 清理任务独立于 outbox，保证切换受管目标时可同时“删旧、写新”。 */
function enqueuePlanningSyncCleanup(input: Pick<PlanningSyncCleanupItem, 'entity' | 'targetId' | 'appEntityId' | 'calendarItemIdentifier' | 'nativeStartAt'>, now = Date.now()): void {
  getDatabase().prepare(`
    INSERT INTO planning_sync_cleanup (id, entity, target_id, app_entity_id, calendar_item_identifier, native_start_at, attempts, next_attempt_at, created_at, updated_at)
    VALUES (:id, :entity, :targetId, :appEntityId, :calendarItemIdentifier, :nativeStartAt, 0, :now, :now, :now)
    ON CONFLICT(entity, target_id, app_entity_id) DO UPDATE SET
  ...
  withPlanningTransaction(() => {
    ...
    if (targetChanged && existing) {
      ...
      for (const binding of bindings) enqueuePlanningSyncCleanup({
        ...
        appEntityId: binding.app_entity_id,
      ...
      for (const pending of pendingUpserts) enqueuePlanningSyncCleanup({
        ...
        appEntityId: pending.app_entity_id,
...
export function listDuePlanningSyncOutbox(now = Date.now(), limit = 25): PlanningSyncOutboxItem[] {
  const rows = getDatabase().prepare(`
    ...
    LEFT JOIN planning_sync_bindings AS bindings ON bindings.profile_id=outbox.profile_id AND bindings.app_entity_id=outbox.app_entity_id AND bindings.target_id=COALESCE(outbox.target_id, profiles.target_id)
    LEFT JOIN planning_sync_profile_conflicts AS conflicts ON conflicts.profile_id=outbox.profile_id AND conflicts.app_entity_id=outbox.app_entity_id
...
export function listDuePlanningSyncCleanup(now = Date.now(), limit = 25): PlanningSyncCleanupItem[] {
  ...
  return rows.map((row) => ({ id: row.id, entity: row.entity, targetId: row.target_id, appEntityId: row.app_entity_id, calendarItemIdentifier: row.calendar_item_identifier ?? undefined, nativeStartAt: row.native_start_at ?? undefined, attempts: row.attempts }))
}
...
export function completePlanningSyncOutbox(item: PlanningSyncOutboxItem, nativeIdentifiers?: { calendarItemIdentifier?: string; calendarItemExternalIdentifier?: string }, nativeHash?: string): void {
  withPlanningTransaction(() => {
    ...
    if (item.operation === 'delete' && isCurrentTarget) {
      getDatabase().prepare('DELETE FROM planning_sync_bindings WHERE profile_id=:profileId AND target_id=:targetId AND app_entity_id=:appEntityId').run({ profileId: item.profile.id, targetId: item.profile.targetId, appEntityId: item.appEntityId })
    } else if (nativeIdentifiers?.calendarItemIdentifier && isCurrentTarget) {
      getDatabase().prepare(`
        INSERT INTO planning_sync_bindings (profile_id, target_id, app_entity_id, calendar_item_identifier, calendar_item_external_identifier, last_synced_hash, last_synced_at)
        VALUES (:profileId, :targetId, :appEntityId, :calendarItemIdentifier, :calendarItemExternalIdentifier, :nativeHash, :now)
        ON CONFLICT(profile_id, app_entity_id) DO UPDATE SET
      ...
      `).run({ profileId: item.profile.id, targetId: item.profile.targetId, appEntityId: item.appEntityId, calendarItemIdentifier: nativeIdentifiers.calendarItemIdentifier, calendarItemExternalIdentifier: nativeIdentifiers.calendarItemExternalIdentifier ?? null, nativeHash: nativeHash ?? null, now })
    } else if (nativeIdentifiers?.calendarItemIdentifier) {
      // 目标切换与 native 写入并发时，将刚创建的旧项移入独立清理队列，不能任其孤儿化。
      enqueuePlanningSyncCleanup({ entity: item.profile.entity, targetId: item.profile.targetId, appEntityId: item.appEntityId, calendarItemIdentifier: nativeIdentifiers.calendarItemIdentifier, nativeStartAt: item.nativeStartAt }, now)
    }
...
export function listDuePlanningNativeOutbox(now = Date.now(), limit = 25): PlanningNativeOutboxItem[] {
  // 冲突未决时绝不能自动把 Canopy 一侧覆盖到系统。
  const rows = getDatabase().prepare(`SELECT outbox.*, connections.entity,connections.target_id,connections.target_title,connections.source_title,connections.source_type,connections.can_write,connections.connected_at,connections.updated_at AS connection_updated_at,bindings.calendar_item_identifier,bindings.due_date_only,bindings.recreate_pending FROM planning_native_outbox AS outbox JOIN planning_native_connections AS connections ON connections.id=outbox.connection_id JOIN planning_native_bindings AS bindings ON bindings.connection_id=outbox.connection_id AND bindings.app_entity_id=outbox.app_entity_id LEFT JOIN planning_native_sync_conflicts AS conflicts ON conflicts.connection_id=outbox.connection_id AND conflicts.app_entity_id=outbox.app_entity_id AND conflicts.resolved_at IS NULL WHERE outbox.next_attempt_at<=:now AND conflicts.id IS NULL ORDER BY outbox.created_at LIMIT :limit`).all({ now, limit }) as Array<NativeOutboxRow & NativeConnectionRow & { connection_updated_at: number; calendar_item_identifier: string; due_date_only: number; recreate_pending: number }>
  return rows.map((row) => ({ id: row.id, connection: nativeConnectionFromRow({ ...row, updated_at: row.connection_updated_at }), operation: row.operation, appEntityId: row.app_entity_id, calendarItemIdentifier: row.calendar_item_identifier, dueDateOnly: row.due_date_only === 1, recreatePending: row.recreate_pending === 1, attempts: row.attempts, revision: row.revision }))
}
...
export function completePlanningNativeOutbox(item: PlanningNativeOutboxItem, nativeIdentifiers?: { calendarItemIdentifier?: string }): void {
  withPlanningTransaction(() => {
    if (item.operation === 'hide') {
      ...
      getDatabase().prepare(`DELETE FROM ${table} WHERE id=:id AND native_connection_id=:connectionId`).run({ id: item.appEntityId, connectionId: item.connection.id })
      getDatabase().prepare('DELETE FROM planning_native_bindings WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ connectionId: item.connection.id, appEntityId: item.appEntityId })
    } else if (nativeIdentifiers?.calendarItemIdentifier) {
      // EventKit locator 会因系统删除后重建等情况变化；写回成功时必须更新 binding，避免下一轮导入出重复投影。
      getDatabase().prepare('UPDATE planning_native_bindings SET calendar_item_identifier=:calendarItemIdentifier,recreate_pending=0,last_native_hash=NULL,last_synced_at=:now WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ calendarItemIdentifier: nativeIdentifiers.calendarItemIdentifier, now: Date.now(), connectionId: item.connection.id, appEntityId: item.appEntityId })
    }
...
export function hideMissingPlanningNativeConnectionItems(connectionId: string, existingIdentifiers: string[]): void {
  ...
  withPlanningTransaction(() => {
    ...
    for (const binding of bindings) {
      ...
      const pending = getDatabase().prepare('SELECT id FROM planning_native_outbox WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').get({ connectionId, appEntityId: binding.app_entity_id })
      if (pending) {
        getDatabase().prepare(`INSERT INTO planning_native_sync_conflicts (id,connection_id,entity,app_entity_id,kind,native_item_json,detected_at) VALUES (:id,:connectionId,:entity,:appEntityId,'deleted',NULL,:now) ON CONFLICT(connection_id,app_entity_id) DO UPDATE SET kind='deleted',native_item_json=NULL,detected_at=excluded.detected_at,resolved_at=NULL`).run({ id: randomUUID(), connectionId, entity: connection.entity, appEntityId: binding.app_entity_id, now: Date.now() })
        continue
      }
      getDatabase().prepare(`DELETE FROM ${table} WHERE id=:id AND native_connection_id=:connectionId`).run({ id: binding.app_entity_id, connectionId })
      getDatabase().prepare('DELETE FROM planning_native_bindings WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ connectionId, appEntityId: binding.app_entity_id })
    }
  })
}
  ...
  withPlanningTransaction(() => {
    ...
    for (const item of items) {
      ...
      if (item.isRecurring) {
        if (binding) {
          const table = connection.entity === 'reminder' ? 'todos' : 'calendar_events'
          getDatabase().prepare('DELETE FROM planning_native_outbox WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ connectionId, appEntityId: binding.app_entity_id })
          getDatabase().prepare(`DELETE FROM ${table} WHERE id=:id AND native_connection_id=:connectionId`).run({ id: binding.app_entity_id, connectionId })
          getDatabase().prepare('DELETE FROM planning_native_bindings WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').run({ connectionId, appEntityId: binding.app_entity_id })
        }
        continue
      ...
      const localId = binding?.app_entity_id ?? randomUUID()
      // 本地写入尚未写回时，让 outbox 优先，避免覆盖用户刚在 Canopy 中完成的编辑。
      const pending = binding && getDatabase().prepare('SELECT id FROM planning_native_outbox WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').get({ connectionId, appEntityId: localId })
      if (pending) {
        // 两侧都在基于同一 binding 修改：停止自动覆盖，留给用户明确选择。
        getDatabase().prepare(`INSERT INTO planning_native_sync_conflicts (id,connection_id,entity,app_entity_id,kind,native_item_json,detected_at) VALUES (:id,:connectionId,:entity,:appEntityId,'changed',:nativeItemJson,:now) ON CONFLICT(connection_id,app_entity_id) DO UPDATE SET kind='changed',native_item_json=excluded.native_item_json,detected_at=excluded.detected_at,resolved_at=NULL`).run({ id: randomUUID(), connectionId, entity: connection.entity, appEntityId: localId, nativeItemJson: JSON.stringify(item), now })
        continue
      }
      if (entityType === 'todo') {
      ...
      getDatabase().prepare('INSERT INTO planning_native_bindings (connection_id,app_entity_id,calendar_item_identifier,due_date_only,last_native_hash,last_synced_at) VALUES (:connectionId,:appEntityId,:calendarItemIdentifier,:dueDateOnly,:hash,:now) ON CONFLICT(connection_id,app_entity_id) DO UPDATE SET calendar_item_identifier=excluded.calendar_item_identifier,due_date_only=excluded.due_date_only,last_native_hash=excluded.last_native_hash,last_synced_at=excluded.last_synced_at').run({ connectionId, appEntityId: localId, calendarItemIdentifier: item.calendarItemIdentifier, dueDateOnly: item.dueDateOnly ? 1 : 0, hash, now })
    }
    // 只有真正的完整快照才能以缺失推断系统端删除；有界 Calendar 列表不能这样做。
    if (!options.fullSnapshot) return
    // 系统端删除后只移除 Canopy 投影和 binding；绝不通过 outbox 删除 EventKit 原项。
    const bindings = getDatabase().prepare('SELECT * FROM planning_native_bindings WHERE connection_id=:connectionId').all({ connectionId }) as NativeBindingRow[]
    for (const binding of bindings) {
      if (seen.has(binding.calendar_item_identifier) || binding.recreate_pending === 1) continue
      const pending = getDatabase().prepare('SELECT id FROM planning_native_outbox WHERE connection_id=:connectionId AND app_entity_id=:appEntityId').get({ connectionId, appEntityId: binding.app_entity_id })
      if (pending) {
        getDatabase().prepare(`INSERT INTO planning_native_sync_conflicts (id,connection_id,entity,app_entity_id,kind,native_item_json,detected_at) VALUES (:id,:connectionId,:entity,:appEntityId,'deleted',NULL,:now) ON CONFLICT(connection_id,app_entity_id) DO UPDATE SET kind='deleted',native_item_json=NULL,detected_at=excluded.detected_at,resolved_at=NULL`).run({ id: randomUUID(), connectionId, entity: connection.entity, appEntityId: binding.app_entity_id, now })
        continue
      }
      const table = connection.entity === 'reminder' ? 'todos' : 'calendar_events'
      getDatabase().prepare(`DELETE FROM ${table} WHERE id=:id AND native_connection_id=:connectionId`).run({ id: binding.app_entity_id, connectionId })
...
/**
 * Canopy 受管 Calendar 的回流路径。直接写 SQLite，绝不调用正常 update/enqueue，
    ...
    for (const item of items) {
      ...
      if (!binding) {
        ...
        if (item.calendarItemExternalIdentifier) {
          for (const candidate of getDatabase().prepare('SELECT * FROM planning_sync_bindings WHERE profile_id=:profileId AND target_id=:targetId AND calendar_item_external_identifier=:externalIdentifier').all({ profileId, targetId: profile.target_id, externalIdentifier: item.calendarItemExternalIdentifier }) as SyncBindingRow[]) candidates.set(candidate.app_entity_id, candidate)
        }
        const markerCandidate = item.appIdentity ? getDatabase().prepare('SELECT * FROM planning_sync_bindings WHERE profile_id=:profileId AND target_id=:targetId AND app_entity_id=:appEntityId').get({ profileId, targetId: profile.target_id, appEntityId: item.appIdentity }) as SyncBindingRow | undefined : undefined
        if (markerCandidate) candidates.set(markerCandidate.app_entity_id, markerCandidate)
        // 仅”Canopy marker + 与既有 binding 相同的 external identifier”可自动认回；单个 external id 或复制 UUID marker 都不足以安全认领。
        if (candidates.size === 1 && markerCandidate && item.calendarItemExternalIdentifier && markerCandidate.calendar_item_external_identifier === item.calendarItemExternalIdentifier) binding = markerCandidate
          ...
          for (const candidate of candidates.values()) getDatabase().prepare(`INSERT INTO planning_sync_profile_conflicts (id,profile_id,app_entity_id,kind,native_item_json,detected_at)
            VALUES (:id,:profileId,:appEntityId,'changed',:nativeItemJson,:now)
            ON CONFLICT(profile_id,app_entity_id) DO UPDATE SET kind='changed',native_item_json=excluded.native_item_json,detected_at=excluded.detected_at`).run({ id: randomUUID(), profileId, appEntityId: candidate.app_entity_id, nativeItemJson: JSON.stringify(item), now })
          continue
        }
      }
      if (item.isRecurring) {
        // 不把 recurrence series/exception 误降维为单次 Canopy event。
        if (binding) {
          getDatabase().prepare('DELETE FROM planning_sync_outbox WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId, appEntityId: binding.app_entity_id })
          getDatabase().prepare("DELETE FROM planning_reminders WHERE target_type='calendar_event' AND target_id=:id").run({ id: binding.app_entity_id })
          getDatabase().prepare('DELETE FROM calendar_events WHERE id=:id').run({ id: binding.app_entity_id })
          getDatabase().prepare('DELETE FROM planning_sync_bindings WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId, appEntityId: binding.app_entity_id })
        }
        continue
      }
      if (binding && binding.calendar_item_identifier !== item.calendarItemIdentifier) {
        // 先持久化新 locator，再比较内容基线；否则内容相同的 locator 迁移会被误判为“无变化”。
        getDatabase().prepare('UPDATE planning_sync_bindings SET calendar_item_identifier=:calendarItemIdentifier,calendar_item_external_identifier=:calendarItemExternalIdentifier,last_synced_at=:now WHERE profile_id=:profileId AND app_entity_id=:appEntityId').run({ profileId, appEntityId: binding.app_entity_id, calendarItemIdentifier: item.calendarItemIdentifier, calendarItemExternalIdentifier: item.calendarItemExternalIdentifier ?? binding.calendar_item_external_identifier ?? null, now })
      ...
      const recoveredPending = !binding && item.appIdentity && getDatabase().prepare(`SELECT outbox.id FROM planning_sync_outbox AS outbox
        JOIN calendar_events AS events ON events.id=outbox.app_entity_id
        WHERE outbox.profile_id=:profileId AND outbox.app_entity_id=:appEntityId AND outbox.operation='upsert'`).get({ profileId, appEntityId: item.appIdentity })
      const localId = binding?.app_entity_id ?? (recoveredPending ? item.appIdentity! : randomUUID())
      const pending = (binding || recoveredPending) && getDatabase().prepare('SELECT id FROM planning_sync_outbox WHERE profile_id=:profileId AND app_entity_id=:appEntityId').get({ profileId, appEntityId: localId })
      if (pending && !recoveredPending) {
        // 双端在同一基线编辑时，宁可暂停出站，也不能静默后写覆盖系统版本。
        getDatabase().prepare(`INSERT INTO planning_sync_profile_conflicts (id,profile_id,app_entity_id,kind,native_item_json,detected_at)
          VALUES (:id,:profileId,:appEntityId,'changed',:nativeItemJson,:now)
          ON CONFLICT(profile_id,app_entity_id) DO UPDATE SET kind='changed',native_item_json=excluded.native_item_json,detected_at=excluded.detected_at`).run({ id: randomUUID(), profileId, appEntityId: localId, nativeItemJson: JSON.stringify(item), now })
        continue
      }
      ...
      getDatabase().prepare(`INSERT INTO planning_sync_bindings (profile_id,target_id,app_entity_id,calendar_item_identifier,calendar_item_external_identifier,last_synced_hash,last_synced_at)
        VALUES (:profileId,:targetId,:appEntityId,:calendarItemIdentifier,:calendarItemExternalIdentifier,:hash,:now)
        ON CONFLICT(profile_id,app_entity_id) DO UPDATE SET target_id=excluded.target_id,calendar_item_identifier=excluded.calendar_item_identifier,calendar_item_external_identifier=excluded.calendar_item_external_identifier,last_synced_hash=excluded.last_synced_hash,last_synced_at=excluded.last_synced_at`).run({ profileId, targetId: profile.target_id, appEntityId: localId, calendarItemIdentifier: item.calendarItemIdentifier, calendarItemExternalIdentifier: item.calendarItemExternalIdentifier ?? null, hash, now })
    }
  })
}
...
export function hideMissingManagedCalendarProfileItems(profileId: string, targetId: string, existingIdentifiers: string[]): void {
  ...
  withPlanningTransaction(() => {
    ...
    for (const binding of bindings) {
      ...
      const pending = getDatabase().prepare('SELECT id FROM planning_sync_outbox WHERE profile_id=:profileId AND app_entity_id=:appEntityId').get({ profileId, appEntityId: binding.app_entity_id })
      const recoveryConflict = getDatabase().prepare('SELECT id FROM planning_sync_profile_conflicts WHERE profile_id=:profileId AND app_entity_id=:appEntityId').get({ profileId, appEntityId: binding.app_entity_id })
      if (pending && !recoveryConflict) {
        getDatabase().prepare(`INSERT INTO planning_sync_profile_conflicts (id,profile_id,app_entity_id,kind,native_item_json,detected_at)
          VALUES (:id,:profileId,:appEntityId,'deleted',NULL,:now)
          ON CONFLICT(profile_id,app_entity_id) DO UPDATE SET kind='deleted',native_item_json=NULL,detected_at=excluded.detected_at`).run({ id: randomUUID(), profileId, appEntityId: binding.app_entity_id, now: Date.now() })
      }
    ...
    WHERE status='pending' AND COALESCE(snoozed_until,trigger_at) <= :now AND last_notified_at IS NULL
      AND NOT (
        origin='todo_due_at' AND target_type='todo' AND EXISTS (
          ...
          JOIN planning_sync_bindings AS bindings ON bindings.profile_id=profiles.id AND bindings.app_entity_id=planning_reminders.target_id
