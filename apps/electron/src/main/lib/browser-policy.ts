...
// 受管浏览器的默认搜索引擎跟随设置项（默认必应，见 @canopy/shared 的 BROWSER_SEARCH_ENGINES）。
// 本模块保持无 Electron 依赖：主进程在设置加载 / 变更时用 setBrowserSearchEngine 把选择同步进来，
// 这里不直接读设置文件，便于在普通 Bun 测试中验证。
import { BROWSER_SEARCH_ENGINES, DEFAULT_BROWSER_SEARCH_ENGINE, isBrowserSearchEngineId, type BrowserSearchEngineId } from '@canopy/shared'

let activeSearchEngine: BrowserSearchEngineId = DEFAULT_BROWSER_SEARCH_ENGINE

/** 同步设置里的搜索引擎选择；非法值一律回落默认。返回实际生效值。 */
export function setBrowserSearchEngine(value: unknown): BrowserSearchEngineId {
  activeSearchEngine = isBrowserSearchEngineId(value) ? value : DEFAULT_BROWSER_SEARCH_ENGINE
  return activeSearchEngine
}

export function getBrowserSearchEngine(): BrowserSearchEngineId {
  return activeSearchEngine
}

/** 用户新建空白标签页 / 首次打开浏览器的首页。 */
export function getBrowserNewTabUrl(engine: BrowserSearchEngineId = activeSearchEngine): string {
  return BROWSER_SEARCH_ENGINES[engine].homeUrl
}

/** 普通文本按搜索词处理时使用的搜索端点。 */
export function getBrowserSearchUrl(engine: BrowserSearchEngineId = activeSearchEngine): string {
  return BROWSER_SEARCH_ENGINES[engine].searchUrl
}
...
/**
 * 把地址栏/BrowserNavigate 输入解析成 URL 或当前搜索引擎的搜索地址。
 * 明确的 URL 和可识别的主机名保持直达，普通文本按搜索词处理。
 */
export function resolveBrowserDestination(input: string, engine: BrowserSearchEngineId = activeSearchEngine): string {
  const value = input.trim()
  if (!value) throw new Error('浏览器地址或搜索内容不能为空。')
  if (isLikelyUrl(value)) return normalizeBrowserUrl(value)
  return `${getBrowserSearchUrl(engine)}?q=${encodeURIComponent(value)}`
}

/**
 * 仅拒绝空值或无法被 URL 标准解析的输入；搜索词会先被转换为当前搜索引擎的搜索 URL。
 */
export async function assertSafeBrowserDestination(input: string, engine: BrowserSearchEngineId = activeSearchEngine): Promise<string> {
  const resolved = resolveBrowserDestination(input, engine)
  try {
