...
async function callGeminiAndBuildResult(
  ...
  for (const part of parts) {
    ...
    if (part.inlineData) {
      ...
      textParts.push(`[CANOPY_IMAGE_ATTACHMENT:${attachmentMeta}]`)
    } else if (part.text) {
...
function toPiToolResult(result: McpToolResult): KernelToolResult {
  // 图片已在生成时保存为 Canopy attachment，并在文本里携带渲染标记。普通 tool
  // result 保持文本形态即可：避免把 Gemini base64 图片重复写入 transcript。
...
/**
 * 构建生图工具定义。会话历史仍按 Canopy sessionId 隔离，因此连续编辑与 Claude
 * runtime 时代保持相同行为；参考图只从用户已授权的工作目录读取。返回值是中性的
 * `KernelToolDefinition`，转成 Pi 认识的形状由 `@canopy/runtime-pi` 的
 * `adaptToolsForPi()` 负责。
 */
      ...
      try {
        const result = await callGeminiAndBuildResult(String(args.prompt), ctx.sessionId, {
          ...
          referenceImagePaths: Array.isArray(args.referenceImagePaths)
            ? args.referenceImagePaths.filter((path: unknown): path is string => typeof path === 'string')
            : undefined,
          cwd: ctx.agentCwd,
