...
/**
 * 保留原导出以兼容启动流程；Canopy 内部听写不再创建独立窗口。
 */
...
export function toggleVoiceDictationWindow(options: VoiceDictationToggleOptions = {}): void {
  ...
  const targetIsApp = captureVoiceDictationTarget(options.targetIsApp)
  const outputMode = getSettings().voiceDictation?.outputMode ?? 'auto'
  const routeToAppInput = shouldRouteVoiceDictationToAppInput(targetIsApp, outputMode)
  const outputContextId = randomUUID()
  beginVoiceDictationOutputContext(outputContextId, { routeToAppInput, outputMode })
  activeOutputContextId = outputContextId
  voiceDictationState = 'active'
  usesExternalIndicator = !targetIsApp
...
function shouldRouteVoiceDictationToAppInput(
  targetIsApp: boolean,
  outputMode: VoiceDictationOutputMode,
): boolean {
  return outputMode === 'app-input' || (outputMode === 'auto' && targetIsApp)
}
