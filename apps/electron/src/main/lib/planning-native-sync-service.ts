...
export interface PlanningNativeSyncItem {
  targetId: string
  /** Canopy UUID；仅用于新建项目的 crash-recovery marker，不覆盖用户已有 URL。 */
  identity: string
  calendarItemIdentifier?: string
  /** 仅用户在冲突中明确选择“保留 Canopy”后才允许 locator 缺失时重建。 */
...
/** 按 Canopy 已保存 locator 精确确认删除，不把有界 Calendar 查询误判成完整快照。 */
...
/** 删除必须带受管目标和 Canopy marker；locator 缺失时 native addon 可恢复定位，避免崩溃留下孤儿项。 */
