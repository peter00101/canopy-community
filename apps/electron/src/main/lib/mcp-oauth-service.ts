...
import type { McpOAuthConfiguration, McpOAuthProvider, McpOAuthStartResult, StartMcpOAuthInput } from '@canopy/shared'
import { getMcpOAuthCredentialsPath } from './config-paths'
import { writeJsonFileAtomic } from '@canopy/kernel'
import { runWithOAuthProxyScope } from '@canopy/runtime-pi'
...
async function registerClient(registrationEndpoint: string, redirectUri: string): Promise<string> {
  const response = await fetch(registrationEndpoint, {
    ...
    body: JSON.stringify({
      client_name: CANOPY_BRAND.productName,
...
async function startCallbackServer(state: string): Promise<{ redirectUri: string; waitForCode: Promise<string>; cancel: () => void }> {
  ...
  const server = createServer((request, response) => {
    ...
    response.end(stateMatches && code
      ? `<!doctype html><title>${CANOPY_BRAND.productName}</title><p>授权完成，可以关闭此页面并返回 ${CANOPY_BRAND.productName}。</p>`
      : `<!doctype html><title>${CANOPY_BRAND.productName}</title><p>授权未完成，请返回 ${CANOPY_BRAND.productName} 重试。</p>`)
...
export async function getMcpOAuthHeaders(workspaceSlug: string, serverName: string, serverUrl: string): Promise<Record<string, string> | undefined> {
  ...
  if (oauthCredential.expiresAt && oauthCredential.expiresAt <= Date.now() + EXPIRY_SKEW_MS) {
    const refreshedCredential = await runWithOAuthProxyScope(() => refreshCredential(oauthCredential), getEffectiveProxyUrl)
