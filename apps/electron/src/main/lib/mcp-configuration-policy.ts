...
import type { McpOAuthConfiguration, McpServerEntry, McpTransportType } from '@canopy/shared'
...
export function requireWorkspaceMcpServerName(name: string): string {
  ...
  if (RESERVED_BUILTIN_KEYS.has(normalized)) throw new Error(`MCP 服务名 ${normalized} 是 ${CANOPY_BRAND.productName} 运行时保留名`)
