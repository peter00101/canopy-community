...
import { IPC_CHANNELS, CHANNEL_IPC_CHANNELS, CHAT_IPC_CHANNELS, AGENT_IPC_CHANNELS, AGENT_ISLAND_IPC_CHANNELS, ENVIRONMENT_IPC_CHANNELS, INSTALLER_IPC_CHANNELS, PROXY_IPC_CHANNELS, SYSTEM_PROMPT_IPC_CHANNELS, CHAT_TOOL_IPC_CHANNELS, FEISHU_IPC_CHANNELS, DINGTALK_IPC_CHANNELS, WECHAT_IPC_CHANNELS, AUTOMATION_IPC_CHANNELS, PLANNING_IPC_CHANNELS, VAULT_IPC_CHANNELS, PLANNING_CONFLICT_ERROR, MAX_ATTACHMENT_SIZE, isAppPermissionMode, normalizePathForCompare, removeMcpServerFromConfig, TERMINAL_IPC_CHANNELS } from '@canopy/shared'
...
import { getBrowserDownloadsDir } from './lib/browser-download-policy'
...
import { registerAppDirectoryPath, registerAppFilePath } from './lib/local-file-protocol'
import {
...
import { resolveAppIconRelativePath } from './lib/app-icon-path'
import {
  listChannels,
  createChannel,
  updateChannel,
...
import { loginCodexOAuth, cancelCodexOAuthLogin, loginXaiOAuth, cancelXaiOAuthLogin, type PiOAuthNetworkDeps } from '@canopy/runtime-pi'
import { resolvePiReasoningCapability } from '@canopy/runtime-pi'
import { serializeCodexCredentials, serializeXaiCredentials } from '@canopy/shared'
import type { CodexOAuthDeviceCode, CodexOAuthLoginMethod, XaiOAuthDeviceCode } from '@canopy/shared'
import {
  listConversations,
  createConversation,
...
} from '@canopy/business-automation'
import {
  listTodos,
  getTodo,
  createTodo,
...
import {
  ...
  addWorktreeRepo,
  removeWorktreeRepo,
  getAttachedPathStatuses,
  getExistingWorkspaceAutoMemoryDir,
...
const workspaceMcpPendingValidations = new Map<string, Map<string, import('@canopy/shared').McpServerEntry>>()
...
function getWorkspaceMcpPendingValidation(workspaceSlug: string, name: string): import('@canopy/shared').McpServerEntry | undefined {
  return workspaceMcpPendingValidations.get(workspaceSlug)?.get(name)
}

function setWorkspaceMcpPendingValidation(workspaceSlug: string, name: string, entry: import('@canopy/shared').McpServerEntry): void {
  const pending = workspaceMcpPendingValidations.get(workspaceSlug) ?? new Map<string, import('@canopy/shared').McpServerEntry>()
...
import {
  ...
  dingtalkBridgeManager,
  getWeChatConfig,
  wechatBridge,
} from './lib/im-bridge-service'
...
import { isPythonEnvironmentDirName, isTransientFileName } from './lib/file-visibility'
import { isPathWithinRoots, realpathOrResolve } from './lib/file-access-policy'
import {
  canRegisterPreviewDirectory,
  filterExplicitPreviewBasePaths,
  isExplicitPreviewDirectoryPath,
  isExplicitPreviewFilePath,
  resolveAuthorizedPreviewPath as resolveAuthorizedPreviewPathInScope,
  type PreviewAccessScope,
} from './lib/preview-access-policy'
import { TUTORIAL_IMAGES_DIR_NAME } from './lib/tutorial-image-paths'
 ...
 */
// realpathOrResolve / isUnderRoot / isPathWithinRoots 的纯逻辑已抽到 lib/file-access-policy.ts
// （0.17.24，审查报告 A-3）：那边可测、这边只剩「收集授权根」的胶水。口径未变。

function getAuthorizedRoots(options?: FileAccessOptions): string[] {
  const roots: string[] = [
    getAgentWorkspacesDir(),
    join(tmpdir(), 'canopy-preview'),
  ]
...
function isPathAllowed(filePath: string, options?: FileAccessOptions): boolean {
  // 根集合惰性收集：路径不存在或 unrestricted 时不会去读会话索引（与抽出前一致）
  return isPathWithinRoots(filePath, () => getAuthorizedRoots(options), options?.unrestricted === true)
}
...
/**
 * 可作为相对路径根或 HTML 资源根的显式目录授权（预览家族专用，上游 #2020）。
 * 不包含 getAgentWorkspacesDir() 这种聚合根，避免单个会话/文件的权限借聚合根横向扩张。
 *
 * 我方在上游集合之外多加了六类根，都是查证过「不加就静默失效」的既有流程：
 * - 工作区长期记忆目录：Agent 裸提记忆文件名时的候选（issue #1691，上游 #2020 没这层候选）；
 * - 工作区 skills 目录：Agent 用 Read 加载 Skill，工具行「预览」与文件芯片指向
 *   `<workspace>/skills/<skill>/SKILL.md`，不列为根就只能得到「未找到文件」（0.19.11）；
 * - ~/.canopy/attachments：聊天附件点开走 chat:resolve-attachment-path → 预览家族；
 * - ~/.canopy/browser-downloads：受管浏览器下载产物，Agent 正文会引用其绝对路径；
 * - resources/tutorial-images：教程配图（0.18.52 起随包分发），教程标签页不带任何会话上下文；
 * - tmpdir()/canopy-preview：剪贴板草稿（agent:write-clipboard-preview 就落在这里，点附件
 *   芯片预览要读它）与预览临时产物。getAuthorizedRoots() 里本来就有这一条，不补上则
 *   「粘贴一段文本 → 点它预览」会静默变空白。
 */
function getExplicitPreviewDirectoryRoots(options?: FileAccessOptions): string[] {
  ...
  add(getManagedSkillBasePath(options))
  ...
  for (const slug of getWorkspaceSlugsForAccess(options)) {
    ...
    add(getExistingWorkspaceAutoMemoryDir(slug))
    // Agent 用普通 Read 加载 Skill（<workspace>/skills/<skill>/SKILL.md），工具行的「预览」
    // 与文件芯片带的就是这个绝对路径；不把工作区 skills 目录列为根，点开只会得到「未找到文件」
    add(getWorkspaceSkillsDir(slug))
  }

  // 与会话无关的应用级产物目录（路径只做 join，不在热路径上 mkdir）
  add(join(getConfigDir(), 'attachments'))
  add(getBrowserDownloadsDir())
  add(join(getBundledResourcesDir(), TUTORIAL_IMAGES_DIR_NAME))
  add(join(tmpdir(), 'canopy-preview'))
  return roots
}
...
/** 一次预览请求收集一次根集合；unrestricted 时不读会话索引（判定层会直接放行）。 */
function getPreviewAccessScope(options?: FileAccessOptions): PreviewAccessScope {
  if (options?.unrestricted) return { directoryRoots: [], filePaths: [], unrestricted: true }
  return {
    directoryRoots: getExplicitPreviewDirectoryRoots(options),
    filePaths: getExplicitPreviewFilePaths(options),
    unrestricted: false,
  }
}

function getAllowedCandidateBasePaths(options?: FileAccessOptions): string[] | undefined {
  const allowed = filterExplicitPreviewBasePaths(getPreviewCandidateBasePaths(options), getPreviewAccessScope(options))
...
function getPreviewCandidateBasePaths(options?: FileAccessOptions): string[] | undefined {
  ...
  // 长期记忆目录排在调用方给的候选（会话工作台 / 附加目录 / 项目根）之后：同名文件以会话侧为准，
  // 记忆目录只兜住「Agent 裸提记忆文件名」这一类（上游 issue #1691）。只加真实存在的目录，
  // 且刻意不加整个工作区根——递归搜索会串到其他会话的私有工作台，宁可「未找到」也不错配。
  for (const slug of getWorkspaceSlugsForAccess(options)) {
    const memoryDir = getExistingWorkspaceAutoMemoryDir(slug)
    if (memoryDir && !bases.includes(memoryDir)) bases.push(memoryDir)
  }
...
/** `file:read-preview-binary` 只放行渲染层后台线程要解析的格式 */
const PREVIEW_BINARY_EXTENSIONS: ReadonlySet<string> = new Set(['.xlsx', '.xls', '.ppt'])
/** 与 PDF / Office 预览同一档上限 */
const PREVIEW_BINARY_MAX_BYTES = 50 * 1024 * 1024

/**
 * 预览家族统一入口：所有内联预览在注册文件 URL 或读取内容前，都必须验证最终 realpath 的授权范围。
 * 纯判定逻辑在 lib/preview-access-policy.ts（可测），这里只负责收集当前请求的根集合。
 */
function resolveAuthorizedPreviewPath(filePath: string, options?: FileAccessOptions): string | null {
  return resolveAuthorizedPreviewPathInScope(filePath, getPreviewCandidateBasePaths(options), getPreviewAccessScope(options))
}
 ...
 *
 * worktree 常被放在主仓库之外（如 ~/canopy-dev/worktrees/xxx），其路径不在任何
...
async function ensurePathAllowedWithWorktree(filePath: string, options?: FileAccessOptions): Promise<boolean> {
  ...
  if (mainRepo) {
    ...
    for (const workspaceSlug of getWorkspaceSlugsForAccess(options)) {
      let repos: import('@canopy/shared').WorkspaceWorktreeRepo[]
      try {
 ...
 */
const defaultAppCache = new Map<string, import('@canopy/shared').DefaultAppInfo>()
...
async function getMacAppIconViaSips(appPath: string): Promise<string> {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'canopy-icon-'))
...
interface McpRefreshValidation {
  name: string
  fingerprint: string
  lastTestResult: NonNullable<import('@canopy/shared').McpServerEntry['lastTestResult']>
}

/**
 * 生成 MCP 可运行配置的稳定摘要。摘要只用于内存中比较，绝不记录或返回，避免暴露 headers/env 中的敏感值。
 */
export function getMcpEntryFingerprint(entry: import('@canopy/shared').McpServerEntry): string {
...
export function mergeMcpRefreshResults(
  currentConfig: import('@canopy/shared').WorkspaceMcpConfig,
  validations: readonly McpRefreshValidation[],
): import('@canopy/shared').WorkspaceMcpConfig {
...
async function validateAndConditionallyPersistMcp(
  ...
  name: string,
  candidateEntry: import('@canopy/shared').McpServerEntry,
  expectedPersistedFingerprint: string,
  expectedRefreshGeneration: number,
): Promise<import('@canopy/shared').McpConnectionMutationResult> {
...
): Promise<import('@canopy/shared').CliIntegrationStatus[]> {
...
): Promise<import('@canopy/shared').DefaultAppInfo | null> {
...
if let appUrl = NSWorkspace.shared.urlForApplication(toOpen: url) {
  ...
  const info: import('@canopy/shared').DefaultAppInfo = { name: appName, appPath, iconDataUrl }
...
/**
 * 解析应用图标变体的文件路径
 * darwin 上非默认变体走 dock/ 下的圆角加工版本，见 resolveAppIconRelativePath。
 */
export function resolveAppIconPath(variantId: string): string | null {
  return join(getBundledResourcesDir(), resolveAppIconRelativePath(variantId, process.platform))
}
...
/** Codex/xAI OAuth 登录复用的网络能力注入——运行时插件（`@canopy/runtime-pi`）不感知 Electron。 */
const piOAuthNetworkDeps: PiOAuthNetworkDeps = {
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@canopy/shared').EditorApp[]> => {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@canopy/shared').DefaultAppInfo | null> => {
  ...
  ipcMain.handle(
    ...
    async (_, channelId: string): Promise<import('@canopy/shared').ChannelPlanQuotaResult> => {
  ...
  ipcMain.handle(
    ...
    async (event, requestedMethod?: CodexOAuthLoginMethod): Promise<import('@canopy/shared').CodexOAuthLoginResult> => {
      const method: CodexOAuthLoginMethod = requestedMethod === 'device_code' ? 'device_code' : 'browser'
      try {
        const credentials = await loginCodexOAuth(piOAuthNetworkDeps, {
          method,
  ...
  ipcMain.handle(
    ...
    async (event): Promise<import('@canopy/shared').XaiOAuthLoginResult> => {
      try {
        const credentials = await loginXaiOAuth(piOAuthNetworkDeps, {
          onDeviceCode: (deviceCode) => {
            void withOAuthDeviceCodeQr(deviceCode).then((payload) => {
              if (!event.sender.isDestroyed()) {
  ...
  // 教程内容（我方保留项，见 shared/chat.ts 说明）
  ipcMain.handle(
  ...
  // 解析附件的绝对路径（供消息附件点击打开文件预览）
  ipcMain.handle(
    CHAT_IPC_CHANNELS.RESOLVE_ATTACHMENT_PATH,
    async (_, localPath: string): Promise<string | null> => {
      try {
        return resolveAttachmentAbsolutePath(localPath)
      } catch {
        return null
      }
    }
  ...
  // 汇总全部会话的 token 用量（只统计不算钱；口径见 kernel/agent-usage-stats.ts）
  // range 省略即全部；时间按消息自带的 _createdAt 过滤，日期分组走主进程所在的本地时区
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_USAGE_STATS,
    async (_event, range?: UsageStatsRange): Promise<AgentUsageStats> => aggregateAgentUsage(range ?? {}),
  )
  ...
  ipcMain.handle(AGENT_IPC_CHANNELS.EXPORT_BROWSER_PDF, async (event, input: BrowserTabInput): Promise<BrowserDownloadItem> => {
    await assertBrowserSessionAccess(event.sender.id, input.sessionId)
    return browserController.exportPdf(input.sessionId, input.tabId)
  })
  ipcMain.handle(AGENT_IPC_CHANNELS.SHOW_BROWSER_DOWNLOAD, async (event, input: { sessionId: string; path?: string }): Promise<void> => {
    await assertBrowserSessionAccess(event.sender.id, input.sessionId)
    // 只允许展示会话下载记录里的路径，或直接打开下载目录，不接受任意路径
    const downloads = browserController.listDownloads(input.sessionId)
    const matched = input.path ? downloads.find((d) => d.path === input.path) : undefined
    if (matched && existsSync(matched.path)) {
      shell.showItemInFolder(matched.path)
      return
    }
    const dir = getBrowserDownloadsDir()
    mkdirSync(dir, { recursive: true })
  ...
  ipcMain.handle(
    ...
    async (_, id: string): Promise<void> => {
      ...
      // 先写墓碑再等中止收束：headless run 可能仍在异步预检，本 handler 删完文件后
      // 它若继续跑就会把 transcript 重新创建出来（收上游 #2074）
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').CreateAgentWorkspaceInput): Promise<AgentWorkspace> => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').CreateAgentWorkspaceInput, channelId?: string, modelId?: string): Promise<import('@canopy/shared').CreateAgentProjectResult> => {
      const workspace = await createAgentWorkspace(input)
      if (workspace.projectRootPath) watchAttachedDirectory(workspace.projectRootPath)
  ...
  ipcMain.handle(
    ...
    async (_, id: string): Promise<void> => {
      ...
      for (const sessionId of affectedSessionIds) {
        // 与单会话删除同一道「墓碑 + 收束」边界：headless / 委派 run 可能还在预检阶段，
        // 此时它并不算 active，光判 isAgentSessionActive 会漏掉（收上游 #2074）
        await stopAgentAndDrain(sessionId)
        closeTerminalsForSession(sessionId)
        // 与单会话删除一致：受管浏览器的原生视图与页面一并关掉（收上游 #2078）
        await browserController.close(sessionId)
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, config: WorkspaceMcpConfig, options?: import('@canopy/shared').SaveWorkspaceMcpConfigOptions): Promise<void> => {
      for (const name of options?.explicitlyDisabledServerNames ?? []) {
        clearWorkspaceMcpPendingValidation(workspaceSlug, name)
      }
      const pendingValidations: Array<{ name: string; candidate: import('@canopy/shared').McpServerEntry }> = []
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, name: string, enabled: boolean): Promise<import('@canopy/shared').McpConnectionMutationResult> => {
      const current = getWorkspaceMcpConfig(workspaceSlug)
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, name: string, entry: import('@canopy/shared').McpServerEntry): Promise<import('@canopy/shared').McpInstallMutationResult> => {
      const current = getWorkspaceMcpConfig(workspaceSlug)
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').StartMcpOAuthInput): Promise<import('@canopy/shared').McpOAuthStartResult> => {
  ...
  ipcMain.handle(
    ...
    (_, input: import('@canopy/shared').SaveMcpOAuthClientSecretInput): void => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').SaveMcpApiKeyInput): Promise<void> => {
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string): Promise<import('@canopy/shared').CliIntegrationStatus[]> => {
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, id: string, enabled: boolean): Promise<import('@canopy/shared').CliIntegrationStatus[]> => {
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, name: string, entry: import('@canopy/shared').McpServerEntry): Promise<{ success: boolean; message: string }> => {
      const { validateMcpServer } = await import('./lib/mcp-validator')
      const result = await validateMcpServer(name, entry, workspaceSlug)
      return {
        success: result.valid,
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string): Promise<import('@canopy/shared').AgentStopResult> => {
      feishuBridgeManager.stopSessionMirrorRun(sessionId)
      return { hadActiveRun: stopAgent(sessionId) }
    }
  )

  // 查询主进程当前仍在执行的会话（渲染层运行态看门狗对账）
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_ACTIVE_SESSION_IDS,
    async (): Promise<string[]> => listActiveAgentSessionIds()
  )
  ...
  ipcMain.handle(
    ...
    async (event, input: import('@canopy/shared').AgentQueueMessageInput): Promise<string> => {
  ...
  ipcMain.handle(
    ...
    async (event, input: import('@canopy/shared').AgentSubmitOrEnqueueInput): Promise<import('@canopy/shared').AgentSubmitOrEnqueueResult> => {
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string): Promise<import('@canopy/shared').AgentQueuedMessageSnapshot[]> => {
  ...
  ipcMain.handle(
    ...
    async (event, input: import('@canopy/shared').AgentDeferredQueueMessageInput): Promise<void> => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').AgentQueuedMessageControlInput): Promise<boolean> => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').AgentMoveQueuedMessageInput): Promise<boolean> => {
    ...
    async (event, response: PermissionResponse): Promise<void> => {
      ...
      if (sessionId) {
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'app_event', event: { type: 'permission_resolved', requestId, behavior } },
        })
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, mode: AppPermissionMode): Promise<void> => {
      if (!isAppPermissionMode(mode)) {
  ...
  ipcMain.handle(
    ...
    async (event, response: AskUserResponse): Promise<void> => {
      ...
      if (sessionId) {
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'app_event', event: { type: 'ask_user_resolved', requestId } },
        })
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (event, response: ExitPlanModeResponse): Promise<ExitPlanModeRespondResult> => {
      // 反馈的用户可见形态：作为一条用户消息落盘（渲染层用同一 uuid 乐观上屏）。
      // 必须在 respond 之前写——此刻 Pi 还挂在审批上，绝不会与下一轮 assistant 消息抢落盘顺序。
      // 只落展示层 JSONL；模型侧由 deny message 通道传达，不会重复送。
      const feedbackText = response.feedback?.trim()
      if (response.action === 'feedback' && feedbackText && exitPlanService.hasPendingRequest(response.requestId)) {
        const pendingRequest = exitPlanService.getPendingRequests().find((r) => r.requestId === response.requestId)
        if (pendingRequest) {
          try {
            appendSDKMessages(pendingRequest.sessionId, [{
              type: 'user',
              uuid: response.feedbackMessageUuid || randomUUID(),
              message: { content: [{ type: 'text', text: feedbackText }] },
              parent_tool_use_id: null,
              _createdAt: Date.now(),
            } as unknown as SDKMessage])
          } catch (err) {
            console.warn(`[IPC] ExitPlanMode 反馈落盘失败: requestId=${response.requestId}`, err)
          }
        }
      }

      const result = exitPlanService.respondToExitPlanMode(response)
      // exit_plan_mode_resolved 已由 service 的 resolvedNotifier 统一广播（覆盖 abort / 会话清理路径）
      ...
      if (result) {
        ...
        if (targetMode) {
          ...
          event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
            sessionId,
            payload: { kind: 'app_event', event: { type: 'permission_mode_changed', mode: targetMode } },
          })
          console.log(`[IPC] ExitPlanMode 权限模式切换: ${targetMode}`)
        }
      }

      // accepted=false 时渲染层必须提示用户（该审批已失效），不能静默——曾表现为「提意见后毫无反应」
      return { accepted: result !== null, planDocumentChanged: result?.planDocumentChanged === true }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@canopy/shared').PendingRequestsSnapshot> => {
      return {
  ...
  // 批量查询附加路径可用状态：只读判定，绝不回写配置
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_ATTACHED_PATH_STATUSES,
    async (_, paths: string[]): Promise<import('@canopy/shared').AttachedPathState[]> => {
      if (!Array.isArray(paths)) return []
      return getAttachedPathStatuses(paths.filter((p): p is string => typeof p === 'string'))
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, repo: import('@canopy/shared').WorkspaceWorktreeRepo) => {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions): Promise<void> => {
      ...
      const safePath = resolve(filePath)
      const options = normalizeFileAccessOptions(access)
      // 预览工具条的「用默认应用打开」「打开当前文件夹」也走这里，而预览家族接授权链后
      // （#2020）附件 / 浏览器下载 / 教程配图这些目录只在预览根集合里、不在 getAuthorizedRoots()。
      // 此前它们是靠 DiffTabContent 无条件传 unrestricted 才通过的；收窄后若只认 isPathAllowed，
      // 用户对着正在预览的附件点这两个按钮会失败。故补一条：预览策略已放行的文件或目录同样放行。
      const previewScope = getPreviewAccessScope(options)
      const allowed = isPathAllowed(safePath, options)
        || isExplicitPreviewFilePath(safePath, previewScope)
        || isExplicitPreviewDirectoryPath(safePath, previewScope)
      if (!allowed) {
        throw new Error('访问路径超出当前会话的授权范围')
      }
  ...
  ipcMain.handle(
    ...
    async (_, filename: string, content: string): Promise<string> => {
      ...
      const tmpDir = join(tmpdir(), 'canopy-preview')
  ...
  ipcMain.handle(
    ...
    async (
      ...
      options?: import('./lib/reveal-target').RevealTargetOptions,
    ): Promise<boolean> => {
      const { resolve } = await import('node:path')
      const { resolveTargetPath } = await import('./lib/file-preview-service')
      const { decideRevealTarget } = await import('./lib/reveal-target')

      const resolvedPath = resolveTargetPath(filePath, candidateBasePaths?.length ? candidateBasePaths : undefined)
      const target = decideRevealTarget(resolvedPath, { fallbackToParentFolder: options?.fallbackToParentFolder === true })
      if (target.kind === 'missing') {
        console.warn('[IPC] shell:show-item-in-folder 路径不存在:', resolvedPath)
        return false
      }
      if (target.kind === 'folder') {
        // 文件还没创建（如项目记忆里的 AGENTS.md）：打开它所在的文件夹，而不是点了没反应
        const error = await shell.openPath(resolve(target.path))
        if (error) {
          console.warn('[IPC] shell:show-item-in-folder 打开所在文件夹失败:', target.path, error)
          return false
        }
        return true
      }
      shell.showItemInFolder(resolve(target.path))
      return true
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@canopy/shared').FilePreviewReadResult | null> => {
      const { resolveAndReadFile } = await import('./lib/file-preview-service')
      const options = normalizeFileAccessOptions(access)
      const resolved = resolveAuthorizedPreviewPath(filePath, options)
  ...
  // Markdown 本体与解析出的图片都必须过预览授权链：否则渲染层可拿任意目录下的
  // 一个 .md 当锚点，换取其同目录任意文件的 token。
  ipcMain.handle(
    ...
    async (_, markdownFilePath: string, src: string, access?: FileAccessOptions | string[]): Promise<ResolvedFileUrl | null> => {
      ...
      const normalized = normalizeFileAccessOptions(access)
      const options = normalized?.unrestricted ? { ...normalized, unrestricted: false } : normalized
      const markdownPath = resolveAuthorizedPreviewPath(markdownFilePath, options)
      if (!markdownPath) return null
      const result = resolveMarkdownRelativeMediaPath(markdownPath, src, options)
      if (!result || !isExplicitPreviewFilePath(result, getPreviewAccessScope(options))) return null
      try {
        return { url: registerAppFilePath(result), path: result, resolvedPath: result }
      } catch (err) {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<(ResolvedFileUrl & { resolvedPath: string }) | null> => {
      const options = normalizeFileAccessOptions(access)
      const result = resolveAuthorizedPreviewPath(filePath, options)
      if (!result) return null
      // registerAppFilePath 对目录路径会抛「不是文件」。渲染端（如悬浮预览解析 markdown
  ...
  // 为内联 HTML 预览生成受限 URL（iframe sandbox 加载；红线：走我方自有 prepareHtmlPreview 链路）。
  // 所在目录本身已授权时整目录注册以加载相对 CSS/脚本/图片；单文件授权仅注册 HTML 本体，
  // 不能因预览把父目录整体注册为 URL 根。返回的仍是 token-gated canopy-file URL。
  ipcMain.handle(
    'file:prepare-html-preview',
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<{ url: string } | null> => {
      const { prepareHtmlPreview } = await import('./lib/file-preview-service')
      const options = normalizeFileAccessOptions(access)
      const resolved = resolveAuthorizedPreviewPath(filePath, options)
      if (!resolved) return null
      const directoryScope = canRegisterPreviewDirectory(resolved, getPreviewAccessScope(options))
      const result = await prepareHtmlPreview(resolved, undefined, { directoryScope })
      return result ? { url: result.url } : null
    }
  )


  // PSD 内联预览：合成图 + 图层树；可临时改写图层显隐（不改文件）。授权同预览家族。
  ipcMain.handle(
    'file:psd-preview',
    async (_, filePath: string, options?: import('@canopy/shared').PsdPreviewOptions, access?: FileAccessOptions | string[]): Promise<import('@canopy/shared').PsdPreviewResult | null> => {
      const { preparePsdPreview } = await import('./lib/psd-preview-service')
      const resolved = resolveAuthorizedPreviewPath(filePath, normalizeFileAccessOptions(access))
      if (!resolved) return null
      const safeOptions = options && typeof options === 'object'
        ? {
            hiddenLayerIds: Array.isArray(options.hiddenLayerIds) ? options.hiddenLayerIds.filter((id): id is string => typeof id === 'string') : undefined,
            visibleLayerIds: Array.isArray(options.visibleLayerIds) ? options.visibleLayerIds.filter((id): id is string => typeof id === 'string') : undefined,
            maxSide: typeof options.maxSide === 'number' ? options.maxSide : undefined,
          }
        : {}
      try {
        return await preparePsdPreview(resolved, safeOptions)
      } catch (error) {
        console.warn('[IPC] file:psd-preview 渲染失败:', resolved, error instanceof Error ? error.message : error)
        return null
      }
    }
  )
  ...
  ipcMain.handle(
    'file:office-to-html',
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@canopy/shared').OfficePreviewResult | null> => {
  ...
  // 预览用原始字节：大表格 / 旧版 .xls / 旧版 .ppt 由渲染层后台线程解析。
  // 授权与文本预览（resolveAndReadFile 送回全文）同一条链；只放行这三种格式，不做通用读文件口子。
  // 直接回二进制而不是 base64：50MB 的表格走 base64 会膨胀成 67MB 字符串，渲染层还得逐字节解码卡住页面。
  ipcMain.handle(
    'file:read-preview-binary',
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<Uint8Array | null> => {
      const { readFile, stat } = await import('node:fs/promises')
      const { extname } = await import('node:path')
      const options = normalizeFileAccessOptions(access)
      const resolved = resolveAuthorizedPreviewPath(filePath, options)
      if (!resolved || !PREVIEW_BINARY_EXTENSIONS.has(extname(resolved).toLowerCase())) return null
      const fileStat = await stat(resolved).catch(() => null)
      if (!fileStat?.isFile() || fileStat.size > PREVIEW_BINARY_MAX_BYTES) return null
      ...
      function scan(
        ...
        try {
          ...
          for (const item of items) {
            ...
            if (item.isDirectory() && (ignoreDirs.has(item.name) || isPythonEnvironmentDirName(item.name))) continue
            if (!item.isDirectory() && isTransientFileName(item.name)) continue
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').FeishuBotConfigInput) => {
  ...
  ipcMain.handle(
    ...
    async (_, appId: string, appSecret: string, domain?: import('@canopy/shared').FeishuDomain): Promise<FeishuTestResult> => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@canopy/shared').DingTalkBotConfigInput) => {
  ...
  // 附加目录/文件刻意不做启动清理：existsSync 在网络盘未挂载、外接盘未插、
  // OneDrive 按需同步未就绪、杀软瞬时占用、超长路径等情况下同样返回 false，
  // 据此删配置会静默丢掉用户手工添加的项目文件（实测复现）。改为与本地项目根
  // 一致的「不可用」状态机：读取时附加即时状态、UI 标灰并由用户手动移除。
  // 详见 getAttachedPathStatus()。
  ...
  ipcMain.handle(
    ...
    async (event, input?: VoiceDictationToggleInput): Promise<void> => {
      ...
      const sourceInputId = typeof input?.sourceInputId === 'string' && input.sourceInputId.length > 0 && input.sourceInputId.length <= 512
        ? input.sourceInputId
        : undefined
      toggleVoiceDictationWindow({ targetIsApp: !!sourceWindow, sourceInputId })
    }
  )
  ...
  ipcMain.handle('migration:open-data-folder', async (): Promise<void> => {
    ...
    if (error) throw new Error(`无法打开 ${CANOPY_BRAND.productName} 数据文件夹：${error}`)
  })
  ...
  ipcMain.handle(PLANNING_IPC_CHANNELS.RESOLVE_NATIVE_SYNC_CONFLICT, async (_, input: ResolvePlanningNativeSyncConflictInput): Promise<boolean> => {
    if (!input || typeof input.id !== 'string' || !['keep_app', 'keep_system'].includes(input.resolution)) throw new Error('冲突解决参数非法')
  ...
  // 否则 NaN / -Infinity / 越界值会污染 ~/.canopy/automations.json，无法回滚。
  ...
  ipcMain.handle(VAULT_IPC_CHANNELS.RESOLVE_MEDIA, async (_, noteRelativePath: unknown, src: unknown): Promise<ResolvedFileUrl | null> => {
    ...
    return resolvedPath ? { url: registerAppFilePath(resolvedPath), path: resolvedPath } : null
  })
