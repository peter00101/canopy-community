...
import { IPC_CHANNELS, CHANNEL_IPC_CHANNELS, CHAT_IPC_CHANNELS, AGENT_IPC_CHANNELS, ENVIRONMENT_IPC_CHANNELS, INSTALLER_IPC_CHANNELS, PROXY_IPC_CHANNELS, SYSTEM_PROMPT_IPC_CHANNELS, CHAT_TOOL_IPC_CHANNELS, FEISHU_IPC_CHANNELS, DINGTALK_IPC_CHANNELS, WECHAT_IPC_CHANNELS, AUTOMATION_IPC_CHANNELS, PLANNING_IPC_CHANNELS, VAULT_IPC_CHANNELS, AGENT_ISLAND_IPC_CHANNELS, TERMINAL_IPC_CHANNELS } from '@canopy/shared'
...
export interface ElectronAPI {
  ...
  /** 获取未暂存的变更文件列表 */
  getUnstagedChanges: (dirPath: string, sessionPath?: string, workspaceFilesPath?: string, extraPaths?: string[], sessionId?: string) => Promise<import('@canopy/shared').UnstagedChangesResult>
  /** 失效 Git Diff 扫描缓存；省略路径时失效全部仓库 */
  invalidateGitDiffCache: (changedPath?: string) => Promise<void>
  /** 获取单个文件的 diff */
  getFileDiff: (input: import('@canopy/shared').GetFileDiffInput) => Promise<string>
  /** 获取未追踪文件内容 */
  getUntrackedContent: (input: import('@canopy/shared').GetFileDiffInput) => Promise<string>
  /** 还原文件变更 */
  revertFile: (input: import('@canopy/shared').RevertFileInput) => Promise<void>
  /** 获取文件新旧版本内容 */
  getDiffContents: (input: import('@canopy/shared').GetFileDiffInput) => Promise<{ oldContent: string; newContent: string } | null>
  /** 列出 Git Worktree */
  listWorktrees: (repoPath: string, sessionId: string) => Promise<import('@canopy/shared').WorktreeInfo[]>
  /** 获取 Worktree 相对于基准分支的全量变更 */
  getWorktreeChanges: (worktreePath: string, baseBranch: string, sessionId: string) => Promise<import('@canopy/shared').UnstagedChangesResult>
  /** 在独立窗口打开当前文件预览 */
  ...
  openAgentBrowser: (sessionId: string) => Promise<import('@canopy/shared').BrowserViewState>
  listAgentBrowserTabs: (sessionId: string) => Promise<import('@canopy/shared').BrowserViewState>
  createAgentBrowserTab: (input: import('@canopy/shared').BrowserCreateTabInput) => Promise<import('@canopy/shared').BrowserViewState>
  selectAgentBrowserTab: (input: import('@canopy/shared').BrowserTabInput) => Promise<import('@canopy/shared').BrowserViewState>
  closeAgentBrowserTab: (input: import('@canopy/shared').BrowserTabInput) => Promise<import('@canopy/shared').BrowserViewState | null>
  getAgentBrowserState: (sessionId: string) => Promise<import('@canopy/shared').BrowserViewState | null>
  setAgentBrowserLayout: (layout: import('@canopy/shared').BrowserViewLayout) => Promise<void>
  /** 最小化受管浏览器：保留 WebContents 与页面状态，X 才是真正销毁（#1743） */
  minimizeAgentBrowser: (sessionId: string) => Promise<void>
  navigateAgentBrowser: (input: import('@canopy/shared').BrowserNavigateInput) => Promise<import('@canopy/shared').BrowserViewState>
  goBackAgentBrowser: (sessionId: string) => Promise<import('@canopy/shared').BrowserViewState>
  goForwardAgentBrowser: (sessionId: string) => Promise<import('@canopy/shared').BrowserViewState>
  reloadAgentBrowser: (sessionId: string) => Promise<import('@canopy/shared').BrowserViewState>
  closeAgentBrowser: (sessionId: string) => Promise<void>
  onAgentBrowserStateChanged: (callback: (state: import('@canopy/shared').BrowserStateChange) => void) => () => void
  /** 将当前显示标签页面导出为 PDF，落盘到受控下载目录 */
  exportAgentBrowserPdf: (input: import('@canopy/shared').BrowserTabInput) => Promise<import('@canopy/shared').BrowserDownloadItem>
  /** 在文件管理器中显示某条下载记录（仅限会话下载记录内路径），无 path 时打开下载目录 */
  showAgentBrowserDownload: (input: { sessionId: string; path?: string }) => Promise<void>
  onAgentBrowserTabFocused: (callback: (change: import('@canopy/shared').BrowserTabFocusChange) => void) => () => void
  ...
  /** Windows：把应用主题色应用到系统标题栏按钮（WCO）；其他平台空转 */
  /** 最大化/还原窗口 */
  ...
  codexOAuthLogin: (method?: import('@canopy/shared').CodexOAuthLoginMethod) => Promise<CodexOAuthLoginResult>

  /** 取消进行中的 ChatGPT (Codex) OAuth 登录 */
  codexOAuthCancel: () => Promise<void>

  /** 订阅登录期间，接收 Codex device code 与授权链接。返回取消订阅函数。 */
  onCodexOAuthDeviceCode: (callback: (deviceCode: import('@canopy/shared').CodexOAuthDeviceCode) => void) => () => void
  ...
  /** 解析附件绝对路径（供消息附件点击打开文件预览；失败返回 null） */
  resolveAttachmentPath: (localPath: string) => Promise<string | null>
  ...
  resolveVaultMedia: (noteRelativePath: string, src: string) => Promise<import('@canopy/shared').ResolvedFileUrl | null>
  ...
  /** 汇总全部会话的 token 用量（按模型 / 按会话）。只统计 token，不含金额。 */
  getAgentUsageStats: (range?: import('@canopy/kernel').UsageStatsRange) => Promise<import('@canopy/kernel').AgentUsageStats>
  ...
  getPiReasoningCapability: (channelId: string, modelId: string) => Promise<import('@canopy/shared').ReasoningCapability | undefined>
  ...
  /** 中止 Agent 执行；hadActiveRun=false 表示后端本就没有 run 在跑，调用方须自行结束本地运行态 */
  stopAgent: (sessionId: string) => Promise<import('@canopy/shared').AgentStopResult>
  /** 查询主进程当前仍在执行的会话 ID（运行态看门狗对账） */
  getActiveAgentSessionIds: () => Promise<string[]>
  ...
  saveWorkspaceMcpConfig: (workspaceSlug: string, config: WorkspaceMcpConfig, options?: import('@canopy/shared').SaveWorkspaceMcpConfigOptions) => Promise<void>
  ...
  setMcpEnabledAndValidate: (workspaceSlug: string, name: string, enabled: boolean) => Promise<import('@canopy/shared').McpConnectionMutationResult>

  /** 原子新增 MCP，并在初始启用时条件持久化验证结果。 */
  installMcpAndValidate: (workspaceSlug: string, name: string, entry: import('@canopy/shared').McpServerEntry) => Promise<import('@canopy/shared').McpInstallMutationResult>
  startMcpOAuth: (input: import('@canopy/shared').StartMcpOAuthInput) => Promise<import('@canopy/shared').McpOAuthStartResult>
  /** 将 OAuth client secret 加密保存到系统 Keychain；不会回传给渲染器或 Agent。 */
  saveMcpOAuthClientSecret: (input: import('@canopy/shared').SaveMcpOAuthClientSecretInput) => Promise<void>

  /** 将静态 MCP API Key / Token 加密保存到系统 Keychain。 */
  saveMcpApiKey: (input: import('@canopy/shared').SaveMcpApiKeyInput) => Promise<void>
  ...
  getCliIntegrationStatuses: (workspaceSlug: string) => Promise<import('@canopy/shared').CliIntegrationStatus[]>

  /** 仅切换 Canopy 对工作区 CLI 集成的使用权限；绝不登出或撤销第三方授权。 */
  setCliIntegrationEnabled: (workspaceSlug: string, id: string, enabled: boolean) => Promise<import('@canopy/shared').CliIntegrationStatus[]>

  /** 测试 MCP 服务器连接 */
  testMcpServer: (workspaceSlug: string, name: string, entry: import('@canopy/shared').McpServerEntry) => Promise<{ success: boolean; message: string }>
  ...
  /** 获取默认 Skills 的 slug 列表（来自 ~/.canopy/default-skills/） */
  ...
  /** 从其他工作区批量导入多个 Skill */
  batchImportSkillsFromWorkspaces: (targetSlug: string, selections: import('@canopy/shared').BulkImportWorkspaceSelection[]) => Promise<import('@canopy/shared').BulkImportSkillsResult>
  ...
  listSkillFiles: (workspaceSlug: string, skillSlug: string) => Promise<import('@canopy/shared').SkillFileNode[]>

  /** 读取 Skill 目录下的子文件内容 */
  readSkillFile: (workspaceSlug: string, skillSlug: string, relativePath: string) => Promise<import('@canopy/shared').SkillFileContent>
  ...
  /** 读取工作区 AGENTS.md */
  readWorkspaceAgentsMd: (workspaceSlug: string) => Promise<import('@canopy/shared').SkillFileContent>

  /** 写入工作区 AGENTS.md */
  writeWorkspaceAgentsMd: (workspaceSlug: string, content: string, expectedContent?: string) => Promise<void>

  /** 列出工作区长期记忆文件树 */
  listWorkspaceAutoMemoryFiles: (workspaceSlug: string) => Promise<import('@canopy/shared').SkillFileNode[]>

  /** 读取工作区长期记忆文件 */
  readWorkspaceAutoMemoryFile: (workspaceSlug: string, relativePath: string) => Promise<import('@canopy/shared').SkillFileContent>
  ...
  subscribeWorkspaceMemoryChanges: (
    ...
    callback: (change: import('@canopy/shared').WorkspaceMemoryFileChange) => void,
  ) => () => void
  ...
  updateSessionPermissionMode: (sessionId: string, mode: AppPermissionMode) => Promise<void>
  ...
  /** 响应 ExitPlanMode 请求 */
  respondExitPlanMode: (response: ExitPlanModeResponse) => Promise<ExitPlanModeRespondResult>
  ...
  /** 批量查询附加目录/文件的即时可用状态（只读，不修改配置） */
  getAttachedPathStatuses: (paths: string[]) => Promise<import('@canopy/shared').AttachedPathState[]>
  /** 获取工作区 worktree 仓库配置列表 */
  getWorktreeRepos: (workspaceSlug: string) => Promise<import('@canopy/shared').WorkspaceWorktreeRepo[]>
  /** 添加 worktree 仓库到工作区配置 */
  addWorktreeRepo: (workspaceSlug: string, repo: import('@canopy/shared').WorkspaceWorktreeRepo) => Promise<import('@canopy/shared').WorkspaceWorktreeRepo[]>
  /** 从工作区配置移除 worktree 仓库 */
  removeWorktreeRepo: (workspaceSlug: string, repoPath: string) => Promise<import('@canopy/shared').WorkspaceWorktreeRepo[]>
  ...
  /** 列出目录内容 */
  listDirectory: (dirPath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<FileEntry[]>

  /** 删除文件/目录 */
  deleteFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 用系统默认应用打开文件 */
  openFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 将剪贴板文本写入临时预览文件并返回绝对路径 */
  writeClipboardPreview: (filename: string, content: string) => Promise<string>

  /** 用系统默认应用打开任意文件（无工作区限制） */
  systemOpenFile: (filePath: string, appName?: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 扫描系统中可用的编辑器应用（仅 macOS） */
  scanEditors: () => Promise<import('@canopy/shared').EditorApp[]>

  /** 查询本机为该文件类型注册的默认打开应用（含图标 dataURL） */
  getDefaultAppForFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<import('@canopy/shared').DefaultAppInfo | null>

  /** 在系统文件管理器中显示文件 */
  showInFolder: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 使用系统终端打开文件夹 */
  openFolderInTerminal: (folderPath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /**
   * 在系统文件管理器中显示文件（无工作区限制，支持候选基础目录）。
   * fallbackToParentFolder：文件不存在时改为打开它所在的文件夹；都不在才返回 false。
   */
  showItemInFolder: (filePath: string, candidateBasePaths?: string[], options?: { fallbackToParentFolder?: boolean }) => Promise<boolean>

  /** 解析文件路径并读取内容（供内联预览使用） */
  resolveAndReadFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<import('@canopy/shared').FilePreviewReadResult | null>

  /** 批量检查文件是否仍存在（用于清理已删除的会话文件变更记录） */
  filterExistingFilePaths: (filePaths: string[], access?: import('@canopy/shared').FileAccessOptions) => Promise<string[]>

  /** 写入文本文件（供 Markdown 内联编辑使用） */
  writeTextFile: (filePath: string, content: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<boolean>

  /** 仅解析文件路径（供 PDF/图片等用 canopy-file:// 加载） */
  resolveFilePath: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<(import('@canopy/shared').ResolvedFileUrl & { resolvedPath?: string }) | null>

  /** 解析当前 Markdown 同目录内的相对媒体文件（仅供 LiveMarkdown 图片使用，上游 #1933 的收窄安全通道） */
  resolveMarkdownMedia: (markdownFilePath: string, src: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<import('@canopy/shared').ResolvedFileUrl | null>


  /** 为内联 PDF 预览生成临时 HTML 文件，返回文件路径 */
  preparePdfPreview: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<{ tmpHtmlUrl: string } | null>

  /** 为内联 HTML 预览生成受限 URL（iframe sandbox 加载） */
  prepareHtmlPreview: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<{ url: string } | null>

  /** 读取文件为 base64（带路径校验，供内联图片预览等） */
  readBinaryBase64: (filePath: string, access?: import('@canopy/shared').FileAccessOptions, maxSize?: number) => Promise<string | null>

  /** Office 文件转高保真 HTML（内联预览；失败时服务层降级到内置解析器） */
  officeToHtml: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<import('@canopy/shared').OfficePreviewResult | null>

  /** PSD 内联预览：合成图地址 + 图层树；options 可临时改写图层显隐（不改文件） */
  psdPreview: (filePath: string, options?: import('@canopy/shared').PsdPreviewOptions, access?: import('@canopy/shared').FileAccessOptions) => Promise<import('@canopy/shared').PsdPreviewResult | null>

  /** 预览用原始字节（仅 .xlsx / .xls / .ppt，走预览授权链，上限 50MB），由渲染层后台线程解析 */
  readPreviewBinary: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<Uint8Array | null>
  ...
  /** 重命名文件/目录 */
  renameFile: (filePath: string, newName: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 移动文件/目录到目标目录 */
  moveFile: (filePath: string, targetDir: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 列出附加目录内容 */
  listAttachedDirectory: (dirPath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<FileEntry[]>

  /** 读取附加目录文件内容为 base64（限制在已附加目录范围内） */
  readAttachedFile: (filePath: string, sessionId?: string, workspaceSlug?: string) => Promise<string>

  /** 在文件管理器中显示附加目录文件 */
  showAttachedInFolder: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 重命名附加目录文件/目录（无工作区路径限制） */
  renameAttachedFile: (filePath: string, newName: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>

  /** 移动附加目录文件/目录（无工作区路径限制） */
  moveAttachedFile: (filePath: string, targetDir: string, access?: import('@canopy/shared').FileAccessOptions) => Promise<void>
  ...
  /** 测试飞书连接 */
  testFeishuConnection: (appId: string, appSecret: string, domain?: import('@canopy/shared').FeishuDomain) => Promise<FeishuTestResult>
  /** 启动飞书 Bridge */
  ...
  /** 获取多 Bot 配置 */
  getFeishuMultiConfig: () => Promise<import('@canopy/shared').FeishuMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveFeishuBotConfig: (input: import('@canopy/shared').FeishuBotConfigInput) => Promise<import('@canopy/shared').FeishuBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getFeishuMultiStatus: () => Promise<import('@canopy/shared').FeishuMultiBridgeState>

  // --- 扫码注册 ---

  /** 启动扫码注册流程，等待用户扫码 + 飞书确认后返回 App ID/Secret */
  registerFeishuApp: () => Promise<import('@canopy/shared').FeishuRegisterAppResult>
  /** 取消正在进行的扫码注册流程 */
  cancelFeishuRegistration: () => Promise<void>
  /** 监听二维码 URL 生成 */
  onFeishuRegisterQrcode: (callback: (payload: import('@canopy/shared').FeishuRegisterAppQRCode) => void) => () => void
  /** 监听注册流程状态变化 */
  onFeishuRegisterStatus: (callback: (payload: import('@canopy/shared').FeishuRegisterAppStatus) => void) => () => void
  ...
  /** 获取多 Bot 配置 */
  getDingTalkMultiConfig: () => Promise<import('@canopy/shared').DingTalkMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveDingTalkBotConfig: (input: import('@canopy/shared').DingTalkBotConfigInput) => Promise<import('@canopy/shared').DingTalkBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getDingTalkMultiStatus: () => Promise<import('@canopy/shared').DingTalkMultiBridgeState>
  ...
  // ===== 教程（我方保留项：上游 v0.19.26 #1984 删除整条教程链路，我方不采）=====
...
const electronAPI: ElectronAPI = {
  ...
  getFileDiff: (input: import('@canopy/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_FILE_DIFF, input)
  },

  getUntrackedContent: (input: import('@canopy/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_UNTRACKED_CONTENT, input)
  },

  revertFile: (input: import('@canopy/shared').RevertFileInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.REVERT_FILE, input)
  },

  getDiffContents: (input: import('@canopy/shared').GetFileDiffInput) => {
  ...
  createAgentBrowserTab: (input: import('@canopy/shared').BrowserCreateTabInput) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.CREATE_BROWSER_TAB, input),
  selectAgentBrowserTab: (input: import('@canopy/shared').BrowserTabInput) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.SELECT_BROWSER_TAB, input),
  closeAgentBrowserTab: (input: import('@canopy/shared').BrowserTabInput) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.CLOSE_BROWSER_TAB, input),
  getAgentBrowserState: (sessionId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_BROWSER_STATE, sessionId)
  },
  setAgentBrowserLayout: (layout: import('@canopy/shared').BrowserViewLayout) => {
  ...
  navigateAgentBrowser: (input: import('@canopy/shared').BrowserNavigateInput) => {
  ...
  onAgentBrowserStateChanged: (callback: (state: import('@canopy/shared').BrowserStateChange) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, state: import('@canopy/shared').BrowserStateChange) => callback(state)
  ...
  exportAgentBrowserPdf: (input: import('@canopy/shared').BrowserTabInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.EXPORT_BROWSER_PDF, input)
  },
  showAgentBrowserDownload: (input: { sessionId: string; path?: string }) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_BROWSER_DOWNLOAD, input)
  },
  onAgentBrowserTabFocused: (callback: (change: import('@canopy/shared').BrowserTabFocusChange) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, change: import('@canopy/shared').BrowserTabFocusChange) => callback(change)
  ...
  codexOAuthLogin: (method?: import('@canopy/shared').CodexOAuthLoginMethod) => {
  ...
  onCodexOAuthDeviceCode: (callback: (deviceCode: import('@canopy/shared').CodexOAuthDeviceCode) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, deviceCode: import('@canopy/shared').CodexOAuthDeviceCode) => callback(deviceCode)
  ...
  resolveAttachmentPath: (localPath: string) => {
    return ipcRenderer.invoke(CHAT_IPC_CHANNELS.RESOLVE_ATTACHMENT_PATH, localPath) as Promise<string | null>
  },
  ...
  // 教程（我方保留项）
  getTutorialContent: () => {
    return ipcRenderer.invoke(CHAT_IPC_CHANNELS.GET_TUTORIAL_CONTENT) as Promise<string | null>
  },

  createWelcomeConversation: () => {
    return ipcRenderer.invoke(CHAT_IPC_CHANNELS.CREATE_WELCOME_CONVERSATION) as Promise<ConversationMeta | null>
  },
  ...
  getAgentUsageStats: (range?: import('@canopy/kernel').UsageStatsRange) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_USAGE_STATS, range)
  },
  ...
  getActiveAgentSessionIds: () => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_ACTIVE_SESSION_IDS)
  },
  ...
  saveWorkspaceMcpConfig: (workspaceSlug: string, config: WorkspaceMcpConfig, options?: import('@canopy/shared').SaveWorkspaceMcpConfigOptions) => {
  ...
  setMcpEnabledAndValidate: (workspaceSlug: string, name: string, enabled: boolean) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SET_MCP_ENABLED_AND_VALIDATE, workspaceSlug, name, enabled) as Promise<import('@canopy/shared').McpConnectionMutationResult>
  },

  installMcpAndValidate: (workspaceSlug: string, name: string, entry: import('@canopy/shared').McpServerEntry) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.INSTALL_MCP_AND_VALIDATE, workspaceSlug, name, entry) as Promise<import('@canopy/shared').McpInstallMutationResult>
  },

  startMcpOAuth: (input: import('@canopy/shared').StartMcpOAuthInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.START_MCP_OAUTH, input)
  },

  saveMcpOAuthClientSecret: (input: import('@canopy/shared').SaveMcpOAuthClientSecretInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SAVE_MCP_OAUTH_CLIENT_SECRET, input)
  },

  saveMcpApiKey: (input: import('@canopy/shared').SaveMcpApiKeyInput) => {
  ...
  getCliIntegrationStatuses: (workspaceSlug: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_CLI_INTEGRATION_STATUSES, workspaceSlug) as Promise<import('@canopy/shared').CliIntegrationStatus[]>
  },

  setCliIntegrationEnabled: (workspaceSlug: string, id: string, enabled: boolean) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SET_CLI_INTEGRATION_ENABLED, workspaceSlug, id, enabled) as Promise<import('@canopy/shared').CliIntegrationStatus[]>
  },

  testMcpServer: (workspaceSlug: string, name: string, entry: import('@canopy/shared').McpServerEntry) => {
  ...
  batchImportSkillsFromWorkspaces: (targetSlug: string, selections: import('@canopy/shared').BulkImportWorkspaceSelection[]) => {
    return ipcRenderer.invoke(
  ...
  subscribeWorkspaceMemoryChanges: (workspaceSlug: string, callback: (change: import('@canopy/shared').WorkspaceMemoryFileChange) => void) => {
    const listener = (_: unknown, payload: { workspaceSlug: string; change: import('@canopy/shared').WorkspaceMemoryFileChange }): void => {
  ...
  updateSessionPermissionMode: (sessionId: string, mode: AppPermissionMode) => {
  ...
  getAttachedPathStatuses: (paths: string[]) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_ATTACHED_PATH_STATUSES, paths)
  },
  ...
  addWorktreeRepo: (workspaceSlug: string, repo: import('@canopy/shared').WorkspaceWorktreeRepo) => {
  ...
  listDirectory: (dirPath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_DIRECTORY, dirPath, access)
  },

  deleteFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.DELETE_FILE, filePath, access)
  },

  openFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
  ...
  systemOpenFile: (filePath: string, appName?: string, access?: import('@canopy/shared').FileAccessOptions) => {
  ...
  getDefaultAppForFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_DEFAULT_APP_FOR_FILE, filePath, access) as Promise<import('@canopy/shared').DefaultAppInfo | null>
  },

  showInFolder: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_IN_FOLDER, filePath, access)
  },

  openFolderInTerminal: (folderPath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.OPEN_FOLDER_IN_TERMINAL, folderPath, access)
  },

  /** 在系统文件管理器中显示文件（无工作区限制，支持候选基础目录） */
  showItemInFolder: (filePath: string, candidateBasePaths?: string[], options?: { fallbackToParentFolder?: boolean }): Promise<boolean> => {
    return ipcRenderer.invoke(IPC_CHANNELS.SHOW_ITEM_IN_FOLDER, filePath, candidateBasePaths, options)
  },

  resolveAndReadFile: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-and-read', filePath, access) as Promise<import('@canopy/shared').FilePreviewReadResult | null>
  },

  filterExistingFilePaths: (filePaths: string[], access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:exists-batch', filePaths, access) as Promise<string[]>
  },

  writeTextFile: (filePath: string, content: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:write-text', filePath, content, access) as Promise<boolean>
  },

  resolveFilePath: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-path', filePath, access) as Promise<(import('@canopy/shared').ResolvedFileUrl & { resolvedPath?: string }) | null>
  },

  resolveMarkdownMedia: (markdownFilePath: string, src: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-markdown-media', markdownFilePath, src, access) as Promise<import('@canopy/shared').ResolvedFileUrl | null>
  },

  preparePdfPreview: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:prepare-pdf-preview', filePath, access) as Promise<{ tmpHtmlUrl: string } | null>
  },

  prepareHtmlPreview: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:prepare-html-preview', filePath, access) as Promise<{ url: string } | null>
  },

  readBinaryBase64: (filePath: string, access?: import('@canopy/shared').FileAccessOptions, maxSize?: number) => {
    return ipcRenderer.invoke('file:read-binary-base64', filePath, access, maxSize) as Promise<string | null>
  },

  officeToHtml: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:office-to-html', filePath, access) as Promise<import('@canopy/shared').OfficePreviewResult | null>
  },

  psdPreview: (filePath: string, options?: import('@canopy/shared').PsdPreviewOptions, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:psd-preview', filePath, options, access) as Promise<import('@canopy/shared').PsdPreviewResult | null>
  },

  readPreviewBinary: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:read-preview-binary', filePath, access) as Promise<Uint8Array | null>
  },
  ...
  renameFile: (filePath: string, newName: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_FILE, filePath, newName, access)
  },

  moveFile: (filePath: string, targetDir: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.MOVE_FILE, filePath, targetDir, access)
  },

  listAttachedDirectory: (dirPath: string, access?: import('@canopy/shared').FileAccessOptions) => {
  ...
  showAttachedInFolder: (filePath: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_ATTACHED_IN_FOLDER, filePath, access)
  },

  renameAttachedFile: (filePath: string, newName: string, access?: import('@canopy/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_ATTACHED_FILE, filePath, newName, access)
  },

  moveAttachedFile: (filePath: string, targetDir: string, access?: import('@canopy/shared').FileAccessOptions) => {
  ...
  testFeishuConnection: (appId: string, appSecret: string, domain?: import('@canopy/shared').FeishuDomain) => {
  ...
  saveFeishuBotConfig: (input: import('@canopy/shared').FeishuBotConfigInput) => {
  ...
  onFeishuRegisterQrcode: (callback: (payload: import('@canopy/shared').FeishuRegisterAppQRCode) => void) => {
    const listener = (_: unknown, payload: import('@canopy/shared').FeishuRegisterAppQRCode) => callback(payload)
  ...
  onFeishuRegisterStatus: (callback: (payload: import('@canopy/shared').FeishuRegisterAppStatus) => void) => {
    const listener = (_: unknown, payload: import('@canopy/shared').FeishuRegisterAppStatus) => callback(payload)
  ...
  saveDingTalkBotConfig: (input: import('@canopy/shared').DingTalkBotConfigInput) => {
