...
import type { BrowserSearchEngineId, EnvironmentCheckResult, ThinkingConfig, AgentEffort, AgentThinkingLevel, FeishuSessionMirrorSettings, TerminalProfile, WindowsShellPreference } from '@canopy/shared'
...
/** 语音输入输出方式 */
export type VoiceDictationOutputMode = 'auto' | 'clipboard' | 'app-input'
...
export interface VoiceDictationOutputContext {
  /** 本次听写是否写入 Canopy 内部输入框。 */
  routeToAppInput: boolean
  /** 会话开始时选择的输出模式。 */
  outputMode: VoiceDictationOutputMode
}

/** 主进程确认开始听写时，告知渲染进程本次输出是否应路由到 Canopy 输入框。 */
...
/** 将当前识别结果作为 Canopy 输入框中的临时组合文本预览。 */
export interface VoiceDictationPreviewInput {
  sessionId: string
  text: string
  /** 本次听写会话冻结的 Canopy 输入目标；null 表示不路由到内部输入框。 */
...
export interface VoiceDictationTextEvent {
  sessionId: string
  text: string
  /** 本次听写会话冻结的 Canopy 输入目标；null 表示交给全局 fallback 处理。 */
  targetInputId?: string | null
}
...
export interface VoiceDictationCommitResult {
  mode: 'app-input' | 'cursor' | 'clipboard'
  success: boolean
  message: string
}
...
/** 默认主题模式
 * 0.18.69 起新装默认走特殊风格（云杉夜色）。已经存过 themeMode 的用户读自己的值，不受影响；
 * 只有字段缺失的极老配置会回落到这里，而新旧默认同为深色，观感变化最小。 */
export const DEFAULT_THEME_MODE: ThemeMode = 'special'

/** 默认特殊风格 —— 新装用户的门面主题 */
export const DEFAULT_THEME_STYLE: ThemeStyle = 'spruce-dark'
...
/**
 * 无视觉输入能力模型的视觉助手路由（Chat 与 Agent 共用）。
...
export interface VisionRelaySettings {
  ...
  modelId?: string
  /**
   * 用户勾选「需要视觉补齐」的模型键列表，键格式 `${channelId}::${modelId}`
   * （见 buildVisionAugmentModelKey）。命中的模型发图时会先经视觉模型转成文字描述。
   */
  augmentModelKeys?: string[]
}

/** 视觉补齐勾选清单的模型键（channelId + modelId 组合，避免同名模型跨渠道误伤）。 */
export function buildVisionAugmentModelKey(channelId: string, modelId: string): string {
  return `${channelId}::${modelId}`
}

/**
 * 视频助手路由（Chat 与 Agent 共用）。
 *
 * 与视觉助手不同：没有任何对话模型能原生接收视频，因此视频附件一律经
 * 视频模型转成文字描述后注入，与当前对话模型无关。当前仅 Kimi API 渠道
 * （api.moonshot.cn，Anthropic 协议 video 块 + 文件上传）支持该能力。
 */
export interface VideoRelaySettings {
  enabled: boolean
  channelId?: string
  modelId?: string
}
...
/**
 * Moonshot 官方声明支持视频理解的模型清单
 * （来源：platform.kimi.com 视觉模型文档，2026-08 核对）。
 */
export const KIMI_VIDEO_CAPABLE_MODELS = [
  'kimi-k3',
  'kimi-k2.6',
  'kimi-k2.7-code',
...
/** 提示缓存保留时长：short = 5 分钟（默认），long = 1 小时（仅 Anthropic Messages 协议生效） */
...
/** 受管浏览器操作免确认的默认值：开启。缺省即视为开启，老配置无需迁移。 */
export const DEFAULT_BROWSER_AUTO_APPROVE = true
...
export interface AppSettings {
  ...
  /**
   * 上下文占用达到该比例时自动压缩（0.5 ~ 0.95，undefined = 默认 0.8）。
   * 越界值由 normalizePiAutoCompactionRatio 夹住，不会透传给 Pi。
   */
  agentAutoCompactionRatio?: number
  /**
   * 提示缓存保留时长：'short' = 5 分钟（默认），'long' = 1 小时。
   * 只对 Anthropic Messages 协议的模型生效（cache_control.ttl），其余协议忽略；见 runtime-pi 的 pi-cache-retention.ts。
   */
  agentPromptCacheRetention?: AgentPromptCacheRetention
  /** 自动归档天数（0 = 禁用，默认 7） */
  ...
  /**
   * Agent 改动文件时是否自动打开侧面板并切到「改动」标签（默认 true）。
   * 用户反馈「老是自己弹出来很烦」，关掉后改动仍会记录进「改动」标签，只是不再自动弹面板。
   * 上游 v0.19.1(#1868) 已彻底移除自动弹出；我方保留开关（开=自动弹，关=上游行为）。
   */
  agentAutoOpenChangesPanel?: boolean
  ...
  /** 输入框是否渲染 Markdown 富文本格式（默认 true，关闭后为纯文本模式，仍保留 Mention 引用） */
  ...
  /** 飞书 Session 镜像设置：每个 Canopy Session 可创建一个仅包含用户与指定 Bot 的飞书群 */
  feishuSessionMirror?: FeishuSessionMirrorSettings
  /** 无视觉输入能力 Agent 的视觉助手路由 */
  visionRelay?: VisionRelaySettings
  /** 视频附件转文字描述的视频助手路由（仅 Kimi 渠道支持） */
  videoRelay?: VideoRelaySettings
  /** 已确认的受管浏览器风险告知版本；低于当前版本时首次使用会再次要求确认。 */
  browserRiskDisclaimerVersion?: number
  /** 受管浏览器默认搜索引擎（新标签页首页 + 搜索词）；缺省按必应处理，Agent 与用户共用同一选择。 */
  browserSearchEngine?: BrowserSearchEngineId
  /**
   * 受管浏览器操作免确认，**默认开启**（维护者 2026-09-09 决策，推翻 08-17 的「BrowserClose 需批准」）。
   *
   * 25 个浏览器工具里此前只有两个会反复要确认，开启后这两个也直接放行：
   * - `BrowserClose`：关闭整个受管浏览器（会丢掉用户手开的标签与未保存页面状态）
   * - `BrowserUpload`：把本机文件选进网页的 file input（**外发动作，不可撤回**）
   *
   * 关掉此开关即恢复逐次确认。计划模式的只读限制与「自动任务 / 委派子 Agent 不得关闭浏览器」
   * 两条不受本开关影响——前者是 plan 语义，后者是无人值守边界，那种场景下没有用户在场可确认。
   */
  browserAutoApproveEnabled?: boolean
  /** 用户手动开启的 Canopy 内置能力 ID 列表（默认关闭的 Nano Banana）。 */
  builtinMcpEnabledIds?: string[]
  /** Todo、日程与 Obsidian 的入口可见性开关，默认全部开启。我方定制：仅控制侧栏/菜单显示，不收回 Agent 工具。 */
  productivityTools: ProductivityToolsSettings
  /** 启动时自动清理临时文件（app-preview、app-installers），默认 true */
  autoCleanupTempOnStart?: boolean
  /** 自动清理 N 天前已归档会话的 SDK 数据（0 = 禁用，默认 0） */
  autoCleanupArchivedDays?: number
  /**
   * Agent 代创建 git commit / PR 时是否附加 Canopy 推广标识。
   * 默认 true：commit trailer（可选）
   * 关闭后不注入任何 Canopy 归因，并覆盖 Claude SDK 默认 Co-Authored-By。
   */
