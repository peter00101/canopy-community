...
function main(): void {
  ...
  // 打印配置信息
  console.log(`\n${color.bgBlue}${color.bold} ${CANOPY_BRAND.productName} 打包工具 ${color.reset}\n`)
  ...
  // ── 步骤 5: 编译 canopy CLI 二进制 ──
  step++
  printStepStart(step, totalSteps, '编译 canopy CLI (bun --compile)')
  results.push(
    runStep('编译 canopy CLI', 'bun', ['run', 'build:cli'], { verbose: opts.verbose })
  )
  printStepResult(results[results.length - 1])
  if (!results[results.length - 1].success) return printSummary(results)
