...
import type { PlatformArch, BunDownloadInfo } from '@canopy/shared'
...
async function downloadFile(url: string, destPath: string): Promise<void> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': `${CANOPY_BRAND.productName}-Build-Script/1.0`,
    },
    redirect: 'follow',
  })
...
async function getBunVersion(): Promise<string> {
  ...
  const version = pkg.canopy?.bun?.version
  if (!version) {
    throw new Error('package.json 中未配置 canopy.bun.version')
  }
...
async function main(): Promise<void> {
  ...
  console.log(`${CANOPY_BRAND.productName} Bun 二进制下载脚本`)
  console.log('='.repeat(60))
