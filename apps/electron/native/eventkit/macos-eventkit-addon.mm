...
static NSURL *identityURL(NSString *entity, NSString *identity) { return identity ? [NSURL URLWithString:[NSString stringWithFormat:@"canopy://planning/%@/%@", entity, identity]] : nil; }
...
// 只回传本应用自己写入的严格 UUID marker；用户的任意 EventKit URL 不得进入 JS 层。
// 写入统一用 canopy://；读取同时认历史的 proma://——品牌重构前写进用户系统日历/提醒事项的
// 存量条目带的是旧 scheme，只认新前缀会让它们被判成「日程已不存在」。旧前缀永远不要删。
static NSString *canopyPlanningIdentity(NSString *entity, EKCalendarItem *item) {
  NSString *absolute = item.URL.absoluteString;
  if (!absolute) return nil;
  for (NSString *scheme in @[@"canopy", @"proma"]) {
    NSString *prefix = [NSString stringWithFormat:@"%@://planning/%@/", scheme, entity];
...
static NSDictionary *itemDTO(NSString *entity, EKCalendarItem *item) {
  ...
  NSString *identity = canopyPlanningIdentity(entity, item); if (identity) result[@"planningIdentity"] = identity;
...
static void upsert(NSString *entity, NSDictionary *payload, CommandContext *ctx, NSArray<EKReminder *> *recoveredReminders) {
  ...
  NSError *error = nil;
  if ([entity isEqualToString:@"calendar"]) {
    EKEvent *event = identifier ? (EKEvent *)[eventStore() calendarItemWithIdentifier:identifier] : nil; if (event && ![event.calendar.calendarIdentifier isEqualToString:target.calendarIdentifier]) { ctx->reject(@"系统项目已移出连接集合"); return; } if (!event && identifier && ![number(payload, @"allowRecreate") boolValue]) { ctx->reject(@"系统日程已不存在；请选择保留 Canopy 后再重建"); return; } if (!event) event = (EKEvent *)recovered; if (!event) event = [EKEvent eventWithEventStore:eventStore()];
  ...
  EKReminder *reminder = identifier ? (EKReminder *)[eventStore() calendarItemWithIdentifier:identifier] : nil; if (reminder && ![reminder.calendar.calendarIdentifier isEqualToString:target.calendarIdentifier]) { ctx->reject(@"系统项目已移出连接集合"); return; } if (!reminder && identifier && ![number(payload, @"allowRecreate") boolValue]) { ctx->reject(@"系统提醒事项已不存在；请选择保留 Canopy 后再重建"); return; } if (!reminder) reminder = (EKReminder *)recovered; if (!reminder) reminder = [EKReminder reminderWithEventStore:eventStore()];
...
static void execute(NSString *command, NSString *entity, NSDictionary *payload, CommandContext *ctx) {
  @autoreleasepool {
    ...
    NSLog(@"[CanopyEventKit] executing %@ for %@ (status=%@)", command, entity, statusName([EKEventStore authorizationStatusForEntityType:typeFor(entity)]));
    ...
    if ([command isEqualToString:@"requestAccess"]) {
      ...
      NSLog(@"[CanopyEventKit] requesting TCC with NSApp=%@ active=%d", NSApp, NSApp.isActive);
...
static Napi::Value subscribeChanges(const Napi::CallbackInfo &info) {
  ...
  eventStoreChangeListener = std::make_unique<Napi::ThreadSafeFunction>(Napi::ThreadSafeFunction::New(env, info[0].As<Napi::Function>(), "CanopyEventKitChanges", 1, 1));
...
static Napi::Value command(const Napi::CallbackInfo &info) {
  ...
  auto *ctx = new CommandContext(env); NSLog(@"[CanopyEventKit] scheduling %@ for %@ on main queue", name, entity); dispatch_async(dispatch_get_main_queue(), ^{ execute(name, entity, payload, ctx); }); return ctx->deferred->Promise();
}
Napi::Object Init(Napi::Env env, Napi::Object exports) { NSLog(@"[CanopyEventKit] N-API addon loaded"); env.AddCleanupHook(cleanupChanges); exports.Set("command", Napi::Function::New(env, command)); exports.Set("subscribeChanges", Napi::Function::New(env, subscribeChanges)); return exports; }
NODE_API_MODULE(canopy_eventkit, Init)
