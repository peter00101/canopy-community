...
import type { XaiOAuthCredentials, XaiOAuthDeviceCode } from '@canopy/shared'
 ...
 */
export async function loginXaiOAuth(deps: PiOAuthNetworkDeps, callbacks?: XaiLoginCallbacks): Promise<XaiOAuthCredentials> {
      ...
      const credentials = await runtime.login('xai', 'oauth', {
        ...
        notify: (event) => {
          if (event.type === 'device_code') {
            ...
            deps.openExternal(event.verificationUri).catch((error) => {
...
export async function refreshXaiOAuth(getProxyUrl: () => Promise<string | undefined>, refreshToken: string): Promise<XaiOAuthCredentials> {
  const sdk = await loadPiSdk()
