...
import type { SDKAssistantMessage } from '@canopy/shared'
import {
  convertPiMessage,
...
function writeToolCall(content: string): AssistantMessage {
  return {
    ...
    content: [{
      ...
      name: 'write',
      arguments: {
        path: 'C:\\Users\\WNI10\\.canopy\\agent-workspaces\\moneybull\\workspace-files\\large.md',
        content,
      },
    }],
...
describe('convertPiMessage', () => {
  test('keeps complete write input in the final tool-call frame', () => {
    ...
    expect(message.message.content[0]?.input).toEqual({
      path: 'C:\\Users\\WNI10\\.canopy\\agent-workspaces\\moneybull\\workspace-files\\large.md',
      file_path: 'C:\\Users\\WNI10\\.canopy\\agent-workspaces\\moneybull\\workspace-files\\large.md',
      content,
    })
