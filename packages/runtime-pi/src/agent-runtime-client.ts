...
export class AgentRuntimeClient {
  ...
  async start(): Promise<AgentRuntimeState> {
    ...
    try {
      ...
      // 启动过程中被 stop：那不是崩溃，别把状态标成 crashed 让用户看到红色错误态
  ...
  private async spawnAndHandshake(): Promise<AgentRuntimeState> {
    ...
    // Windows 上 fork 会同步抛瞬时 ENOTCONN，退避重试；退避期间若被 stop /
    // 新一代启动抢占，startUtilityProcessWithRetry 抛显式取消错误（收上游 #2074）
    const shouldContinueStartup = (): boolean =>
      generation === this.generation
      && this.stopPromise === undefined
      && this.state.status === 'starting'
    const runtimeProcess = await startUtilityProcessWithRetry(
      () => utilityProcess.fork(this.entryPath, [], {
        serviceName: `${CANOPY_BRAND.productName} Runtime`,
        env: { ...process.env, ...this.env, CANOPY_AGENT_SESSION_ID: this.sessionId },
      }),
    ...
    const transfer: AgentRuntimePortTransfer = {
      type: 'canopy-agent-runtime-port',
  ...
  private async handleIncomingRequest(request: AgentRuntimeRequest): Promise<void> {
    // 能力 handler 可能在 stop / restart 之后才 resolve：把身份在入口处钉死，
    // 绝不把旧 runtime 的回包投到替换后的 port / bootId 上（收上游 #2074）
    const port = this.port
