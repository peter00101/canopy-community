 ...
 * Pi Agent SDK 适配器
 *
 * Canopy 内部继续使用 SDKMessage 兼容协议，避免渲染层、Jotai 状态、
...
import type { CanUseToolOptions, PermissionResult } from '@canopy/kernel'
import { isPromptTooLongError, normalizeToolResultDetails, serializeToolResultPayload } from '@canopy/kernel'
...
import {
  createAppManagedResourceLoaderOptions,
  createAppProjectInstructionFilesOverride,
  type AppProjectInstructionFile,
} from './pi-resource-loader-overrides'
import { ProjectInstructionScopeController } from './pi-project-instruction-scope'
import type { ProjectInstructionSource } from '@canopy/kernel'
...
import { mergeRuntimeEnv, type AgentRuntimeEnv } from '@canopy/kernel'
...
import { withDefaultBashTimeout } from './pi-bash-timeout'
...
import { buildRoutingIdentity, createRoutingEpochStore, injectRoutingIdentity, isCountableFailure } from './channel-routing-identity'
import { applyPiCacheRetention, resolvePiCacheRetention, type AgentPromptCacheRetention } from './pi-cache-retention'
import {
...
/**
 * Pi 原生 retry 设置（收上游 #2081）。Pi 0.86 起单次 agent 退避被 maxAgentDelayMs 封顶（默认 60 秒），
 * 次数照旧的话预算会从 0.85 的 255 秒缩到 183 秒；抬到 14 次保住长任务扛网关短时故障的能力：
 * 1 + 2 + 4 + 8 + 16 + 32 + 60 × 8 = 543 秒。确定性错误（无效 Key / 额度 / 计费）不在可重试串里，仍快速失败。
 */
export const PI_NATIVE_RETRY_SETTINGS = {
  enabled: true,
  maxRetries: 14,
  baseDelayMs: 1_000,
...
/**
 * Pi 0.86 新增的缓存预热会在工具长时间运行时额外发请求续命缓存（每次按整段上下文的缓存读取计费）。
 * 用户付费的中转渠道上不能静默多花钱，在提供显式开关前一律关闭（与上游 #2081 一致）。
 */
export const PI_CACHE_WARMING_MODE = 'off' as const
const MAX_AUTOMATIC_COMPACTION_CONTINUATIONS = 20
/** per-session 路由代数：渠道类错误后 +1，让 CCH 类中转网关把后续请求当新会话重新选路（详见 channel-routing-identity.ts）。 */
const routingEpochStore = createRoutingEpochStore()
...
export interface PiAgentQueryOptions extends AgentQueryInput {
  ...
  /** OAuth credential coordination key; equals the selected Canopy channel id. */
  channelId?: string
  channelName?: string
  maxTurns?: number
  ...
  systemPrompt: string
  /** Canopy 已验证的项目根 instruction files；不触发 Pi 的磁盘自动发现。 */
  projectInstructionFiles?: AppProjectInstructionFile[]
  ...
  /** Canopy 聚合的附加目录；Pi 内置工具 factory 不接收多 root 参数，编排层会把它们注入 systemPrompt。 */
  ...
  /** 上下文占用达到该比例时自动压缩；undefined = 默认 0.8，越界值会被夹到 0.5~0.95 */
  autoCompactionRatio?: number
  /** 提示缓存保留时长；'long' 只对 Anthropic Messages 协议注入 1 小时 TTL（见 pi-cache-retention.ts） */
  promptCacheRetention?: AgentPromptCacheRetention
  ...
  /** Pi 运行中刷新 OAuth 后，将新凭据回写到 Canopy 渠道存储。 */
  ...
  /** Pi 运行中刷新 xAI OAuth 后，将新凭据回写到 Canopy 渠道存储。 */
...
function isAppSkillPath(path: string | undefined, allowedRoots: string[]): boolean {
...
function createAppSkillsOverride(additionalSkillPaths: string[] | undefined): (base: SkillLoadResult) => SkillLoadResult {
  const allowedRoots = buildAllowedSkillRoots(additionalSkillPaths)
  return (base) => ({
    skills: base.skills.filter((skill) =>
      isAppSkillPath(skill.filePath, allowedRoots) || isAppSkillPath(skill.baseDir, allowedRoots)),
    diagnostics: base.diagnostics.filter((diagnostic) => isAppSkillPath(diagnostic.path, allowedRoots)),
  })
}
...
async function preparePromptWithAppSkills(
  resourceLoader: ResourceLoader,
  prompt: string,
...
// Pi 0.86 把工具结果 details 收紧为 JSON 值：文本与 details 一律经内核的 JSON 安全化（收上游 #2081）。
...
function createTextToolResult(text: string, details?: unknown): AgentToolResult<unknown> {
  return {
    ...
    ...(details === undefined ? {} : { details: normalizeToolResultDetails(details) }),
  } as AgentToolResult<unknown>
}
...
/**
 * Pi 的 post-run 自动压缩（threshold / overflow-完成态）压缩完不会调 agent.continue()——
 * 上游 `_runAutoCompaction` 仅在 overflow willRetry 场景返回 true，threshold 一律不续。
 * 当模型只是提前 stop（长任务中途汇报后停下）而非真正完成时，任务会在「上下文已整理」
 * 后原地停摆（维护者报障「压缩上下文后不继续」的根因）。判定这类压缩，交由 runPromptChain
 * 注入续跑提示（复用 CompactContext 工具的续跑链与 20 次上限）。
 *
 * - manual 排除：手动 /compact 与 CompactContext 工具（走 session.compact()，reason 同为
 *   manual）各有语义——前者不该自动续跑，后者已有自己的续跑安排，不能双重注入。
 * - willRetry 排除：overflow 中断恢复由 Pi 自己 agent.continue()。
 * - afterCompletedTurn=false 排除：轮前压缩（prepareNextTurnWithContext 钩子）压缩后
 *   agent loop 会带新上下文继续本轮，无需外部续跑。
 */
export function shouldResumeAfterAutoCompaction(
  event: {
...
/** 零总结收尾自动续跑的上限：连续两轮都不产出面向用户的总结就放弃（防模型异常时空转）。 */
const MAX_FINAL_OUTPUT_CONTINUATIONS = 2

/**
 * 零总结续跑提示：先判断是不是在等用户，再决定复述、总结还是继续做。
 * 模型常把给用户的话和最后一次工具调用放在同一条消息里，工具跑完再回一条空消息，
 * 这时「最后一条没以文字收尾」会被当成零总结。旧提示只有「完成就总结 / 没完成就继续」两档，
 * 模型已经在问用户「口吻合适吗」也会被催着接着干活（用户反馈「回复完又自己跑一轮」）。
 * 这条提示不上屏，所以开头要讲明它不是用户的新消息，免得模型当成用户的回答或批准。
 */
export const PI_FINAL_OUTPUT_CONTINUATION_PROMPT = `<app_final_output_continuation>
这是系统自动发出的检查，不是用户发来的新消息。你上一轮结束时，最后一条输出不是给用户看的文字（为空，或停在思考、工具调用上）。按顺序判断：
- 若你在本轮已经向用户提问、请用户确认或选择，正在等用户回答：用一两句话把问题再说一遍就结束。不要调用任何工具，不要开始新的操作。
- 若原始任务已完成：用一段话向用户总结做了什么、结果是什么，不要再调用工具。
- 若任务确实还没完成，而且不需要等用户回答：继续执行剩余步骤，全部完成后给出总结。
- 不要重复已经完成或已提交的操作。
</app_final_output_continuation>`

/** 一条 assistant 消息是否以非空 text 收尾（Pi 侧 content block 形态，toolCall/thinking 结尾即零总结）。 */
export function assistantEndsWithTrailingText(content: Array<{ type: string; text?: string }>): boolean {
  const last = content[content.length - 1]
  return !!last && last.type === 'text' && typeof last.text === 'string' && last.text.trim().length > 0
}

/**
 * run 以 success 收尾但模型没给面向用户的总结（提前 stop / 拖流被掐后按正常收尾）时，
 * 自动注入续跑让模型把话说完——用户要的是结果，不是一行「请你自己发继续」的提示
 * （2026-08-31 维护者纠偏：提示行会让用户分心补操作、没看到就耽误任务，降级为极端兜底）。
 */
export function planPiFinalOutputContinuation(options: {
  resultSubtype: string | undefined
  lastAssistantEndedWithText: boolean
  continuationCount: number
  abortRequested: boolean
  runtimeLimitReached: boolean
}): { shouldContinue: true; prompt: string } | { shouldContinue: false } {
  if (options.abortRequested || options.runtimeLimitReached) return { shouldContinue: false }
  if (options.resultSubtype !== 'success') return { shouldContinue: false }
  if (options.lastAssistantEndedWithText) return { shouldContinue: false }
  if (options.continuationCount >= MAX_FINAL_OUTPUT_CONTINUATIONS) return { shouldContinue: false }
  return { shouldContinue: true, prompt: PI_FINAL_OUTPUT_CONTINUATION_PROMPT }
}
...
/**
 * TaskCreate 的任务 ID（收上游 #2076）：默认用 Pi 的 toolCallId，让流式阶段的 tool_use 与
 * TaskCreate 结果从一开始就按同一个键关联，渲染层不必等结果到了才换键。显式 id 只为旧调用格式保留。
 *
 * 上游没兜的两种情况我方退回自增编号：部分 OpenAI 兼容网关不下发工具调用 id（Pi 原样保留为空串），
 * 或每轮都从 call_0 重新编号——直接用会让多个任务挤进同一个键互相覆盖。
 */
export function resolveAppTaskId(
  input: Record<string, unknown>,
  toolCallId: string,
  isTaken: (id: string) => boolean,
  nextFallbackId: () => string,
): string {
  const explicitId = stringFromInput(input, ['id', 'taskId', 'task_id'])
  if (explicitId) return explicitId
  const callId = typeof toolCallId === 'string' ? toolCallId.trim() : ''
  if (callId && !isTaken(callId)) return callId
  return nextFallbackId()
}

function normalizeTaskStatus(value: unknown, fallback: AppTaskItem['status']): AppTaskItem['status'] {
  if (
    value === 'pending' ||
    value === 'in_progress' ||
    value === 'completed' ||
...
/** 导出仅供测试：包入口 `index.ts` 只做具名导出，不会漏到包的公开 API。 */
export function buildAppProductToolDefinitions(sdk: PiSdk, canUseTool: PiAgentQueryOptions['canUseTool']): ToolDefinition[] {
  const tasks = new Map<string, AppTaskItem>()
  let nextTaskId = 1
  const nextFallbackTaskId = (): string => {
    let id = String(nextTaskId++)
    while (tasks.has(id)) id = String(nextTaskId++)
    return id
  }
  ...
  const definitions = [
    sdk.defineTool({
      name: 'EnterPlanMode',
      label: '进入计划模式',
      description: `进入 ${CANOPY_BRAND.productName} 计划模式。进入后只能调研、整理计划；将完整计划写入会话 plan/ 目录，并等待用户批准后再执行写操作。`,
    ...
    sdk.defineTool({
      name: 'ExitPlanMode',
      label: '提交计划审批',
      description: '向用户提交计划并请求批准。必须先把完整计划写入当前会话 plan/ 目录的 Markdown 文件，再把该文件绝对路径传入 planFile，用户会在右侧只读预览它；用户批准后才能退出计划模式并继续执行。',
      promptSnippet: '提交计划审批：附上已写入 plan/ 目录的 Markdown 计划文件（planFile），等待用户批准后继续执行。',
      parameters: Type.Object({
        plan: Type.Optional(Type.String({ description: '计划正文或摘要。' })),
        // schema 层故意不设 required：Pi 的参数校验先于主进程 canUseTool，若在这里设必填，
        // 漏传时模型只会收到 Pi 的泛化校验错误；放到主进程拒绝，文案才能带上真实 plan/ 目录路径与重提步骤。
        planFile: Type.Optional(Type.String({ description: '必填：当前会话 plan/ 目录内、待审批的 Markdown 计划文件绝对路径。缺失或不在该目录内会被拒绝。' })),
        ...
        userApproved: Type.Optional(Type.Boolean({ description: '仅当用户在最近一条计划反馈中明确表示同意/批准（如「执行」「可以」「删了吧」）时设为 true：计划将直接获批执行，不再弹出审批横幅。用户反馈是修改意见或态度不明时绝不可设置。' })),
      }),
      async execute(_toolCallId, params) {
    ...
    sdk.defineTool({
      ...
      label: '询问用户',
      description: `当需要用户选择、补充信息或确认偏好时调用，${CANOPY_BRAND.productName} 会展示可交互问答横幅。`,
    ...
    sdk.defineTool({
      ...
      async execute(toolCallId, params) {
        const input = params as Record<string, unknown>
        const id = resolveAppTaskId(input, toolCallId, (candidate) => tasks.has(candidate), nextFallbackTaskId)
        const task: AppTaskItem = {
          id,
...
function buildWslCommand(command: string, env: NodeJS.ProcessEnv | undefined): string {
  ...
  for (const key of WSL_EXPORT_ENV_KEYS) {
    ...
    const value = key === 'CANOPY_CLI' ? windowsPathToWslPath(rawValue) : rawValue
...
function createAppBashToolOptions(runtimeEnv: AgentRuntimeEnv | undefined): BashToolOptions | undefined {
...
export function isPiBashToolAvailable(
  ...
  // Pi 的 Windows Bash 工具只能通过 Canopy 配置的 Git Bash 或 WSL 执行。
  // 没有可用 Bash Shell 时不暴露必然失败的 Bash 工具；Pi 0.84.4 起 Windows 会回落到原生 PowerShell 工具（见 selectPiBuiltinShellTool）。
...
function createAppPowerShellToolOptions(runtimeEnv: AgentRuntimeEnv | undefined): PowerShellToolOptions | undefined {
  if (!runtimeEnv) return undefined
  return {
...
/**
 * 去掉 Pi 内置工具自带的严格 JSON 采样声明（我方分叉，收上游 #2081 时定）。
 *
 * Pi 0.86 默认给 read / bash / powershell / edit / write 声明 `strict: "prefer"`，OpenAI 兼容协议下除少数几家外
 * 一律发 `strict: true` 与改写过的 schema（可选参数变成必填的 `anyOf[原类型, null]`）。0.85 时这只在实验开关下启用，
 * 我方用户大量走智谱 / 通义 / 豆包 / 自建网关这类 OpenAI 兼容渠道，对严格 schema 的支持参差不齐、无从逐家实测，
 * 一旦某家拒绝，读写文件这些最基础的工具会整体 400。保持 0.85 的请求形状（`strict: false` + 原 schema）。
 */
function withoutConstrainedSampling<T extends object>(tool: T): T {
  if (!('constrainedSampling' in tool)) return tool
  const { constrainedSampling: _constrainedSampling, ...rest } = tool as T & { constrainedSampling?: unknown }
  return rest as T
}
...
export function buildBuiltinToolDefinitions(
  ...
  const definitions = [
    ...
    ...(shellTool === 'bash'
      // 模型没传 timeout 时注入默认 10 分钟（第三层）：Pi 原生 bash 默认无超时，
      // 会让 Agent 跟着一条永不返回的命令一起卡死；超时后模型拿到明确结果自行决策。
      ? [withDefaultBashTimeout(sdk.createBashToolDefinition(cwd, createAppBashToolOptions(runtimeEnv)))]
      : []),
    ...(shellTool === 'powershell'
      ? [sdk.createPowerShellToolDefinition(cwd, createAppPowerShellToolOptions(runtimeEnv))]
      : []),
  ...
  return definitions.map((tool) =>
    wrapToolWithPermission(withoutConstrainedSampling(tool) as unknown as ToolDefinition<TSchema, unknown, unknown>, { canUseTool }) as ToolDefinition)
}
...
<runtime_capabilities>
当前 Windows 设备未配置 Git Bash 或 WSL，因此 Bash 工具不可用。你仍可使用 Read、Write、Edit、Grep、Find、Ls 及 ${CANOPY_BRAND.productName} 提供的其他工具完成任务；不要声称已运行命令、测试或 Git 操作。若任务确实需要命令行，请默认调用 InstallWindowsShell 帮助用户安装 Git Bash；该工具会要求用户确认下载并打开官方安装程序。
</runtime_capabilities>`
}
...
export function installRuntimeGuardHooks(session: AgentSession, guard: AgentRuntimeGuard): void {
  ...
  session.agent.afterToolCall = async (context, signal) => {
    ...
    return {
      ...previousResult,
      content: sanitizedContent,
      details: normalizeToolResultDetails(resultAfterPreviousHooks.details),
  ...
  session.agent.prepareNextTurnWithContext = async (context, signal) => {
    ...
    if (guard.shouldStopBeforeNextTurn()) {
      // Pi 的 steer/follow-up 队列在 turn 完成后才 drain；达到 Canopy 上限时必须在这里清空，
...
export class PiAgentAdapter implements AgentProviderAdapter {
  ...
  async *query(input: PiAgentQueryOptions): AsyncIterable<SDKMessage> {
    ...
    try {
      ...
      // post-run 自动压缩续跑判定（shouldResumeAfterAutoCompaction）：
      // compaction_start 缓存本次压缩是否发生在已完成 turn 之后；compaction_end 据此置续跑标志；
      // 其后任何完整 assistant 输出（= Pi 已自行继续）都会清掉它。
      let activeCompactionAfterCompletedTurn = false
      let autoCompactionResumePending = false
      // 零总结收尾自动续跑（planPiFinalOutputContinuation）：跟踪最后一条完整 assistant
      // 是否以非空 text 收尾；run success 收尾但没有总结时自动补一轮让模型把话说完。
      let lastAssistantEndedWithText = false
      let finalOutputContinuations = 0
      let pendingFinalOutputContinuation: string | undefined
      const customTools = [
        ...
        ...buildAppProductToolDefinitions(sdk, input.canUseTool),
      ...
      const settingsManager = sdk.SettingsManager.inMemory({
        ...
        // - 自动压缩在上下文达到设定占用比例（默认 80%）时触发；Pi 以 reserveTokens 表示预留空间。
        ...
        retry: { ...PI_NATIVE_RETRY_SETTINGS },
        cacheWarming: PI_CACHE_WARMING_MODE,
      ...
      const resourceLoader = new sdk.DefaultResourceLoader({
        ...
        settingsManager,
        ...createAppManagedResourceLoaderOptions(),
        agentsFilesOverride: createAppProjectInstructionFilesOverride(input.projectInstructionFiles ?? []),
        additionalSkillPaths: input.additionalSkillPaths ?? [],
        skillsOverride: createAppSkillsOverride(input.additionalSkillPaths),
      ...
      // 按访问路径激活的项目指令由 projectInstructionScope 扩展在每次调用模型前的 context 事件里注入
      // （Pi 0.86 的 AgentContext 已没有 systemPrompt 字段），见 pi-project-instruction-scope.ts。
      ...
      // 路由身份注入在每次调用时读取当前 epoch——渠道类错误推进 epoch 后，Pi native retry 的
      // 下一发与用户手动重试都自动携带新身份，CCH 类网关据此重新选路。
      const providerStreamFn = session.agent.streamFunction
      // 提示缓存保留时长同样按请求注入：用户在弹层里改了设置，下一次模型调用即生效。
      session.agent.streamFunction = (requestModel, context, options) => runWithPiRequestProxy(
        requestProxyDispatcher,
        () => providerStreamFn(requestModel, context, injectRoutingIdentity(
          applyPiCacheRetention(
            options as Parameters<typeof injectRoutingIdentity>[0],
            resolvePiCacheRetention({ api: requestModel.api, preference: input.promptCacheRetention }),
          ),
          buildRoutingIdentity(input.sessionId, routingEpochStore.get(input.sessionId)),
        ) as typeof options),
      )
        ...
        try {
          switch (event.type) {
            ...
            case 'message_end': {
              ...
              // 压缩后又有完整 assistant 输出 = Pi 已自行继续（轮前压缩 / 队列派发），无需外部续跑。
              if (isAssistant) autoCompactionResumePending = false
              ...
              // 路由身份轮换：任何失败连续满阈值（3 次）→ 换身份，成功清零——判定只看
              // stopReason 结构化字段，零文案耦合。前 2 次原地重试保 prompt cache（偶发 529/5xx
              // 抖动第二发就通）；真死路第 3 次失败即换，native retry 下一发就带新身份重新选路。
              if (isAssistant && (event.message as AssistantMessage).stopReason === 'error') {
                const errText = (event.message as AssistantMessage).errorMessage
                if (isCountableFailure(errText)) {
                  const record = routingEpochStore.recordFailure(input.sessionId)
                  if (record.advanced) {
                    console.warn(`[Pi] 连续失败达阈值，路由代数推进至 r${record.epoch}（会话 ${input.sessionId}）：${(errText ?? '').slice(0, 160)}`)
                  }
                }
              } else if (isAssistant) {
                routingEpochStore.recordSuccess(input.sessionId)
              }
              if (shouldDeferAssistantTerminal && converted?.type === 'assistant' && assistantUuid) {
                // Native retry 会丢弃该失败 assistant；不应消耗 Canopy 的 turn/budget 配额。
                ...
                if (isAssistant && assistantUuid) {
                  lastAssistantEndedWithText = assistantEndsWithTrailingText(
                    (event.message as AssistantMessage).content as Array<{ type: string; text?: string }>,
                  )
            ...
            case 'compaction_start': {
              ...
              completedAgentTurnPendingCompaction = false
              activeCompactionAfterCompletedTurn = afterCompletedTurn
            ...
            case 'compaction_end':
              autoCompactionResumePending = shouldResumeAfterAutoCompaction(
                {
                  reason: event.reason,
                ...
                activeCompactionAfterCompletedTurn,
              )
              activeCompactionAfterCompletedTurn = false
          ...
          while (nextPrompt !== undefined) {
            ...
            try {
              preparedPrompt = promptInput.skipSkillExpansion
                ...
                : await preparePromptWithAppSkills(
                  resourceLoader,
                  promptInput.content,
            ...
            try {
              ...
              if (autoCompactionResumePending) {
                // post-run 自动压缩（threshold / overflow-完成态）后 Pi 不会自行继续；
                // 若模型只是提前 stop，任务会在「上下文已整理」后停摆。注入续跑提示，
                // 复用 CompactContext 的续跑链：模型自查后要么继续原任务、要么复述完成结论。
                ...
                if (continuation.shouldContinue) {
                  automaticCompactionContinuations += 1
                  pendingCompactionContinuation = appendOutputFormatInstruction(continuation.prompt, input.outputFormat)
                  // 压缩挂起的成功终态只说明压缩前那轮 stop 了，任务未确认完成，不上抛。
                  pendingTerminalResult = undefined
                } else if (continuation.reason === 'continuation_limit') {
                  pendingTerminalResult = createCompactionContinuationLimitResult(session.sessionId)
                }
              }
              if (pendingCompactionContinuation === undefined) {
                // run success 收尾但模型没给面向用户的总结（停在思考/工具上）：自动补一轮
                // 让它把话说完，用户直接拿结果，而不是靠 UI 提示让用户自己发「继续」。
                const finalOutput = planPiFinalOutputContinuation({
                  resultSubtype: (pendingTerminalResult as { subtype?: string } | undefined)?.subtype,
                  lastAssistantEndedWithText,
                  continuationCount: finalOutputContinuations,
                  abortRequested: active.abortRequested,
                  runtimeLimitReached: runtimeGuard.shouldStopBeforeNextTurn(),
                })
                if (finalOutput.shouldContinue) {
                  finalOutputContinuations += 1
                  console.log(`[Pi] 零总结收尾，自动续跑补总结（第 ${finalOutputContinuations} 次）: ${session.sessionId}`)
                  pendingFinalOutputContinuation = appendOutputFormatInstruction(finalOutput.prompt, input.outputFormat)
                  pendingTerminalResult = undefined
                }
              }
            ...
            } else if (pendingFinalOutputContinuation) {
              nextPrompt = { content: pendingFinalOutputContinuation, skipSkillExpansion: true }
              pendingFinalOutputContinuation = undefined
            }
          }
        }
  ...
  async setPermissionMode(_sessionId: string, _mode: string): Promise<void> {
    // Canopy 权限由工具包装层实时读取 sessionPermissionModes，自身无需同步给 Pi。
  }
