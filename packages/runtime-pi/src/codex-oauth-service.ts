...
import type { CodexOAuthCredentials, CodexOAuthDeviceCode, CodexOAuthLoginMethod } from '@canopy/shared'
import { runWithOAuthProxyScope, type PiOAuthNetworkDeps } from './oauth-proxy-scope'
...
/**
 * Pi 的 ModelRuntime 只要求 CredentialStore 的结构契约。OAuth 凭据仍由 Canopy
 ...
 * Canopy 的代理 fetch 重新接回该流程；本 service 只负责与公开 ModelRuntime 交互。
 */
 ...
 */
export async function loginCodexOAuth(deps: PiOAuthNetworkDeps, options?: CodexLoginOptions): Promise<CodexOAuthCredentials> {
      ...
      const credentials = await runtime.login('openai-codex', 'oauth', {
        ...
        notify: (event) => {
          if (event.type === 'auth_url') {
            ...
            deps.openExternal(event.url).catch((err) => console.error('[Codex OAuth] 打开浏览器失败:', err))
            ...
            deps.openExternal(event.verificationUri).catch((err) => console.error('[Codex OAuth] 打开设备授权页面失败:', err))
 ...
 */
export async function refreshCodexOAuth(getProxyUrl: () => Promise<string | undefined>, refreshToken: string): Promise<CodexOAuthCredentials> {
  const sdk = await loadPiSdk()
