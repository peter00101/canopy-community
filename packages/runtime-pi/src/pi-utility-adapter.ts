 ...
 * Only serializable query data crosses the boundary; Canopy callbacks return
 * through capability RPC so business ownership stays in the main process.
 */
export class PiUtilityAdapter implements AgentProviderAdapter {
  ...
  async abort(sessionId: string): Promise<void> {
    ...
    for (const pending of this.pendingQueries.values()) {
      ...
      aborts.push(pending.client.call<QueryAbortResponse>(
        ...
        // QUERY_ABORT 返回即代表 utility 侧的 query 已结束：在这里关掉它的进程，
        // 让删除路径的 stop-and-drain 栅栏真的等到进程退出，
        // 而不是依赖异步迭代器稍后的 finally（收上游 #2074）
