...
import { resolveReasoningProfile } from '@canopy/shared'
...
describe('Pi Codex request settings', () => {
  ...
  test('Given GPT-6 Astra with thinking disabled When injecting Then sends low instead of the rejected none', () => {
    // 官方：Astra 不支持 none；旧会话残留的 off 必须落到 low，而不是沿用 5.x 的 off→none。
    expect(injectOpenAIReasoningLevel({ model: 'gpt-6-astra' }, { thinkingLevel: 'off' })).toEqual({
      model: 'gpt-6-astra',
      reasoning: { effort: 'low' },
    })
    expect(injectOpenAIReasoningLevel({ model: 'gpt-6-astra' }, { thinkingLevel: 'minimal' })).toEqual({
      model: 'gpt-6-astra',
      reasoning: { effort: 'low' },
    })
  })

  test('Given GPT-6 Astra xhigh / max thinking When injecting Then keeps the extended efforts verbatim', () => {
    expect(injectOpenAIReasoningLevel({ model: 'gpt-6-astra' }, { thinkingLevel: 'xhigh' })).toEqual({
      model: 'gpt-6-astra',
      reasoning: { effort: 'xhigh' },
    })
    expect(injectOpenAIReasoningLevel({ model: 'gpt-6-astra' }, { thinkingLevel: 'max' })).toEqual({
      model: 'gpt-6-astra',
      reasoning: { effort: 'max' },
    })
  })
