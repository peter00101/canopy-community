 ...
 * Agent runtime 行为守卫。
 *
 * 这层只表达 Canopy 自己承诺的语义，不绑定某个 SDK 的入口调用方式：
 * - 最大轮次：在 turn 完成后、下一轮 prompt / steer / follow-up 前停止。
 * - 最大预算：在 runtime 返回已知费用后，停止后续工具批次、prompt 与队列消息。
 * - 结构化输出：Pi 当前没有 provider-native schema response format，Canopy 用提示词约束
 *   加最终结果校验补齐兼容行为。
 */

import type { AgentMessage, AgentToolResult } from '@earendil-works/pi-agent-core'
import type { JsonSchemaOutputFormat, SDKResultMessage } from '@canopy/shared'
...
function extractLastAssistantText(messages: AgentMessage[]): string {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    // 避免 TypeScript 把联合类型提前收窄为 toolResult（收上游 #2081）。
