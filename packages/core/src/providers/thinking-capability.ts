 ...
 */
import { resolveReasoningProfile, type ProviderType } from '@canopy/shared'
...
export interface ThinkingCapability {
  ...
  effort?: string
  /**
   * 模型**不能关闭思考**时的最低强度（如 GLM-5.3：档位表里没有 off，端点收到
   * `thinking:{type:'disabled'}` 或不带思考参数的请求都会 400）。
   * 设置了它，「关闭思考」的对话请求与标题生成都改发 adaptive + 这个 effort，而不是 disabled / 省略。
   */
  alwaysOnEffort?: string
}
...
export function detectThinkingCapability(
  ...
  if (encoding?.kind === 'adaptive-effort') {
    ...
    // 档位表里没有 off = 该模型始终思考；取最低档的 effort 作为「想关也关不掉」时的替代强度。
    const lowestLevel = profile?.levels.includes('off') ? undefined : profile?.levels[0]
    const alwaysOnEffort = lowestLevel ? encoding.effortMap[lowestLevel] : undefined
    return {
      mode: 'adaptive-preferred',
      disableStrategy: 'explicit-disabled',
      ...(typeof effort === 'string' ? { effort } : {}),
      ...(typeof alwaysOnEffort === 'string' ? { alwaysOnEffort } : {}),
    }
  }
