...
describe('normalizeAnthropicProviderUrl', () => {
  ...
  test('ark-coding-plan 补全 /v1', () => {
    expect(normalizeAnthropicProviderUrl('https://ark.cn-beijing.volces.com/api/coding', 'ark-coding-plan')).toBe(
...
describe('resolveAnthropicMessagesUrl', () => {
  ...
  test('火山方舟 Coding Plan 协议根地址补全 /v1/messages', () => {
    expect(resolveAnthropicMessagesUrl('https://ark.cn-beijing.volces.com/api/coding', 'ark-coding-plan')).toBe(
...
describe('resolveAnthropicModelsUrl', () => {
  ...
  test('火山方舟 Coding Plan 协议根地址推导 /v1/models', () => {
    expect(resolveAnthropicModelsUrl('https://ark.cn-beijing.volces.com/api/coding', 'ark-coding-plan')).toBe(
...
describe('migrateCompatibleChannelBaseUrl', () => {
  ...
  // 0.18.67 维护者报障的真实输入：中转站渠道只填了裸域名（无任何路径）。
  // 补全后主对话与自动命名两条路才一致——SDK 路径会剥掉 /v1/messages 还原成根地址，
  // 标题/Chat 路径则原样使用完整端点。
  test('anthropic-compatible: 裸域名（无路径）补全为 /v1/messages', () => {
    expect(migrateCompatibleChannelBaseUrl('https://us.example.com', 'anthropic-compatible')).toBe(
      'https://us.example.com/v1/messages',
    )
  })

  test('anthropic-compatible: 裸域名带尾斜杠同样补全，不产生双斜杠', () => {
    expect(migrateCompatibleChannelBaseUrl('https://us.example.com/', 'anthropic-compatible')).toBe(
      'https://us.example.com/v1/messages',
    )
  })

  // custom 只补 /chat/completions、**不补 /v1** —— 这是刻意的：迁移的契约是「复现旧版
  // OpenAIAdapter 实际请求过的 URL」（normalizeBaseUrl(base) + '/chat/completions'），
  // 擅自补 /v1 会改掉存量渠道的行为。填裸域名的 custom 用户仍需自己写全路径。
  test('custom: 裸域名只补 /chat/completions，不额外补 /v1（保持与旧版一致）', () => {
    expect(migrateCompatibleChannelBaseUrl('https://us.example.com', 'custom')).toBe(
      'https://us.example.com/chat/completions',
    )
  })
