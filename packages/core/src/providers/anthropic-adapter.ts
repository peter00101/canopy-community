 ...
 * - 必须发送自有 User-Agent（服务端白名单校验）
 * - UA 格式：`Canopy/<version>`（取自 `CANOPY_BRAND.userAgentProduct`）
 */

import { extractZhipuCodingTeamApiToken, type ProviderType } from '@canopy/shared'
import type {
  ProviderAdapter,
  ProviderRequest,
  StreamRequestInput,
...
export class AnthropicAdapter implements ProviderAdapter {
   ...
   * - User-Agent 使用 Canopy 自有标识（通过 setAppVersion 初始化）
   */
  ...
  buildStreamRequest(input: StreamRequestInput): ProviderRequest {
    ...
    // 始终思考的模型（GLM-5.3）：用户关掉思考时不能发 disabled 也不能省略字段（端点都会 400），
    // 改按最低强度开着——请求形态与「开启」完全一致，只是 effort 更低。
    const forcedEffort = !input.thinkingEnabled ? capability.alwaysOnEffort : undefined
    const thinkingActive = Boolean(input.thinkingEnabled) || forcedEffort !== undefined
    ...
    if (capability.mode === 'effort-based-max') {
      ...
      if (capability.mode === 'adaptive-only' || capability.mode === 'adaptive-preferred') {
        ...
        const effort = forcedEffort ?? capability.effort
        if (effort) {
    ...
    // 调试：开启 CANOPY_DEBUG_REQUEST 时打印请求体，便于排查思考+工具场景的消息结构
    const procReq = (globalThis as { process?: { env?: Record<string, string | undefined> } }).process
    if (procReq?.env?.CANOPY_DEBUG_REQUEST) {
  ...
  parseSSELine(jsonLine: string): StreamEvent[] {
    try {
      ...
      // 调试：开启 CANOPY_DEBUG_SSE 时打印原始事件，便于排查 Provider 的 SSE 格式差异
      const proc = (globalThis as { process?: { env?: Record<string, string | undefined> } }).process
      if (proc?.env?.CANOPY_DEBUG_SSE) {
  ...
  buildTitleRequest(input: TitleRequestInput): ProviderRequest {
    ...
    // - 始终思考的模型（GLM-5.3）：关不掉，按最低强度开着；思考会占输出预算，上限放到 512 保证标题正文能出来
    //   （与 openai-adapter 标题预算同款取值；max_tokens 是上限不是消耗）
    // - Mythos Preview 不接受 disabled，省略字段即可
    // - 其它 Claude 显式 disabled（对 manual / adaptive 模型都有效）
    if (capability.alwaysOnEffort) {
      body.max_tokens = 512
      body.thinking = { type: 'adaptive', display: 'summarized' }
      body.output_config = { effort: capability.alwaysOnEffort }
    } else if (capability.disableStrategy === 'explicit-disabled') {
      body.thinking = { type: 'disabled' }
    }
