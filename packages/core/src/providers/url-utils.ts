...
/**
 * 规范化 Anthropic Base URL（用于 Canopy Chat 直接调用 API）
 *
 ...
 *
 * ⚠️ custom 的提前返回是刻意契约：用户填什么就请求什么。中转站/网关的端点路径未必是
 * 标准 `/chat/completions`，解析期静默追加后缀会把本来能用的渠道改坏。旧 Base URL 的
 * 补全由 `migrateCompatibleChannelBaseUrl` 在迁移期一次性负责，不要挪到这里。
 * 上游 af28c536 把这段注释掉以「增加容错性」，我方不采（该改动同时会让本文件
 * 测试里「custom 不向自定义 Chat 请求地址追加后缀」一例失败）。
 */
