...
export function saveImageToSession(
  ...
  const sessionDir = requireImBridgeBusinessDeps().paths.getAgentSessionWorkspacePath(workspaceSlug, sessionId)
 ...
 * 构建会话目录的文件树文本行
 *
 * 递归遍历 ~/.canopy/agent-workspaces/{slug}/{sessionId}/（只读，不创建目录）。
 *
...
export function buildSessionFileTree(
  ...
  const sessionDir = requireImBridgeBusinessDeps().paths.resolveAgentSessionWorkspacePath(workspaceSlug, sessionId)
