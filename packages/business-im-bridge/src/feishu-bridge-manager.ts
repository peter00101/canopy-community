...
class FeishuBridgeManager {
  ...
  setSessionMirrorOperator(botId: string, operatorOpenId: string | undefined): void {
    ...
    try {
      writeJsonFileAtomic(requireImBridgeBusinessDeps().paths.getFeishuBotMetadataPath(botId), { lastInteractedUserOpenId: userOpenId })
    } catch (error) {
  ...
  /** 为桌面端 Canopy Session 创建或恢复飞书镜像群。 */
  async ensureSessionMirror(session: AgentSessionMeta): Promise<void> {
    const config = getFeishuMultiBotConfig()
    const bot = resolveSessionMirrorBot(requireImBridgeBusinessDeps().getSettings().feishuSessionMirror, config.bots)
    if (!bot) return
  ...
  async startSessionMirrorRun(session: AgentSessionMeta): Promise<void> {
    const mirrorSettings = requireImBridgeBusinessDeps().getSettings().feishuSessionMirror
  ...
  removeBindingsForDeletedWorkspace(workspaceId: string, sessionIds: readonly string[]): number {
    ...
    for (const bot of getFeishuMultiBotConfig().bots) {
      ...
      const bindingsPath = requireImBridgeBusinessDeps().paths.getFeishuBotBindingsPath(bot.id)
