...
import type { AgentStreamPayload } from '@canopy/shared'
...
export class BridgeCommandHandler {
  ...
  isSessionActive(chatId: string): boolean {
    ...
    return requireImBridgeBusinessDeps().agentRunner.isAgentSessionActive(binding.sessionId)
  }
  ...
  private resolveValidWorkspaceId(fallbackWorkspaceId?: string): string {
    ...
    if (requestedWorkspaceId && requireImBridgeBusinessDeps().workspaces.getAgentWorkspace(requestedWorkspaceId)) return requestedWorkspaceId
    return requireImBridgeBusinessDeps().workspaces.listAgentWorkspacesByUpdatedAt()[0]?.id ?? ''
  }
  ...
  private isBindingValid(binding: BridgeChatBinding): boolean {
    ...
    return !binding.workspaceId || Boolean(requireImBridgeBusinessDeps().workspaces.getAgentWorkspace(binding.workspaceId))
  }
  ...
  private async createNewSession(chatId: string, title?: string, contextData?: unknown): Promise<void> {
    ...
    const channelId = settings.agentChannelId
    if (!channelId) {
      await this.send(chatId, `请先在 ${CANOPY_BRAND.productName} 设置中选择 Agent 渠道。`, contextData)
      return
    }
    ...
    // /new 没给标题时同样落到默认标题（上游仍写死「新会话」，同一类缺陷，我方一并修）：
    // 「新会话」≠ 自动命名的触发条件，会话会永远叫「新会话」。
    const session = createAgentSession(
  ...
  private async handleModelCommand(chatId: string, arg: string, contextData?: unknown): Promise<void> {
    ...
    if (!binding) {
      ...
      if (!binding) {
        await this.send(chatId, `请先发送一条消息创建会话，或在 ${CANOPY_BRAND.productName} 设置中选择 Agent 渠道。`, contextData)
        return
      }
    }
  ...
  private async handleUserMessage(
    ...
    if (requireImBridgeBusinessDeps().agentRunner.isAgentSessionActive(binding.sessionId)) {
    ...
    if (!binding) {
      await this.send(chatId, `当前项目已不可用，请在 ${CANOPY_BRAND.productName} 中重新选择项目后再试。`, contextData)
      return
    }
    ...
    requireImBridgeBusinessDeps().agentRunner.runAgentHeadless(input, {
      onError: (error) => {
  ...
  private notifySessionCreated(sessionId: string, title: string): void {
    requireImBridgeBusinessDeps().window.sendToMainWindow(AGENT_IPC_CHANNELS.TITLE_UPDATED, { sessionId, title })
  }
}
