 ...
 * - 发送消息/输入状态
 *
 * 消息路由到 Canopy Agent，回复通过 iLink API 发送。
 */
...
import { WECHAT_IPC_CHANNELS, WECHAT_ITEM_TYPE, WECHAT_MESSAGE_TYPE, WECHAT_MESSAGE_STATE } from '@canopy/shared'
...
class WeChatBridge {
   ...
   *
   * 延迟到首次访问才构造：`wechatBridge` 是模块加载时立即创建的单例
   * （见文件末尾 `export const wechatBridge = new WeChatBridge()`），
   * 而 `BridgeCommandHandler` 的构造过程需要读取 `requireImBridgeBusinessDeps()`
   * ——这一步必须晚于 electron 侧 `configureImBridgeBusiness(...)` 调用，
   * 不能在模块求值阶段就执行，否则会在 `@canopy/business-im-bridge` 包
   * 自身求值期间（早于调用方装配依赖）抛错。
   */
  private _commandHandler: BridgeCommandHandler | null = null
  private get commandHandler(): BridgeCommandHandler {
    if (!this._commandHandler) {
      this._commandHandler = new BridgeCommandHandler({
        platformName: '微信',
        adapter: {
        ...
        bindingStore: createJsonBridgeChatBindingStore(requireImBridgeBusinessDeps().paths.getWeChatBindingsPath(), '微信 Bridge'),
  ...
  private async handleMessage(msg: WeChatIncomingMessage): Promise<void> {
    ...
    if (!binding) {
      await this.client.sendText(chatId, `请先在 ${CANOPY_BRAND.productName} 设置中选择 Agent 渠道。`, contextToken)
      return
    }
  ...
  private loadSyncBuf(): void {
    const syncPath = requireImBridgeBusinessDeps().paths.getWeChatSyncPath()
  ...
  private updateStatus(partial: Partial<WeChatBridgeState>): void {
    ...
    requireImBridgeBusinessDeps().window.broadcastToAllWindows(WECHAT_IPC_CHANNELS.STATUS_CHANGED, this.state)
  }
}
