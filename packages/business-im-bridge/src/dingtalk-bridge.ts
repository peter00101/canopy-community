 ...
 * - 消息路由到 Canopy Agent，通过 sessionWebhook 回复
 *
...
import { DINGTALK_IPC_CHANNELS } from '@canopy/shared'
...
class DingTalkBridge {
  ...
  constructor(botConfig: DingTalkBotConfig) {
    ...
    this.commandHandler = new BridgeCommandHandler({
      ...
      bindingStore: createJsonBridgeChatBindingStore(
        requireImBridgeBusinessDeps().paths.getDingTalkBotBindingsPath(botConfig.id),
  ...
  private async processRobotMessage(msg: DWClientDownStream): Promise<void> {
    ...
    const preCheckWorkspaceId = this.botConfig.defaultWorkspaceId ?? requireImBridgeBusinessDeps().getSettings().agentWorkspaceId ?? ''
    if (!preCheckWorkspaceId || !requireImBridgeBusinessDeps().workspaces.getAgentWorkspace(preCheckWorkspaceId)) {
    ...
    if (!binding) {
      await this.replyTextViaWebhook(data.sessionWebhook, `请先在 ${CANOPY_BRAND.productName} 设置中选择 Agent 渠道。`)
      return
    }
    const workspace = requireImBridgeBusinessDeps().workspaces.getAgentWorkspace(binding.workspaceId)
    if (!workspace) {
      await this.replyTextViaWebhook(data.sessionWebhook, '⚠️ 当前未设置项目，无法保存图片')
      return
    }
  ...
  private updateStatus(partial: Partial<DingTalkBridgeState>): void {
    ...
    requireImBridgeBusinessDeps().window.broadcastToAllWindows(DINGTALK_IPC_CHANNELS.STATUS_CHANGED, botState)
  }
}
