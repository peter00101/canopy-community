...
export function buildAgentReplyCard(result: FormattedAgentResult, subtitle?: string): Record<string, unknown> {
  ...
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: `${CANOPY_BRAND.productName} Agent` },
...
export function buildErrorCard(errorMessage: string): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: `${CANOPY_BRAND.productName} 错误` },
      template: 'red',
    },
    elements: [
...
export function buildHelpCard(): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: `${CANOPY_BRAND.productName} Bot 命令` },
      template: 'blue',
    },
    elements: [
      {
...
function truncateForFeishu(text: string, maxLength = 25000): string {
  ...
  return text.slice(0, maxLength) + `\n\n... [内容过长，请在 ${CANOPY_BRAND.productName} 中查看完整回复]`
}
