 ...
 * 数据持久化到 ~/.canopy/dingtalk.json（v2 格式：{ version: 2, bots: [...] }）。
 *
...
function encryptSecret(plainSecret: string): string {
  const safeStorage = requireImBridgeBusinessDeps().safeStorage
...
function migrateDingTalkBindingFile(oldBotId: string, nextBotId: string): void {
  ...
  const oldPath = requireImBridgeBusinessDeps().paths.getDingTalkBotBindingsPath(oldBotId)
  if (!existsSync(oldPath)) return

  const nextPath = requireImBridgeBusinessDeps().paths.getDingTalkBotBindingsPath(nextBotId)
  try {
...
function readRawConfig(): DingTalkMultiBotConfig {
  const configPath = requireImBridgeBusinessDeps().paths.getDingTalkConfigPath()
