...
import { FEISHU_IPC_CHANNELS, AGENT_IPC_CHANNELS } from '@canopy/shared'
import { getDecryptedBotAppSecret } from './feishu-config'
import { createAgentSession, listAgentSessions, getAgentSessionMeta } from '@canopy/kernel'
import { readJsonFileSafe, writeJsonFileAtomic } from '@canopy/kernel'
import { requireImBridgeBusinessDeps } from './im-bridge-business-deps'
...
class FeishuBridge {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // 注册 EventBus 监听器
      this.eventBusUnsubscribe = requireImBridgeBusinessDeps().agentRunner.onAgentEvent((sessionId, payload) => {
  ...
  stop(): void {
    ...
    // 重启后仍需用来给桌面 Session 镜像建群。完整重置请删除 ~/.canopy/feishu-metadata-{botId}.json。
    this.botOpenId = null
  ...
  private loadBindings(): void {
    const bindingsPath = requireImBridgeBusinessDeps().paths.getFeishuBotBindingsPath(this.botConfig.id)
    ...
    try {
      ...
      const appSettings = requireImBridgeBusinessDeps().getSettings()
  ...
  private loadMetadata(): void {
    const metaPath = requireImBridgeBusinessDeps().paths.getFeishuBotMetadataPath(this.botConfig.id)
  ...
  private saveMetadata(): void {
    try {
      writeJsonFileAtomic(requireImBridgeBusinessDeps().paths.getFeishuBotMetadataPath(this.botConfig.id), {
  ...
  /**
   * 为 Canopy 桌面端会话准备飞书镜像群。
   *
   * 该群只包含用户与当前 Bot。用户在群里继续发送消息时，会通过
   * source=session-mirror 的绑定回到同一个 Canopy session。
   */
  ...
  private async createNewSession(
    ...
    if (!workspaceId) {
      const byTime = requireImBridgeBusinessDeps().workspaces.listAgentWorkspacesByUpdatedAt()
      const def = byTime.find((w) => w.slug === 'default')
      workspaceId = def?.id ?? byTime[0]?.id
    }

    if (!workspaceId) {
      await this.sendMessage(chatId, `请先在 ${CANOPY_BRAND.productName} 设置中创建项目。`)
      return
    }
    ...
    if (!channelId) {
      await this.sendMessage(chatId, `请先在 ${CANOPY_BRAND.productName} Agent 设置中选择渠道。`)
      return
    }
    ...
    requireImBridgeBusinessDeps().window.sendToMainWindow(AGENT_IPC_CHANNELS.TITLE_UPDATED, {
      sessionId: session.id,
      title: session.title,
    })
  ...
  private async handleListCommand(msgCtx: FeishuMessageContext): Promise<void> {
    ...
    const workspaces = requireImBridgeBusinessDeps().workspaces.listAgentWorkspacesByUpdatedAt()
    const binding = this.chatBindings.get(chatId)
  ...
  private async handleStopCommand(msgCtx: FeishuMessageContext): Promise<void> {
    ...
    requireImBridgeBusinessDeps().agentRunner.stopAgent(binding.sessionId)
  ...
  private async handleNowCommand(msgCtx: FeishuMessageContext): Promise<void> {
    ...
    if (binding) {
      ...
      const nowSettings = requireImBridgeBusinessDeps().getSettings()
    ...
    const workspace = workspaceId ? requireImBridgeBusinessDeps().workspaces.getAgentWorkspace(workspaceId) : undefined
    if (workspace) {
      lines.push(`**项目**: ${workspace.name} (\`${workspace.slug}\`)`)

      // MCP Servers
      const capabilities = requireImBridgeBusinessDeps().workspaces.getWorkspaceCapabilities(workspace.slug)
      ...
      const projectRoot = requireImBridgeBusinessDeps().workspaces.getProjectFilesPath(workspace.slug)
      try {
  ...
  private async handleModelCommand(msgCtx: FeishuMessageContext, arg: string): Promise<void> {
    ...
    if (channels.length === 0) {
      await this.sendMessage(
        chatId,
        `暂无可用渠道。请先在 ${CANOPY_BRAND.productName} 设置中配置并启用渠道（需填入 API Key 且至少启用一个模型）。`,
      )
      return
    }
    ...
    if (!targetBinding) {
      ...
      if (!targetBinding) {
        await this.sendMessage(chatId, `请先发送一条消息创建会话，或在 ${CANOPY_BRAND.productName} 设置中选择 Agent 渠道。`)
        return
      }
    }
  ...
  private async handleUserMessage(
    ...
    if (!session?.workspaceId) {
      ...
      await this.sendCardMessage(chatId, buildErrorCard(`当前会话项目不可用，请在 ${CANOPY_BRAND.productName} 中重新选择会话。`))
      return
    }
    ...
    const workspace = requireImBridgeBusinessDeps().workspaces.getAgentWorkspace(session.workspaceId)
    ...
    const latestSettings = requireImBridgeBusinessDeps().getSettings()
    ...
    try {
      await requireImBridgeBusinessDeps().agentRunner.runAgentHeadless(input, {
        source: 'feishu',
        onError: (error) => {
      ...
      }, piCustomTools ? { customTools: piCustomTools } : undefined)
    } catch (error) {
  ...
  private handleAgentPayload(sessionId: string, payload: AgentStreamPayload): void {
    ...
    if (payload.kind === 'app_event' && payload.event.type === 'title_updated') {
   ...
   */
  private buildPiFeishuChatHistoryTool(chatId: string): KernelToolDefinition {
    return {
      ...
      execute: async (_toolCallId, args): Promise<KernelToolResult<unknown>> => {
  ...
  private resolveContextPrefix(chatId: string): string {
    ...
    const workspace = binding.workspaceId ? requireImBridgeBusinessDeps().workspaces.getAgentWorkspace(binding.workspaceId) : undefined
    const session = getAgentSessionMeta(binding.sessionId)
  ...
  private updateStatus(partial: Partial<FeishuBridgeState>): void {
    ...
    requireImBridgeBusinessDeps().window.sendToMainWindow(FEISHU_IPC_CHANNELS.STATUS_CHANGED, {
      ...this.status,
      botId: this.botConfig.id,
