...
import type { Channel, ChannelModel } from '@canopy/shared'
...
export function listSwitchableChannels(): Channel[] {
  return requireImBridgeBusinessDeps().channels.listChannels().filter((c) => c.enabled && getEnabledModels(c).length > 0)
}
...
export function describeBindingModel(
  ...
  const channel = channelId ? requireImBridgeBusinessDeps().channels.getChannelById(channelId) : undefined
