...
/** 已授权本地文件的 canopy-file URL */
export interface ResolvedFileUrl {
  url: string
  /**
   * 主进程实际解析到的绝对路径。相对引用（尤其裸文件名）可能经候选目录搜索命中子目录或
   * 长期记忆目录，渲染层若只按「首个候选目录 + 文件名」拼展示路径会误导用户（上游 issue #1691）。
   */
  path: string
  /**
   * 与 path 同值的别名。上游 v0.19.26 起新代码读 resolvedPath，我方 path 是 #1691 分叉的必填字段，
   * 主进程两者同时返回，两侧消费方都不用改。
   */
  resolvedPath?: string
}

/** PSD 图层树节点（预览面板与 Agent 工具共用的中性结构；id 是自下而上的路径，如 "2/0"） */
export interface PsdLayerNode {
  id: string
  name: string
  kind: 'group' | 'artboard' | 'pixel' | 'text' | 'adjustment' | 'smart-object' | 'shape'
  hidden: boolean
  /** 0~1 */
  opacity: number
  blendMode: string
...
/** PSD 内联预览请求的可选项：临时改写图层可见性（点眼睛）*/
export interface PsdPreviewOptions {
...
/** PSD 内联预览结果 */
export interface PsdPreviewResult {
  resolvedPath: string
  width: number
  height: number
  /** 合成图（受令牌保护的 canopy-file 地址） */
  compositeUrl: string
  layers: PsdLayerNode[]
  /** 实际产出合成图的引擎：psd-tools（用户机器上有 Python + psd-tools）或内置 ag-psd */
  engine: 'psd-tools' | 'ag-psd'
  colorMode?: string
  warnings?: string[]
}
...
export interface OfficePreviewResult {
   ...
   * `grid`：主进程不产出 HTML，由渲染层后台线程解析、虚拟滚动显示（大表格 / OfficeCLI 截断的表格）。
   */
  renderer?: 'builtin' | 'officecli' | 'grid'
  /** renderer 为 `grid` 时的补充信息 */
  grid?: SpreadsheetGridHint
  text: string
}

export interface SpreadsheetGridHint {
  /** large：行数超过 OfficeCLI 显示上限；truncated：OfficeCLI 已渲染但自报截断 */
  reason: 'large' | 'truncated'
  /** 主进程从工作表 `<dimension>` 读到的最大行数（读不到为 undefined） */
  maxRows?: number
  /** 各工作表的冻结窗格（工作表名 → 冻结的行数 / 列数），xlsx 从 `<pane state="frozen">` 读出 */
