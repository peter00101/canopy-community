...
/** macOS 中用户可选、且当前允许 Canopy 写入的 Calendar / Reminders List。 */
...
/** 同一已连接系统项被 Canopy 与系统并发修改时，必须由用户选择保留哪一侧。 */
export interface PlanningNativeSyncConflict {
  id: string
  /** 已连接集合冲突；受管 Canopy Calendar 冲突则为 profileId。 */
  connectionId?: string
  profileId?: string
  entity: PlanningNativeSyncEntity
  appEntityId: string
...
export type ResolvePlanningNativeSyncConflictInput = { id: string; resolution: 'keep_app' | 'keep_system' }
