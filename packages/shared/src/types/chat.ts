...
export interface FileAttachment {
  ...
  /** 文件大小（字节） */
  size: number
  /** 视觉/视频补齐生成并缓存的文字描述（图片或视频附件）；重放历史时复用，避免重复调用视觉/视频模型 */
 ...
 * 对话轻量索引项
 *
 * 存储在 ~/.canopy/conversations.json 中，
 * 不包含消息列表，用于快速加载对话列表。
 */
...
export const CHAT_IPC_CHANNELS = {
  ...
  /** 解析附件绝对路径（供消息附件点击打开文件预览） */
  RESOLVE_ATTACHMENT_PATH: 'chat:resolve-attachment-path',
  ...
  // 教程（我方保留项：上游 v0.19.26 #1984 删除了整条教程链路，我方不采）
  /** 获取教程内容 */
