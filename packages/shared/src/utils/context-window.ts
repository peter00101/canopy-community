 ...
 * 故不再下发任何 beta。Canopy 统一按模型能力推断窗口，前端展示、后端用量统计和运行时注册
...
/**
 * GPT-6 Astra：与 GPT-5.6 系列统一按 372K 注册。
 *
 * OpenAI 模型页标注的是 1,050,000（2026-09-05 核对），0.18.54 起我方一直取该精确值；
 * 2026-09-08 维护者改口径按 372K：长文场景实际效果明显下降、Codex 渠道在超长上下文下
 * 明显变慢，宁可让自动压缩早触发也不要拖到窗口尾部。上游 v0.19.37（#2010）同此值。
 */
export const CODEX_GPT_6_ASTRA_CONTEXT_WINDOW = CODEX_GPT_56_CONTEXT_WINDOW

/**
 * 为与 ChatGPT Codex 同名的 GPT-5.x / GPT-6 模型返回统一上下文窗口。
 *
...
export function inferCodexAlignedGPT5ContextWindow(modelId: string | undefined): number | undefined {
  ...
  switch (model) {
    ...
    case 'gpt-6-astra': return CODEX_GPT_6_ASTRA_CONTEXT_WINDOW
