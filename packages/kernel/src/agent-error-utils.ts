 ...
 *
 * 这些规则属于 Canopy 产品层，不依赖具体 Agent SDK。
 */

import type { ErrorCode, TypedError } from '@canopy/shared'
import {
...
const FRIENDLY_ERROR_MESSAGES: Array<{ pattern: RegExp; message: string }> = [
  {
    ...
    message: `请检查是否选择了正确的 ${CANOPY_BRAND.productName} 供应渠道和模型`,
  },
  {
...
export function mapAgentErrorToTypedError(
  ...
  const isInvalidChannelOrModel = mapped.message === `请检查是否选择了正确的 ${CANOPY_BRAND.productName} 供应渠道和模型`
