...
import type { RuntimeStatus, WindowsShellPreference } from '@canopy/shared'
...
export interface BuildAgentRuntimeEnvOptions {
  ...
  /**
   * 已打包 CLI 的绝对路径。内核层不知道如何定位应用资源目录（那是宿主
   * app 的打包/分发细节），调用方必须显式传入；不再有内部默认值。
   */
...
function collectWindowsShellEnv(
  ...
  if (shellKind === 'wsl' && shellStatus?.wsl.available) {
    ...
    env.CANOPY_WINDOWS_SHELL = 'wsl'
    env.SHELL = wslCommand
    if (shellStatus.wsl.defaultDistro) {
      env.CANOPY_WSL_DISTRO = shellStatus.wsl.defaultDistro
    }
    return {
      env,
      shellKind: 'wsl',
  ...
  if (shellKind === 'git-bash' && shellStatus?.gitBash.path) {
    ...
    env.CANOPY_WINDOWS_SHELL = 'git-bash'
    env.SHELL = shellPath
    return {
      env,
      shellKind: 'git-bash',
...
export function buildAgentRuntimeEnv(options: BuildAgentRuntimeEnvOptions = {}): AgentRuntimeEnv {
  ...
  const bundledCliPath = options.bundledCliPath
  const env: Record<string, string> = {}

  if (bundledCliPath) {
    env.CANOPY_CLI = bundledCliPath
  }
